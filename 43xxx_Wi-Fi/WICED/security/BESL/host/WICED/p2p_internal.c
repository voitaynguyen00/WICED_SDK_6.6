...
static besl_result_t p2p_start_client( p2p_workspace_t* workspace )
{
    ...
    host_rtos_delay_milliseconds( 10 ); /* Delay to allow BA window size sets to complete */
    ...
    else
    {
        ...
        host_rtos_delay_milliseconds( 300 );/* wait 300ms for GO moves to operating channel */
...
static besl_result_t p2p_group_owner_stop( p2p_workspace_t* workspace )
{
    ...
    *data = wwd_get_bss_index( WWD_P2P_INTERFACE );
    ...
    /* Set BSS down before removing the p2p interface. */
    ...
    if ( ( result != WWD_SUCCESS ) &&
         ( result != WWD_WLAN_NOTFOUND ) )
    {
        BESL_ERROR( ("p2p can't stop GO\n") );
        /* Ignore the error and proceed to delete p2p interface. */
    }
