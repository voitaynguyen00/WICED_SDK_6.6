...
static besl_result_t        besl_supplicant_management_set_event_handler( supplicant_workspace_t* workspace, wiced_bool_t enable );
...
extern void           besl_supplicant_set_identity        ( supplicant_workspace_t* workspace, const uint8_t* eap_identity, uint32_t eap_identity_length );
extern void           besl_supplicant_set_inner_identity  ( supplicant_workspace_t* workspace, eap_type_t eap_type, void* inner_identity );
extern besl_result_t  supplicant_tls_agent_finish_connect ( supplicant_workspace_t* workspace );
extern besl_result_t  supplicant_process_event            ( supplicant_workspace_t* workspace, besl_event_message_t* message);
extern wiced_result_t wiced_supplicant_enable_tls         ( supplicant_workspace_t* supplicant, void* context );
extern wiced_result_t wiced_supplicant_start_tls          ( supplicant_workspace_t* supplicant, wiced_tls_endpoint_type_t type, wiced_tls_certificate_verification_t verification );
extern besl_result_t  supplicant_process_peap_event       ( supplicant_workspace_t* workspace, besl_packet_t packet);
extern besl_result_t  supplicant_process_ttls_phase2_event( supplicant_workspace_t* workspace, besl_packet_t packet);
extern void           supplicant_eap_handshake_cleanup    ( supplicant_workspace_t* workspace );
...
besl_result_t besl_supplicant_init(supplicant_workspace_t* workspace, supplicant_connection_info_t *conn_info)
{
    ...
    {
        wiced_result = wiced_tls_init_identity( conn_info->tls_identity, NULL, 0, NULL, 0 );
    }
    else if ( conn_info->eap_type == EAP_TYPE_TLS || ( conn_info->eap_type == EAP_TYPE_TTLS && conn_info->is_client_cert_required ) )
    {
        wiced_result = wiced_tls_init_identity( conn_info->tls_identity, (char*) conn_info->private_key, conn_info->key_length, conn_info->user_cert, conn_info->user_cert_length );
    }
    ...
    wiced_result = wiced_tls_init_context( conn_info->context, conn_info->tls_identity, NULL );
    if ( wiced_result != WICED_SUCCESS )
    {
        result = BESL_ERROR;
    ...
    wwd_result = host_rtos_init_queue( &supplicant_host_workspace->host_workspace.event_queue, supplicant_host_workspace->host_workspace.event_buffer, sizeof( supplicant_host_workspace->host_workspace.event_buffer ), sizeof(besl_event_message_t) );
    ...
    wwd_result = host_rtos_init_queue( &supplicant_host_workspace->outgoing_packet_queue, supplicant_host_workspace->outgoing_packet_buffer, sizeof( supplicant_host_workspace->outgoing_packet_buffer ), sizeof(besl_event_message_t) );
    if ( wwd_result != WWD_SUCCESS )
    {
        result = BESL_ERROR;
    ...
    besl_supplicant_set_identity( workspace, conn_info->eap_identity, strlen( (char*) conn_info->eap_identity ) );
    ...
    if ( conn_info->eap_type == EAP_TYPE_PEAP || conn_info->eap_type == EAP_TYPE_TTLS )
    {
        ...
        for ( i = 0; i <= strlen( (char*) conn_info->password ); i++ )
        {
            *unicode++ = *password++;
            *unicode++ = '\0';
        }
    ...
    SUPPLICANT_DEBUG( ( "set supplicant event handlers\n" ) );
    ...
    EXIT_FROM_DEINIT_HOST_WORKSPACE_OUTGOING_PACKET_QUEUE:
        SUPPLICANT_DEBUG( ("Deinitializing host workspace outgoing packet queue\n" ) );
        host_rtos_deinit_queue( &supplicant_host_workspace->outgoing_packet_queue );

    EXIT_FROM_DEINIT_HOST_WORKSPACE_EVENT_QUEUE:
        SUPPLICANT_DEBUG( ("Deinitializing host workspace event queue\n" ) );
    ...
    EXIT_FROM_EAPOL_RECV_HANDLER_REGISTRATION:
        SUPPLICANT_DEBUG( ("Deregister Eapol receive handler\n" ) );
        wwd_eapol_unregister_receive_handler( );

    EXIT_FROM_TLS_INIT_ROOT_CA:
        SUPPLICANT_DEBUG( ("Deinitialize TLS ROOT CA\n" ) );
        wiced_tls_deinit_root_ca_certificates( );

    EXIT_FROM_TLS_INIT_CONTEXT:
        SUPPLICANT_DEBUG( ("Deinitialize TLS context\n" ) );
        wiced_tls_deinit_context( conn_info->context );

    EXIT_FROM_TLS_INIT_IDENTITY:
        SUPPLICANT_DEBUG( ("Deinitialize TLS identity\n" ) );
        wiced_tls_deinit_identity( conn_info->tls_identity );
...
static void* supplicant_external_event_handler( const wwd_event_header_t* event_header, const uint8_t* event_data, /*@returned@*/ void* handler_user_data )
{
    ...
    SUPPLICANT_DEBUG(("[%s()] : L%d : Event type = [%u], Status = [%u], Reason = [%u], Flags = [%u]\n", __FUNCTION__, __LINE__, (unsigned int)event_header->event_type, (unsigned int)event_header->status,
