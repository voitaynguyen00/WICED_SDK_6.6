...
wiced_result_t get_ssl_cache_entry(wiced_ip_address_t *ip_address, uint16_t port, wiced_ssl_cache_entry **entry)
{
    wiced_result_t result;
    wiced_ip_port  ip_port;
    uint32_t count = 0;;

    linked_list_get_count(&wiced_ssl_cache.cache_list, &count);
    if( count == 0 )
    {
...
wiced_result_t add_ssl_cache_entry(wiced_ip_address_t *ip_address, uint16_t port, mbedtls_ssl_session *tls_session)
{
...
int wiced_tls_random ( void *rng_state, unsigned char *output, size_t len );
 ...
 ******************************************************/
static mbedtls_options opt_config;
...
static wiced_result_t tls_init_root_ca_certificates(mbedtls_x509_crt** root_ca_certs, const char* trusted_ca_certificates, const uint32_t length)
{
    int result;
    ...
    *root_ca_certs = tls_host_malloc("tls", sizeof(mbedtls_x509_crt));
    if (*root_ca_certs == NULL)
    {
    ...
    result = mbedtls_x509_crt_parse(*root_ca_certs, (const unsigned char *)trusted_ca_certificates, length);
    if (result != 0)
    {
        WPRINT_SECURITY_DEBUG(("Result from mbedtls_x509_crt_parse is %lx (%ld) (~%lx)\n", (uint32_t)result, (uint32_t)result, ~((uint32_t)result)));
        mbedtls_x509_crt_free(*root_ca_certs);
        tls_host_free(*root_ca_certs);
        *root_ca_certs = NULL;
        return WICED_ERROR;
...
{
    return tls_init_root_ca_certificates(&context->root_ca_certificates, trusted_ca_certificates, length);
}
...
wiced_result_t wiced_generic_start_tls_with_ciphers( wiced_tls_context_t* tls_context, void* referee, wiced_tls_endpoint_type_t type, wiced_tls_certificate_verification_t verification, const cipher_suite_t* cipher_list[], tls_transport_protocol_t transport_protocol )
{
   ...
   opt_config.debug_level = MBEDTLS_DEBUG_LOG_LEVEL;
    ...
    mbedtls_ssl_conf_rng( conf, wiced_tls_random, &tls_context->ctr_drbg );
    ...
    if ( type == WICED_TLS_AS_CLIENT && tls_context->session == NULL && transport_protocol != TLS_EAP_TRANSPORT)
    {
        ...
        memset( &ip_address, 0, sizeof(wiced_ip_address_t));
        wiced_tcp_client_peer( socket, &ip_address, &port );

        entry_found = get_ssl_cache_entry( &ip_address, port, &cache_entry);
...
#if defined(MBEDTLS_SSL_ALPN)
        alp = mbedtls_ssl_get_alpn_protocol( &tls_context->context );
    ...
    do
    {
        if ( type == WICED_TLS_AS_CLIENT && transport_protocol != TLS_EAP_TRANSPORT )
        {
            ...
            if ( tls_session.id_len > 0 )
            {
                ...
                memset( &ip_address, 0, sizeof(wiced_ip_address_t));
                result = wiced_tcp_client_peer( socket, &ip_address, &port );
                if( result != WICED_SUCCESS )
                {
                    wiced_rtos_unlock_mutex( &wiced_ssl_cache.mutex );
                    break;
...
wiced_result_t wiced_tls_deinit_context( wiced_tls_context_t* tls_context )
{
    ...
    if (tls_context->root_ca_certificates != NULL)
    {
        ...
        tls_host_free( tls_context->root_ca_certificates );
...
int wiced_tls_random( void *rng_state, unsigned char *output, size_t len )
{
    return mbedtls_ctr_drbg_random(rng_state, output, len);
}
...
wiced_result_t wiced_tls_encrypt_packet( wiced_tls_workspace_t* workspace, wiced_packet_t* packet )
{
    ...
    if( ( ret = mbedtls_ssl_write_record( workspace, SSL_FORCE_FLUSH ) ) != 0 )
    {
...
wiced_result_t wiced_tls_receive_eap_packet( supplicant_workspace_t* supplicant, besl_packet_t* packet, uint32_t timeout )
{
    ...
    else if ( ( ( supplicant->tls_context->context.in_hdr ) != supplicant->tls_context->context.defragmentation_buffer ) )
    {
        ...
        wiced_buffer_eap_set_data_start( supplicant->tls_context->context.received_packet, data );
        wiced_buffer_eap_set_data_end( supplicant->tls_context->context.received_packet, end_of_data );
...
static int tls_add_alpn_extension( wiced_tls_context_t* context, wiced_tls_extension_t* extension )
{
    ...
    if( ( ret = mbedtls_ssl_conf_alpn_protocols( (mbedtls_ssl_config*) context->context.conf, (const char**) extension->extension_data.alpn_protocol_list ) ) != 0 )
    {
