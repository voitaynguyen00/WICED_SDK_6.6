...
void* p2p_event_handler( const wwd_event_header_t* event_header, const uint8_t* event_data, /*@returned@*/ void* handler_user_data )
{
    ...
    switch ( event_header->event_type )
    {
        ...
        {
            BESL_INFO( ( "status : %d \n", event_header->status) );
            if ( event_header->status == WLC_E_STATUS_SUCCESS ) /* Action frame received ACK */
            {
                if ( workspace->sent_negotiation_confirm == 1 || workspace->sent_invitation_response == 1 )
                {
                    p2p_stop_timer( workspace );
                    workspace->sent_negotiation_confirm = 0;
                    workspace->sent_invitation_response = 0;
            ...
            else if ( workspace->sent_invitation_response == 1 && event_header->status == WLC_E_STATUS_NO_ACK ) /* Action frame packet not ack'd */
            {
                BESL_INFO( ( "%s: no ack for packet \n", __func__) );
                workspace->sent_invitation_response = 0;
             ...
             ( memcmp( &message->fixed.oui, P2P_OUI, 3 ) == 0 ) )
        {
            ...
            switch (message->fixed.p2p_sub_type)
            {
                case P2P_GO_NEGOTIATION_REQUEST:
                    if ( ( workspace->p2p_current_state != P2P_STATE_GROUP_OWNER ) && ( workspace->p2p_current_state != P2P_STATE_GROUP_CLIENT ) )
                    {
                        ...
                        else
                        {
                            ...
                            if ( p2p_tlv != NULL )
                            {
                                /* p2p_tlv->data is uint8_t array, we need to do some stuff to appease the compiler gods */
                                //workspace->group_candidate.p2p_capability = *(uint16_t*)p2p_tlv->data;
                                uint16_t *tlvdata = (uint16_t *)p2p_tlv->data;
                                workspace->group_candidate.p2p_capability = *tlvdata;
                            }
                        }
                ...
                case P2P_INVITATION_REQUEST:
                    ...
                    if ( event_header->event_type != WLC_E_ACTION_FRAME_COMPLETE )
                    {
                        /* Send p2p invitation response again if not ack'd */
                        besl_p2p_send_action_frame( workspace, device, p2p_write_invitation_response, channel, 2 );
                    }
