...
wiced_result_t wiced_time_convert_iso8601_to_utc_ms( wiced_iso8601_time_t* iso8601_time, wiced_utc_time_ms_t *utc_time_ms )
{
    ...
    wiced_utc_time_ms_t utc_time_ms_tmp = 0;

    if ( ( NULL==iso8601_time ) || (NULL==utc_time_ms) )
    {
        return WICED_ERROR;
    }

    /* current datetime values */
    year       = (uint16_t) atoi( iso8601_time->year );
    month      = (uint8_t) atoi( iso8601_time->month );
    day        = (uint8_t) atoi( iso8601_time->day );
    hour       = (uint8_t) atoi( iso8601_time->hour );
    minute     = (uint8_t) atoi( iso8601_time->minute );
    second     = (uint8_t) atoi( iso8601_time->second );
    sub_second = (uint32_t) atol( iso8601_time->sub_second );
    ...
    /* years without leap adj */
    utc_time_ms_tmp = ((uint64_t)year - BASE_UTC_YEAR) * SECONDS_IN_365_DAY_YEAR;

    utc_time_ms_tmp += (uint64_t) number_of_leap_years * SECONDS_IN_A_DAY;

    /* Remember if the current year is a leap year */
    is_a_leap_year = ( IS_LEAP_YEAR( year ) ) ? WICED_TRUE : WICED_FALSE;

    if( (is_a_leap_year) && ( month > 2) )
    {
        utc_time_ms_tmp += SECONDS_IN_A_DAY;
    }

    /* add current day/hour/minute */
    utc_time_ms_tmp +=  (uint64_t)secondsPerMonth[month] + (uint64_t)day*SECONDS_IN_A_DAY +
        (uint64_t)hour*SECONDS_IN_A_HOUR + (uint64_t)minute*SECONDS_IN_A_MINUTE + second;

    /* convert to ms */
    utc_time_ms_tmp = utc_time_ms_tmp *1000;

    utc_time_ms_tmp += sub_second/1000;

    /* copy result */
    *utc_time_ms = utc_time_ms_tmp;
