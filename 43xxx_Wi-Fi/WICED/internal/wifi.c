...
static wiced_bool_t                 too_many_ie_error = WICED_FALSE;
...
wiced_result_t wiced_resume_after_deep_sleep( void )
{
    ...
    WICED_SLEEP_CALL_EVENT_HANDLERS( WICED_DEEP_SLEEP_IS_WARMBOOT( ), WICED_LOW_POWER_DEEP_SLEEP, WICED_SLEEP_EVENT_WLAN_RESUME, &result );
...
wiced_result_t
wiced_get_bss_info(wl_bss_info_t *bi, wwd_interface_t interface)
{
        ...
        if ( wiced_network_is_ip_up( interface ) )
        {
            switch ( interface )
            {
                case WICED_STA_INTERFACE:
                {
                    ...
                    if (wiced_get_bss_info(&bss, WWD_STA_INTERFACE) == WICED_SUCCESS ) {

                        bss.SSID[bss.SSID_len] = 0;
                        wwd_wifi_get_iovar_value( IOVAR_STR_CHANSPEC, &chanspec, WICED_STA_INTERFACE );
                        bandwidth = ( ( chanspec & WL_CHANSPEC_BW_MASK  ) == WL_CHANSPEC_BW_20 ) ? 20 : ((  ( chanspec & WL_CHANSPEC_BW_MASK  ) == WL_CHANSPEC_BW_40 ) ? 40 : 80 );

                        WPRINT_APP_INFO( ( "   SSID        : %s\n", bss.SSID ) );
                        WPRINT_APP_INFO( ( "   Channel     : %d\n", bss.chanspec & 0xff ) );
                        WPRINT_APP_INFO( ( "   Bandwdith   : %d\n", (unsigned int) bandwidth ) );
                ...
                case WICED_AP_INTERFACE:
                {
                    ...
                    if (wiced_get_bss_info(&bss, WWD_AP_INTERFACE) == WICED_SUCCESS ) {
                        bss.SSID[bss.SSID_len] = 0;
                        wwd_wifi_get_iovar_value( IOVAR_STR_CHANSPEC, &chanspec, WICED_AP_INTERFACE );
                        bandwidth = ( ( chanspec & WL_CHANSPEC_BW_MASK  ) == WL_CHANSPEC_BW_20 ) ? 20 : ((  ( chanspec & WL_CHANSPEC_BW_MASK  ) == WL_CHANSPEC_BW_40 ) ? 40 : 80 );
                    ...
                    if ( result == WWD_SUCCESS ) {
                        ...
                        wwd_wifi_get_iovar_value( IOVAR_STR_CHANSPEC, &chanspec, WICED_AP_INTERFACE );
                        WPRINT_APP_INFO( ( "   chanspec    : 0x%x\n", (unsigned int) chanspec & 0x0000ffff ));
                        bandwidth = ( ( chanspec & WL_CHANSPEC_BW_MASK  ) == WL_CHANSPEC_BW_20 ) ? 20 : ((  ( chanspec & WL_CHANSPEC_BW_MASK  ) == WL_CHANSPEC_BW_40 ) ? 40 : 80 );
                        WPRINT_APP_INFO( ( "   SSID        : %s\n", ssid_params->SSID ) );
        ...
        case WLC_E_LINK:
            ...
            else
            {
                ...
                if ( too_many_ie_error )
                {
                    ...
                    handshake_timeout_handler( NULL );

                    /* Clear the error flag */
                    too_many_ie_error = WICED_FALSE;
                }
                else
                {
