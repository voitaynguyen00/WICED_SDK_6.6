...
#define WICED_IPV4_HEADER_OFFSET_DST_IP_ADDR    (3)
...
wiced_result_t wiced_udp_packet_get_info_ext( wiced_packet_t* packet, wiced_ip_address_t* src_address, uint16_t* src_port, wiced_ip_address_t* dst_address )
{
    UINT    result;
    UINT    p;
    ULONG   *ipv4_pkt_header;

    wiced_assert("Bad args", (packet != NULL) && (packet->nx_packet_prepend_ptr != NULL));
    ...
    result = nx_udp_source_extract( packet, (ULONG*) &src_address->ip.v4, &p );
    if ( result != NX_SUCCESS )
    {
        return netx_returns[result];
    }

    src_address->version = WICED_IPV4;
    *src_port = (uint16_t)p;

    /* Get the destination IP address */
    /* Build an address to the current top of the packet.  */
    ipv4_pkt_header =  (ULONG *) packet -> nx_packet_prepend_ptr;

    /* Pickup the destination IP address.  */
    dst_address->ip.v4 =  *(ipv4_pkt_header - WICED_IPV4_HEADER_OFFSET_DST_IP_ADDR);

    /* Determine if IP address is non-zero.  */
    if (dst_address->ip.v4)
    {
        dst_address -> version = WICED_IPV4;
        return WICED_TCPIP_SUCCESS;
    }
    else
    {
        /* Return an invalid packet error.  */
        return WICED_TCPIP_INVALID_PACKET;
    }
}
