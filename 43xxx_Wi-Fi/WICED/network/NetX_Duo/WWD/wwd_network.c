...
static ethernet_filter_tx_packet_t ethernet_tx_packet_callback;
static void* ethernet_tx_packet_userdata;
...
static UINT nx_send_ethernet_packet( NX_IP_DRIVER* driver, USHORT ethertype )
{
    ...
    if (ethernet_tx_packet_callback != NULL)
    {
        ethernet_tx_packet_callback((wiced_buffer_t)packet_ptr, ethernet_tx_packet_userdata);
    }
...
{
    ethernet_tx_packet_userdata = userdata;
    ethernet_tx_packet_callback = tx_packet_callback;
}
