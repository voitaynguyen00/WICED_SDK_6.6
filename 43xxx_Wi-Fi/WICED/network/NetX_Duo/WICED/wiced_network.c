...
#elif defined(WICED_USE_PLATFORM_ALLOCATED_POOL)
static uint8_t* tx_buffer_pool_memory;
static uint8_t* rx_buffer_pool_memory;
#else
...
static void (* const wiced_ip_driver_entries[ 4 ])(struct NX_IP_DRIVER_STRUCT *) =
{
    ...
    [WICED_AP_INTERFACE]  = wiced_ap_netx_duo_driver_entry,  /* This doesn't need to chnage for multiSTA, brru */
...
wiced_result_t wiced_network_init( void )
{
    ...
    tx_buffer_pool_memory = platform_get_tx_buffer_pool(APP_TX_BUFFER_POOL_SIZE + PLATFORM_L1_CACHE_BYTES);

    if( tx_buffer_pool_memory == NULL )
    {
        WPRINT_NETWORK_ERROR(("Couldn't find TX packet pool\n"));
        return WICED_ERROR;
    }

    rx_buffer_pool_memory = platform_get_rx_buffer_pool(APP_TX_BUFFER_POOL_SIZE + PLATFORM_L1_CACHE_BYTES);

    if( rx_buffer_pool_memory == NULL )
    {
        WPRINT_NETWORK_ERROR(("Couldn't find RX packet pool\n"));
        return WICED_ERROR;
    }

    if ( wiced_network_init_packet_pool( &wiced_packet_pools[0], "", tx_buffer_pool_memory, APP_TX_BUFFER_POOL_SIZE + PLATFORM_L1_CACHE_BYTES, WICED_LINK_MTU_ALIGNED ) != NX_SUCCESS )
    {
        WPRINT_NETWORK_ERROR(("Couldn't create TX packet pool\n"));
        return WICED_ERROR;
    }
    if ( wiced_network_init_packet_pool( &wiced_packet_pools[1], "", rx_buffer_pool_memory, APP_RX_BUFFER_POOL_SIZE + PLATFORM_L1_CACHE_BYTES, WICED_LINK_MTU_ALIGNED ) != NX_SUCCESS )
    {
...
wiced_result_t wiced_init_autoipv6( wiced_interface_t interface )
{
    ...
    nxd_ipv6_address_delete(&IP_HANDLE(  interface  ), 0);
    nxd_ipv6_enable( &IP_HANDLE(interface));
  ...
  /* Things need to happen in the following order, so that extra packet loss is not seen.
    ...
    */
    ...
    if ( status == NX_SUCCESS )
    {
        ...
        /* To retain the PM setting before DHCP exchange */
...
static wiced_result_t wiced_network_restore_dhcp_state_from_deep_sleep( wiced_interface_t interface )
{
    ...
    {
        printf("Deep sleep not enabled for this platform; error restoring DHCP state\n");
    }
...
wiced_result_t wiced_wireless_link_down_handler( void* arg )
{
    ...
    if ( DHCP_CLIENT_IS_INITIALISED( interface ) )
    {
        res = nx_dhcp_stop( &DHCP_HANDLE( interface ) );

        if ( ( res != NX_SUCCESS ) && ( res != NX_DHCP_NOT_STARTED ) )
        {
            WPRINT_NETWORK_ERROR((" L%d : %s() : Stop DHCP service failed. Result = [%d]\r\n", __LINE__, __FUNCTION__, res));
            result = WICED_ERROR;
        }
    }

    if ( (res = nx_arp_dynamic_entries_invalidate( &IP_HANDLE(  interface  ) )) != NX_SUCCESS )
    {
        WPRINT_NETWORK_ERROR((" L%d : %s() : Clearing ARP cache failed! Result = [%d]\r\n", __LINE__, __FUNCTION__, res));
        result = WICED_ERROR;
    }
...
wiced_result_t wiced_wireless_link_up_handler( void* arg )
{
    ...
    if ( DHCP_CLIENT_IS_INITIALISED( interface ) )
    {
        ...
        {
            WPRINT_NETWORK_ERROR((" L%d : %s() : Failed to initialize DHCP transaction. Error = [%d]\r\n", __LINE__, __FUNCTION__, res));
            result = WICED_ERROR;
        }
...
wiced_result_t wiced_wireless_link_renew_handler( void* arg )
{
    ...
    if ( (res = nx_arp_dynamic_entry_set( &IP_HANDLE( WICED_STA_INTERFACE ), IP_ADDRESS((unsigned int)((GET_IPV4_ADDRESS(ipv4_address) >> 24) & 0xFF),
    ...
    {
        WPRINT_NETWORK_ERROR((" L%d : %s() : Dynamically changing the ARP entry failed!. Error = [%d]\r\n", __LINE__, __FUNCTION__, res));
        result = WICED_ERROR;
...
static wiced_result_t dhcp_client_init( wiced_interface_t interface, NX_PACKET_POOL* packet_pool )
{
    ...
    WPRINT_NETWORK_INFO((" L%d : %s() : DHCP CLIENT hostname = [%s]\r\n", __LINE__, __FUNCTION__, dhcp_hostname->value));

    /* clear DHCP info to start */
    memset( dhcp_handle, 0, sizeof( *dhcp_handle ) );

    /* Create the DHCP instance. */
    if ( (res = nx_dhcp_create( dhcp_handle, ip_handle, dhcp_hostname->value )) != NX_SUCCESS )
    {
        WPRINT_NETWORK_INFO((" L%d : %s() : Failed to create DHCP instance. Error = [%d]\n", __LINE__, __FUNCTION__, res));
        return WICED_ERROR;
    }
    ...
    /* Start DHCP. */
    if ( (res = nx_dhcp_start( dhcp_handle )) != NX_SUCCESS )
    {
        WPRINT_NETWORK_INFO((" L%d : %s() : Failed to start DHCP transaction. Error = [%d]\r\n", __LINE__, __FUNCTION__, res));
        return WICED_ERROR;
    }
...
#else
#error DHCP option NX_DHCP_CLIENT_USER_CREATE_PACKET_POOL not set!
#endif
