...
#define TCPIP_DEBUG( X )     mini_printf X
#else
#define TCPIP_DEBUG( X )
#endif
...
#define MAX_EVENT_RETRY_COUNT                   (20)
#define EVENT_RETRY_WAIT_IN_MSEC                (2)
...
#define WICED_IPV4_HEADER_OFFSET_DST_IP_ADDR    (4)
#define WICED_IPV6_HEADER_OFFSET_DST_IP_ADDR    (6)
...
static void           internal_wiced_tcp_server_link_up_cbf          ( void *arg);
static void           internal_wiced_tcp_server_link_down_cbf        ( void *arg);

/**
 * IMPORTANT NOTE:
 * ===============
 *   This helper function assumes, mutex associated with tcp-server object is locked before invoking this function.
 *   Please ensure, the mutex is locked before calling this function.
 */
static wiced_result_t internal_wiced_tcp_server_listen               ( wiced_tcp_server_t* tcp_server );

static wiced_result_t internal_wiced_tcp_peer                        ( wiced_tcp_socket_t* socket, wiced_ip_address_t* address, uint16_t* port );
...
wiced_result_t wiced_tcp_server_start( wiced_tcp_server_t* tcp_server, wiced_interface_t interface, uint16_t port, uint16_t max_sockets, wiced_tcp_socket_callback_t connect_callback, wiced_tcp_socket_callback_t receive_callback, wiced_tcp_socket_callback_t disconnect_callback, void* arg )
{
    ...
    TCPIP_DEBUG((" L%d : %s() : --- MU --- Init mutex\r\n", __LINE__, __FUNCTION__));
    res = wiced_rtos_init_mutex(&tcp_server->mutex);
    if ( res != WICED_SUCCESS )
    {
        TCPIP_DEBUG((" L%d : %s() : --- MU --- Failed to init mutex\r\n", __LINE__, __FUNCTION__));
        free(tcp_socket);
        tcp_socket = NULL;
        return res;
    }
    TCPIP_DEBUG((" L%d : %s() : --- MU --- Inited mutex\r\n", __LINE__, __FUNCTION__));

    linked_list_init( &tcp_server->socket_list );

    for ( i = 0; i < max_sockets; i++ )
    {
        res = wiced_tcp_create_socket ( &tcp_socket[i].socket, interface );
        if( res != WICED_SUCCESS )
        {
            goto clean_up;
        }

        TCPIP_DEBUG((" L%d : %s() : Created socket [%p]\r\n", __LINE__, __FUNCTION__, (void*)&tcp_socket[i].socket));

        res = wiced_tcp_register_callbacks( &tcp_socket[i].socket, connect_callback, receive_callback, disconnect_callback, arg );
        if( res != WICED_SUCCESS )
        {
            goto clean_up;
        }
    ...
    /* Register for link update callbacks */
    TCPIP_DEBUG((" L%d : %s() : Register link cbf. TCP server = [%p]\r\n", __LINE__, __FUNCTION__, tcp_server));
    res = wiced_network_register_link_callback(internal_wiced_tcp_server_link_up_cbf, tcp_server, internal_wiced_tcp_server_link_down_cbf, tcp_server, tcp_server->interface);
    if (res != WICED_SUCCESS)
    {
        TCPIP_DEBUG((" L%d : %s() : Failed to register link cbf. Error = [%d]\r\n", __LINE__, __FUNCTION__, res));
        goto clean_up;
    }

    /* start server listen */
    TCPIP_DEBUG((" L%d : %s() : Listen on socket [%p]\r\n", __LINE__, __FUNCTION__, (void*)&tcp_socket[0].socket));
    res = netx_returns[ nx_tcp_server_socket_listen( tcp_socket[0].socket.socket.nx_tcp_socket_ip_ptr, tcp_server->port,  &tcp_socket[0].socket.socket, PERSISTANT_TCP_SERVER_TCP_LISTEN_QUEUE_SIZE, internal_nx_tcp_listen_callback ) ];
    if( res != WICED_SUCCESS )
    {
        goto clean_up;
    }
...
clean_up:
    if ( tcp_socket != NULL )
    {
        for ( i = 0; i < max_sockets; i++ )
        {
            wiced_tcp_delete_socket( &tcp_socket[i].socket );
        }
        linked_list_deinit( &tcp_server->socket_list );
        free( tcp_socket );
    }

    TCPIP_DEBUG((" L%d : %s() : --- MU --- DeInit mutex\r\n", __LINE__, __FUNCTION__));
    wiced_rtos_deinit_mutex(&tcp_server->mutex);
    TCPIP_DEBUG((" L%d : %s() : --- MU --- DeInited mutex\r\n", __LINE__, __FUNCTION__));
...
wiced_result_t wiced_tcp_server_accept( wiced_tcp_server_t* tcp_server, wiced_tcp_socket_t* socket )
{
    ...
    TCPIP_DEBUG((" L%d : %s() : Result of wiced_tcp_accept. Result = [%d]\r\n", __LINE__, __FUNCTION__, result));

    if ( result == WICED_SUCCESS )
    {
        TCPIP_DEBUG((" L%d : %s() : --- MU --- Lock the mutex\r\n", __LINE__, __FUNCTION__));
        wiced_rtos_lock_mutex(&tcp_server->mutex);
        TCPIP_DEBUG((" L%d : %s() : --- MU --- Locked the mutex\r\n", __LINE__, __FUNCTION__));
        ...
        {
            TCPIP_DEBUG((" L%d : %s() : Identify the new socket to listen for incomming connection request\r\n", __LINE__, __FUNCTION__));
            result = internal_wiced_tcp_server_listen( tcp_server );
            TCPIP_DEBUG((" L%d : %s() : Result of internal_wiced_tcp_server_listen. Result = [%d]\r\n", __LINE__, __FUNCTION__, result));
        }
        TCPIP_DEBUG((" L%d : %s() : --- MU --- UnLock the mutex\r\n", __LINE__, __FUNCTION__));
        wiced_rtos_unlock_mutex(&tcp_server->mutex);
        TCPIP_DEBUG((" L%d : %s() : --- MU --- UnLocked the mutex\r\n", __LINE__, __FUNCTION__));
    }
    else
    {
        TCPIP_DEBUG((" L%d : %s() : Relisten on the socket [%p]\r\n", __LINE__, __FUNCTION__, (void *)socket));
        nx_tcp_server_socket_unaccept( &socket->socket );
        nx_result = nx_tcp_server_socket_relisten( socket->socket.nx_tcp_socket_ip_ptr, tcp_server->port, &socket->socket );
        result = netx_returns[ nx_result ];
        TCPIP_DEBUG((" L%d : %s() : Result of relisten. Result = [%d], nx-result = [%d]\r\n", __LINE__, __FUNCTION__, result, nx_result));
        wiced_assert("Re-Listen failed!", nx_result == NX_SUCCESS);
    }

    TCPIP_DEBUG((" L%d : %s() : Processed accepting the connection. Result = [%d]\r\n", __LINE__, __FUNCTION__, result));
    return result;
}
...
wiced_result_t wiced_tcp_server_disconnect_socket_with_timeout( wiced_tcp_server_t* tcp_server, wiced_tcp_socket_t* socket, uint32_t timeout_ms )
{
    ...
    TCPIP_DEBUG((" L%d : %s() : Received disconnect event for socket [%p]\r\n", __LINE__, __FUNCTION__, (void *)socket));
    ...
    TCPIP_DEBUG((" L%d : %s() : Call internal_wiced_tcp_server_listen()\r\n", __LINE__, __FUNCTION__));
    result = internal_wiced_tcp_server_listen( tcp_server );
    TCPIP_DEBUG((" L%d : %s() : Result of internal_wiced_tcp_server_listen = [%d]\r\n", __LINE__, __FUNCTION__, result));
...
wiced_result_t wiced_tcp_server_stop( wiced_tcp_server_t* tcp_server )
{
    ...
    /* De-register link update callbacks */
    TCPIP_DEBUG((" L%d : %s() : De-register link cbf. TCP server = [%p]\r\n", __LINE__, __FUNCTION__, tcp_server));
    result = wiced_network_deregister_link_callback(internal_wiced_tcp_server_link_up_cbf, internal_wiced_tcp_server_link_down_cbf, tcp_server->interface);
    if (result != WICED_SUCCESS)
    {
        TCPIP_DEBUG((" L%d : %s() : Failed to deregister link cbf. Error = [%d]\r\n", __LINE__, __FUNCTION__, result));
    }
...
wiced_result_t wiced_tcp_get_socket_state( wiced_tcp_socket_t* socket, wiced_socket_state_t* socket_state )
{
    ...
    TCPIP_DEBUG((" L%d : %s() : Socket [%p] state = [%d]\r\n", __LINE__, __FUNCTION__, (void *)socket, *socket_state));
...
wiced_result_t wiced_tcp_accept( wiced_tcp_socket_t* socket )
{
    ...
    if ( socket->callbacks.connect == NULL )
    {
        ...
        {
            TCPIP_DEBUG((" L%d : %s() : Failed to accept socket [%p]. nx-error = [%d]\r\n", __LINE__, __FUNCTION__, (void *)socket, net_result));
        ...
        if ( socket->tls_context != NULL )
        {
            ...
            if ( result != WICED_SUCCESS )
            {
                TCPIP_DEBUG((" L%d : %s() : Failed to start TLS for socket [%p]. Error = [%d]\r\n", __LINE__, __FUNCTION__, (void *)socket, result));
                return result;
            }
        }
#endif /* ifndef WICED_DISABLE_TLS */
    ...
    else
    {
        if ( ( socket->socket.nx_tcp_socket_state == NX_TCP_LISTEN_STATE ) || ( socket->socket.nx_tcp_socket_state == NX_TCP_SYN_RECEIVED ) )
        {
            ...
            if ( net_result != NX_SUCCESS )
            {
                TCPIP_DEBUG((" L%d : %s() : Failed to accept socket [%p]. nx-error = [%d]. Unaccepting the socket\r\n", __LINE__, __FUNCTION__, (void *)socket, net_result));
                nx_tcp_server_socket_unaccept( &socket->socket );
                return netx_returns[net_result];
            }
        ...
        TCPIP_DEBUG((" L%d : %s() : error = [%d]\r\n", __LINE__, __FUNCTION__, result));
        return result;
    }

    TCPIP_DEBUG((" L%d : %s() : END of function\r\n", __LINE__, __FUNCTION__));
...
wiced_result_t wiced_udp_packet_get_info_ext( wiced_packet_t* packet, wiced_ip_address_t* src_address, uint16_t* src_port, wiced_ip_address_t* dst_address )
{
    ...
    wiced_assert("Bad args", (packet != NULL) && (packet->nx_packet_ip_header != NULL));

    if( packet == NULL || src_address == NULL || src_port == NULL || dst_address == NULL )
    {
        return WICED_TCPIP_BADARG;
    }

    result = nxd_udp_source_extract( packet, (NXD_ADDRESS*) src_address, &p );
    if ( result != NX_SUCCESS )
    {
        return netx_returns[result];
    }
    *src_port = (uint16_t)p;
    ...
    dst_address -> version = packet -> nx_packet_ip_version;

    if (dst_address -> version == WICED_IPV4)
    {
        ULONG *ipv4_header;

        /* Obtain the IPv4 header. */
        ipv4_header = (ULONG *)packet -> nx_packet_ip_header;
        ipv4_header += WICED_IPV4_HEADER_OFFSET_DST_IP_ADDR;

        /* Pickup the source IP address.  */
        dst_address -> ip.v4 =  *ipv4_header;
    }
    ...
    else
    {
        ...
        ipv6_header = (ULONG*) packet -> nx_packet_ip_header;
        ipv6_header += WICED_IPV6_HEADER_OFFSET_DST_IP_ADDR;

        dst_address->ip.v6[0] = ipv6_header[0];
        dst_address->ip.v6[1] = ipv6_header[1];
        dst_address->ip.v6[2] = ipv6_header[2];
        dst_address->ip.v6[3] = ipv6_header[3];
    }
...
{
    return internal_wiced_tcp_peer( socket, address, port );
}
...
static void internal_nx_tcp_socket_disconnect_callback( NX_TCP_SOCKET* socket_ptr )
{
    ...
    UNUSED_VARIABLE(retry);

    while ( 1 )
    {
        res = internal_defer_tcp_callback_to_wiced_network_thread( socket, socket->callbacks.disconnect );
        if ((res == WICED_SUCCESS) || (++retry >= MAX_EVENT_RETRY_COUNT))
        {
            break;
        }
        /* Failed to post the event to network worker thread */
        TCPIP_DEBUG((" L%d : %s() : Failed in disconnect cbf for TCP socket [%p]. Res = [%d]\r\n", __LINE__, __FUNCTION__, (void *)socket, res));
        /* Wait and retry */
        wiced_rtos_delay_milliseconds(EVENT_RETRY_WAIT_IN_MSEC);
    }

    if (res != WICED_SUCCESS)
    {
        TCPIP_DEBUG((" L%d : %s() : Failed to queue disconnect event for TCP socket [%p] after [%d] retries\r\n", __LINE__, __FUNCTION__, (void *)socket, retry));
    }
}
...
static void internal_nx_tcp_socket_receive_callback( NX_TCP_SOCKET* socket_ptr )
{
    ...
    while ( 1 )
    {
        res = internal_defer_tcp_callback_to_wiced_network_thread( socket, socket->callbacks.receive );
        if ((res == WICED_SUCCESS) || (++retry >= MAX_EVENT_RETRY_COUNT))
        {
            break;
        }
        ...
        /* Failed to post the event to network worker thread */
        TCPIP_DEBUG((" L%d : %s() : Failed in receive cbf for TCP socket [%p]. Res = [%d]\r\n", __LINE__, __FUNCTION__, (void *)socket, res));
        /* Wait and retry */
        wiced_rtos_delay_milliseconds(EVENT_RETRY_WAIT_IN_MSEC);
    }
    ...
    if (res != WICED_SUCCESS)
    {
        TCPIP_DEBUG((" L%d : %s() : Failed to queue receive event for TCP socket [%p] after [%d] retries\r\n", __LINE__, __FUNCTION__, (void *)socket, retry));
    }
}
...
static VOID internal_nx_tcp_listen_callback( NX_TCP_SOCKET* socket_ptr, UINT port )
{
    ...
    UNUSED_VARIABLE(res);
    UNUSED_VARIABLE(nx_result);
    UNUSED_PARAMETER( port );

    res = internal_defer_tcp_callback_to_wiced_network_thread( socket, socket->callbacks.connect );
    if (res != WICED_SUCCESS)
    {
        TCPIP_DEBUG((" L%d : %s() : Listen cbf for socket [%p]. Res = [%d]\r\n", __LINE__, __FUNCTION__, (void *)socket, res));
        /* Couldn't service the connection request due to n/w worker thread queue error. Re-listen on the same socket */
        nx_tcp_server_socket_unaccept( &socket->socket );
        nx_result = nx_tcp_server_socket_relisten( socket->socket.nx_tcp_socket_ip_ptr, port, &socket->socket );
        TCPIP_DEBUG((" L%d : %s() : Rejecting the current listen request from [%p]. Re-listening on the socket. Nx-result = [%d]\r\n", __LINE__, __FUNCTION__, (void *)socket, nx_result));
    }
}
...
static void internal_nx_udp_socket_receive_callback( NX_UDP_SOCKET* socket_ptr )
{
    ...
    if ( socket->socket_magic_number == WICED_SOCKET_MAGIC_NUMBER )
    {
        while ( 1 )
        {
            res = internal_defer_udp_callback_to_wiced_network_thread( socket );
            if ((res == WICED_SUCCESS) || (++retry >= MAX_EVENT_RETRY_COUNT))
            {
                break;
            }
            /* Failed to post the event to network worker thread */
            TCPIP_DEBUG((" L%d : %s() : Failed in Receive cbf for UDP socket [%p]. Res = [%d]\r\n", __LINE__, __FUNCTION__, (void *)socket, res));
            /* Wait and retry */
            wiced_rtos_delay_milliseconds(EVENT_RETRY_WAIT_IN_MSEC);
        }
        ...
        if (res != WICED_SUCCESS)
        {
            TCPIP_DEBUG((" L%d : %s() : Failed to queue receive event for UDP socket [%p] after [%d] retries\r\n", __LINE__, __FUNCTION__, (void *)socket, retry));
        }
    }
}
...
{
    wiced_result_t      res;
    wiced_tcp_server_t* tcp_server = (wiced_tcp_server_t*)arg;
    linked_list_node_t* socket_node = NULL;
    wiced_tcp_socket_t* tcp_socket;
    wiced_tcp_server_socket_t* aligned_serv_sock = NULL;
    UINT                status;

    UNUSED_VARIABLE(res);
    TCPIP_DEBUG((" L%d : %s() : Link up detected. Start listening for new client TCP connection. TCP server = [%p]\r\n", __LINE__, __FUNCTION__, tcp_server));

    TCPIP_DEBUG((" L%d : %s() : --- MU --- Lock the mutex\r\n", __LINE__, __FUNCTION__));
    wiced_rtos_lock_mutex(&tcp_server->mutex);
    TCPIP_DEBUG((" L%d : %s() : --- MU --- Locked the mutex\r\n", __LINE__, __FUNCTION__));

    /* Get the first TCP socket */
    res = linked_list_get_front_node( &tcp_server->socket_list, &socket_node );
    if (res != WICED_SUCCESS)
    {
        TCPIP_DEBUG((" L%d : %s() : No socket found in the list. Something wrong. Error = [%d]\r\n", __LINE__, __FUNCTION__, res));
        goto unlock_and_exit;
    }
    aligned_serv_sock = (wiced_tcp_server_socket_t*)socket_node->data;
    if (aligned_serv_sock == NULL)
    {
        TCPIP_DEBUG((" L%d : %s() : Node data of the socket list can't be 'null'. Something wrong.\r\n", __LINE__, __FUNCTION__));
        goto unlock_and_exit;
    }
    tcp_socket = &aligned_serv_sock->socket;

    /* Re-listen on the socket for incoming connection request */
    status = nx_tcp_server_socket_relisten( tcp_socket->socket.nx_tcp_socket_ip_ptr, tcp_server->port, &tcp_socket->socket );
    if ( status == NX_ALREADY_BOUND )
    {
        TCPIP_DEBUG((" L%d : %s() : Relisten failed during the 1st attempt for socket [%p]\r\n", __LINE__, __FUNCTION__, (void*)tcp_socket));
        nx_tcp_server_socket_unaccept( &tcp_socket->socket );
        TCPIP_DEBUG((" L%d : %s() : Retrying relisten for socket [%p]\r\n", __LINE__, __FUNCTION__, (void*)tcp_socket));
        status = nx_tcp_server_socket_relisten( tcp_socket->socket.nx_tcp_socket_ip_ptr, tcp_server->port, &tcp_socket->socket );
    }
    TCPIP_DEBUG((" L%d : %s() : Link up handled\r\n", __LINE__, __FUNCTION__));
...
static void internal_wiced_tcp_server_link_down_cbf( void *arg)
{
    ...
    TCPIP_DEBUG((" L%d : %s() : Link down detected. Disconnect all the TCP sockets. TCP server = [%p]\r\n", __LINE__, __FUNCTION__, tcp_server));
    linked_list_get_front_node( &tcp_server->socket_list, &socket_node );

    /* Disconnect all the TCP sockets */
    while( socket_node != NULL )
    {
        aligned_serv_sock = (wiced_tcp_server_socket_t*)socket_node->data;
        tcp_socket = &aligned_serv_sock->socket;
        TCPIP_DEBUG((" L%d : %s() : Disconnect Socket [%p]\r\n", __LINE__, __FUNCTION__, (void *)tcp_socket));
    ...
    wiced_rtos_unlock_mutex(&tcp_server->mutex);
    TCPIP_DEBUG((" L%d : %s() : --- MU --- UnLocked the mutex\r\n", __LINE__, __FUNCTION__));
    TCPIP_DEBUG((" L%d : %s() : Link down handled\r\n", __LINE__, __FUNCTION__));
}
...
static wiced_result_t internal_wiced_tcp_server_listen( wiced_tcp_server_t* tcp_server )
{
    ...
    TCPIP_DEBUG((" L%d : %s() : Find the free socket for listening\r\n", __LINE__, __FUNCTION__));
    ...
    while( socket_node != NULL )
    {
        ...
        {
            TCPIP_DEBUG((" L%d : %s() : Socket [%p] is already in listen state\r\n", __LINE__, __FUNCTION__, (void *)tcp_socket));
            return WICED_SUCCESS;
        }
        ...
        {
            TCPIP_DEBUG((" L%d : %s() : Socket [%p] is in close-wait state. Disconnect and unaccept it\r\n", __LINE__, __FUNCTION__, (void *)tcp_socket));
            nx_tcp_socket_disconnect( &tcp_socket->socket, NX_NO_WAIT );
            nx_tcp_server_socket_unaccept( &tcp_socket->socket );
            free_socket = tcp_socket;
        }
    ...
    if ( free_socket == NULL )
    {
        TCPIP_DEBUG((" L%d : %s() : No free socket found. Looks like all the sockets are currently in use\r\n", __LINE__, __FUNCTION__));
        return WICED_ERROR;
    }

    TCPIP_DEBUG((" L%d : %s() : Relisten on the socket [%p]\r\n", __LINE__, __FUNCTION__, (void *)free_socket));
    ...
    TCPIP_DEBUG((" L%d : %s() : Relisten status [%d] for socket [%p]\r\n", __LINE__, __FUNCTION__, status, (void*)free_socket));
