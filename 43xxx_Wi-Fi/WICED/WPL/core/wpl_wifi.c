...
static uint32_t saved_tx_frag;
static uint32_t saved_rx_frag;
...
void wpl_wifi_update_power_data(void)
{
    ...
    uint32_t txfrag = 0, rxfrag = 0;
    uint32_t ampdu_tx_frame_cnt = 0, ampdu_rx_frame_cnt = 0;
    uint32_t log_txfrag = 0, log_rxfrag = 0;
    ...
    {
       WPRINT_APP_DEBUG( ( "WPL: Couldn't get PM mode \n" ) );
       return;
    }

    /* Put interface in higher 4 bit and power mode in lower 4 bits */
    pm_mode |= (interface << 4);
    ...
    /* Capture control and management frames count */
    if( wwd_get_txrx_frag( &txfrag, &rxfrag )  )
          return;
    WPRINT_APP_DEBUG( ( "ampdu_tx_frame_cnt : [%u] ampdu_rx_frame_cnt : [%u]\n", (unsigned int)ampdu_tx_frame_cnt,(unsigned int)ampdu_rx_frame_cnt ) );
    WPRINT_APP_DEBUG( ( "txfrag : [%u] rxfrag : [%u] \n", (unsigned int)txfrag,(unsigned int)rxfrag ) );
    WPRINT_APP_DEBUG( ( "saved_tx_frag : [%u] saved_rx_frag : [%u] \n", (unsigned int)saved_tx_frag,(unsigned int)saved_rx_frag ) );
    WPRINT_APP_DEBUG( ( "( ( txfrag - saved_tx_frag - ampdu_tx_frame_cnt ) << 16 ) | ( rxfrag - saved_rx_frag - ampdu_rx_frame_cnt ) : [%u] \n", (unsigned int)(( ( txfrag - saved_tx_frag - ampdu_tx_frame_cnt ) << 16 ) | ( rxfrag - saved_rx_frag - ampdu_rx_frame_cnt ) ) ) );

    log_txfrag = ((txfrag - saved_tx_frag) < ampdu_tx_frame_cnt) ? 0 : (txfrag - saved_tx_frag) - ampdu_tx_frame_cnt;
    log_rxfrag = ((rxfrag - saved_rx_frag) < ampdu_rx_frame_cnt) ? 0 : (rxfrag - saved_rx_frag) - ampdu_rx_frame_cnt;

    cpl_log_update( EVENT_PROC_ID_WIFI, EVENT_ID_WIFI_DATA, EVENT_DESC_WIFI_TXRX_FRAG,  ( log_txfrag << 16 ) |  log_rxfrag ) ;

    saved_tx_frag = txfrag;
    saved_rx_frag = rxfrag;
}
...
void wpl_wifi_reset_power_data(void)
{
    ...
    saved_tx_frag = txfrag;
    saved_rx_frag = rxfrag;
    if(    wiced_get_default_ready_interface( &interface ) != WICED_SUCCESS)
        return;
    //Clear the dump for all the supported interfaces
    wwd_ampdu_clear_dump( WICED_STA_INTERFACE );
    wwd_ampdu_clear_dump( WICED_AP_INTERFACE );
    return;
}
