...
static wiced_result_t handle_get_target_status_cmd( void );
 ...
 ******************************************************/
extern wpl_semaphore_t          wpl_stdio_uart_rx_semaphore;
static wpl_mutex_t wpl_handle_request_lock;
...
{
    platform_wpl_rtos_init_semaphore( &wpl_stdio_uart_rx_semaphore );

    platform_wpl_rtos_init_mutex( &wpl_handle_request_lock );

    if ( !WPL_DEEP_SLEEP_IS_WARMBOOT_HANDLE( ) )
    {
    ...
    if( platform_wpl_init() != WICED_SUCCESS )
    {
        WPRINT_LIB_ERROR(("wpl_init: Failed to do platform init\n"));
        return WICED_ERROR;
    }
...
void wpl_start( )
{
    ...
    WPRINT_LIB_INFO( ( "CyPE Started....\n" ) );

    if( wpl_init() != WICED_SUCCESS )
    {
        WPRINT_LIB_ERROR(("wpl_start: Failed to init\n"));
        return;
    }
...
wiced_result_t packets_from_wpl_host_handler( wpl_packet_t* packet )
{
    ...
    platform_wpl_rtos_lock_mutex( &wpl_handle_request_lock );
    ...
    platform_wpl_rtos_unlock_mutex( &wpl_handle_request_lock );
...
wiced_result_t handle_get_target_status_cmd( void )
{
    wiced_result_t      result;
    wpl_packet_t*       out_packet;
    ...
    /* Create the wpl response packet. Add one for the length of the payload*/
    result = wpl_dynamic_allocate_packet( &out_packet, WPL_PAD_RESPONSE, CMD_GET_TARGET_STATUS, payload_len + PACKET_COUNT_SIZE );
    if ( result != WICED_SUCCESS )
        return result;

    payload = ( uint32_t )wpl_logging_status( );
...
static wiced_bool_t time_to_send_log_data(void)
{
    ...
    if ( ( platform_wpl_get_time_stamp() - ( log_sent_time_offset + log_start_timestamp ) ) < ( wpl_log_poll_time - (wpl_log_poll_time/POLL_TIME_MARGIN_PERCENTAGE) ) )
    {
...
wiced_result_t handle_request_log_cmd( void )
{
    ...
    current_time_stamp = platform_wpl_get_time_stamp( );
...
wiced_result_t handle_start_log_cmd( wpl_packet_t* request_packet )
{
    ...
    log_start_timestamp = platform_wpl_get_time_stamp( );
...
wiced_result_t wpl_send_power_data_to_host( void )
{
    ...
    return wpl_send_pad_cmd( CMD_LOG_REQUEST );
}
