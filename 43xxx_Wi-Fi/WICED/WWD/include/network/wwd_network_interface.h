...
typedef int (*wwd_transmit_priority_filter_t)( void *context, void *buffer, int buffer_length, int priority );
...
/*
 * Called by a WICED application to register callback function for filter packets to change WiFi transmitting priority.
 *
 * @param context                     : Opaque data pointer to pass back to filter function in callback.
 * @param fn_transmit_priority_filter : Callback function to invoke when there's transmitting frame.
 * @param interface                   : The interface (AP or STA) on which the packet is going out.
 */
extern wwd_result_t wwd_register_transmit_priority_filter( void *context, wwd_transmit_priority_filter_t fn_transmit_priority_filter, wwd_interface_t interface );

/*
 * Called by a WICED application to unregister callback function for filter packets to change WiFi transmitting priority.
 *
 * @param context                     : Opaque data pointer to pass back to filter function in callback.
 * @param fn_transmit_priority_filter : Callback function to invoke when there's transmitting frame.
 * @param interface                   : The interface (AP or STA) on which the packet is going out.
 */
extern wwd_result_t wwd_unregister_transmit_priority_filter( void *context, wwd_transmit_priority_filter_t fn_transmit_priority_filter, wwd_interface_t interface );
