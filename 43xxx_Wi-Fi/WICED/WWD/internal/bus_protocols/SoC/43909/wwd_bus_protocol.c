...
#define PEEK_TX_CREDIT_WAIT_TIME                    (10)    /* 10 ms wait time */
#define FORCE_TX_FLOW_CONTROL_OFF_TIME              (10)    /* 10 mw wait time */
...
static wwd_time_t wifi_last_credit_update                                 = 0;
...
wwd_result_t wwd_bus_read_frame( wiced_buffer_t* buffer )
{
    ...
    else
    {
        *buffer = packet;
        ...
        wifi_last_credit_update = host_rtos_get_time();
        DELAYED_BUS_RELEASE_SCHEDULE( WICED_TRUE );
    }
...
void wwd_wait_for_wlan_event( host_semaphore_type_t* transceive_semaphore )
{
    ...
    if ( wwd_sdpcm_has_tx_packet() == WICED_TRUE )
    {
        if ( m2m_rxactive_dma() == 0 )
        {
            /* No RX buffer in receive descriptor. The TX credit and flow control status
             * can't be updated in this condition. Try to mitigate the situation.
             */
            now = host_rtos_get_time();

            if ( ( wwd_sdpcm_get_available_credits( ) == 0 )&& ( ( now - wifi_last_credit_update ) > PEEK_TX_CREDIT_WAIT_TIME ) )
            {
                /* No TX credit and the credit is not updated for quite a while.
                 * Force adding bus credit to resume TX. This might cause the TX packet to be dropped at WLAN side.
                 */
                wwd_sdpcm_force_add_credit( 1 );
            }
            if ( wwd_bus_is_flow_controlled() && ( ( now - wifi_last_credit_update ) > FORCE_TX_FLOW_CONTROL_OFF_TIME ) )
            {
                /* RX is stopped and flow control is on for quite a while.
                 * Force flow control off.
                 */
                wwd_bus_set_flow_control(0);
            }
        }
    }
    else
