...
/* Maximum WLAN interface number */
#define WWD_WLAN_INTERFACE_MAX              WWD_P2P_INTERFACE
#define WWD_WLAN_INTERFACE_NUMBER           ( WWD_WLAN_INTERFACE_MAX + 1 )

/* IEEE 802.1D user priority (ref: IEEE 802.11/2016 10.2.4.2 EDCA)
 *
 *  Priority    UPs/802.1D Prior    AC          Desigination
 ...
 *  Highest         7               AC_VO       Voice (alternate)
 */
#define WWD_TX_PRIORITY_MIN         0
#define WWD_TX_PRIORITY_MAX         7
...
/* Transmitting priority filter for boosting WiFi priority of selected frames */
typedef struct wwd_tx_priority_filter_s
{
        wwd_transmit_priority_filter_t  fn_priority_filter;
        void*                           context;
} wwd_tx_priority_filter_t;
...
static wwd_tx_priority_filter_t wwd_transmit_priority_filters[WWD_WLAN_INTERFACE_NUMBER] = { { NULL, NULL } };
...
void wwd_network_send_ethernet_data( /*@only@*/ wiced_buffer_t buffer, wwd_interface_t interface ) /* Returns immediately - Wiced_buffer_tx_completed will be called once the transmission has finished */
{
    ...
    if ( wwd_transmit_priority_filters[interface].fn_priority_filter != NULL )
    {
        new_priority = wwd_transmit_priority_filters[ interface ].fn_priority_filter(wwd_transmit_priority_filters[ interface ].context, (uint8_t *)ethernet_header, packet_length, priority );
        if ( new_priority >= WWD_TX_PRIORITY_MIN && new_priority <= WWD_TX_PRIORITY_MAX )
        {
            priority = (uint8_t) new_priority;
        }
    }
...
void wwd_sdpcm_update_credit(uint8_t* data)
{
    ...
    if ( ( header->channel_and_flags & 0x0f ) < (uint8_t) 3 )
    {
        ...
        WPRINT_WWD_INFO(("credit update =%d\n ",header->bus_data_credit) );
...
void wwd_sdpcm_force_add_credit(uint8_t credit)
{
    uint32_t new_credit;

    /* Force add credit if current credit is zero */
    if ( ( wwd_sdpcm_last_bus_data_credit - wwd_sdpcm_packet_transmit_sequence_number ) != 0 )
    {
        /* There's still some TX credit. No force addition. */
        return;
    }
    if ( credit > (uint8_t) CHIP_MAX_BUS_DATA_CREDIT_DIFF ) {
        credit = (uint8_t) CHIP_MAX_BUS_DATA_CREDIT_DIFF;
    }

    new_credit = (wwd_sdpcm_last_bus_data_credit + credit) & 0xff;
    wwd_sdpcm_last_bus_data_credit = (uint8_t) new_credit;
}
    ...
    switch ( packet->sdpcm_header.sw_header.channel_and_flags & 0x0f )
    {
        case CONTROL_HEADER:  /* IOCTL/IOVAR reply packet */
            ...
            if ( id == wwd_sdpcm_requested_ioctl_id )
            {
                ...
                WPRINT_WWD_INFO( ( "Wcd:< Procd pkt 0x%08lX: IOCTL Response (%d bytes)\n", (unsigned long)buffer, size ) );
        ...
        case DATA_HEADER:
            {
                ...
                WPRINT_WWD_INFO( ( "Wcd:< Procd pkt 0x%08lX: Data (%d bytes)\n", (unsigned long)buffer, size ) );
...
wwd_result_t wwd_register_transmit_priority_filter( void *context, wwd_transmit_priority_filter_t fn_transmit_priority_filter, wwd_interface_t interface )
{
    if ( fn_transmit_priority_filter == NULL || interface > WWD_WLAN_INTERFACE_MAX )
        return WWD_BADARG;

    if ( wwd_transmit_priority_filters[ interface ].fn_priority_filter != NULL )
        return WWD_RTOS_ERROR;

    wwd_transmit_priority_filters[ interface ].context = context;
    wwd_transmit_priority_filters[ interface ].fn_priority_filter = fn_transmit_priority_filter;

    return WWD_SUCCESS;
}

wwd_result_t wwd_unregister_transmit_priority_filter( void *context, wwd_transmit_priority_filter_t fn_transmit_priority_filter, wwd_interface_t interface )
{
    if ( fn_transmit_priority_filter == NULL || interface > WWD_WLAN_INTERFACE_MAX )
        return WWD_BADARG;

    if ( wwd_transmit_priority_filters[ interface ].fn_priority_filter != fn_transmit_priority_filter ||
            wwd_transmit_priority_filters[ interface ].context != context )
        return WWD_RTOS_ERROR;

    wwd_transmit_priority_filters[ interface ].fn_priority_filter = NULL;
    wwd_transmit_priority_filters[ interface ].context = NULL;
