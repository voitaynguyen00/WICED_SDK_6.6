...
/* TX buffer filtering support */
static wwd_thread_filter_tx_packet_t wwd_tx_packet_callback;
static void* wwd_tx_packet_userdata;
...
int8_t wwd_thread_send_one_packet( void ) /*@modifies internalState @*/
{
    ...
    /*
     * Do we have a packet filter installed? If so call it now.
     */

    if (wwd_tx_packet_callback)
    {
        wwd_tx_packet_callback(tmp_buf_hnd, wwd_tx_packet_userdata);
    }

    WPRINT_WWD_INFO(("Wcd:> Sending pkt 0x%08lX\n", (unsigned long)tmp_buf_hnd ));
...
int8_t wwd_thread_receive_one_packet( void )
{
    ...
    if ( recv_buffer != NULL )  /* Could be null if it was only a credit update */
    {
        ...
        WPRINT_WWD_INFO(("Wcd:< Rcvd pkt 0x%08lX\n", (unsigned long)recv_buffer ));
...
static void wwd_thread_func( wwd_thread_arg_t /*@unused@*/thread_input ) /*@globals killed wwd_transceive_semaphore@*/ /*@modifies wwd_wlan_status, wwd_bus_interrupt, wwd_thread_quit_flag, wwd_inited, wwd_thread@*/
{
    ...
    WPRINT_WWD_INFO(("Started Wiced Thread\n"));
    ...
    while ( wwd_thread_quit_flag != WICED_TRUE )
    {
        ...
        WPRINT_WWD_INFO(("Wiced Thread: Woke\n"));
    }
    ...
    WPRINT_WWD_INFO(("Stopped Wiced Thread\n"));
...
{
    wwd_tx_packet_userdata = userdata;
    wwd_tx_packet_callback = tx_packet_callback;
}
