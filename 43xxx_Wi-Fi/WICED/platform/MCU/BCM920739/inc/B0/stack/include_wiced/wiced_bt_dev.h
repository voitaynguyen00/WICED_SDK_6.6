...
enum wiced_bt_dev_le_key_type_e
{
    ...
    BTM_LE_KEY_PLK = ( 1 << 3 ), BTM_LE_KEY_LENC = ( 1 << 4 ), /**< master role security information:div */
...
{
BTM_OOB_INVALID_TYPE, BTM_OOB_PEER, /**< Peer OOB data requested */
...
/* The peer_oob_t has been added back in Low-Energy stack for compatibility of BIG app.
...
typedef struct
{
    UINT8 status; /**< connection parameters update status */
    UINT16 handle; /**< connection handle */
    UINT16 conn_interval; /**< updated connection interval */
    UINT16 conn_latency; /**< updated connection latency */
    UINT16 supervision_timeout; /**< updated supervision timeout */
...
BTM_PAIRING_IO_CAPABILITIES_BLE_REQUEST_EVT, /**< Requesting IO capabilities for BLE pairing. Slave can check peer io capabilities in event data before fill local io capabilities. Event data: #wiced_bt_dev_ble_io_caps_req_t */
...
BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT, /**< Updated remote device link keys (store device_link_keys to  NV memory). This is the place to verify that the correct link key has been generated. Event data: #wiced_bt_device_link_keys_t */
...
/* KEEP THIS TO END ONLY */
BTM_LPM_STATE_LOW_POWER = 0xFF /** This value is expected by default WICED SDK apps which is conflicting with BT ROM Code. To avoid compilation error we add this at the end with 0xFF */
...
/**
 * Function         wiced_bt_dev_get_identity_address
 *
 ...
 *
 */
#define wiced_bt_get_identity_address   wiced_bt_dev_get_identity_address
 ...
 *
 * @param[in]       seci_in_pin          :  SECI input pin
 * @param[in]       seci_out_pin        :  SECI output pin
 *
 * @return          wiced_result_t
 *
 **/
wiced_result_t wiced_bt_coex_enable( uint8_t seci_in_pin, uint8_t seci_out_pin );
...
//Allow application/library code to use backward compatible API
#define wiced_bt_set_local_bdaddr(x)  wiced_bt_set_local_bdaddr(x,BLE_ADDR_PUBLIC);
