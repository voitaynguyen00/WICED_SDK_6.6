        ...
        while ( *data_size_left_to_read > 0 )
        {
            ...
            *data_size_left_to_read -= bytes_read;
            ...
            if ( ring_size > 0 )
            {
                ...
                host_rtos_set_semaphore( &driver->rx_complete, WICED_FALSE );
            }
        }
    }
...
static platform_result_t platform_uart_transmit_bytes_irq( platform_uart_driver_t* driver, const uint8_t* data_out, uint32_t size )
{
    ...
    if ( driver->tx_buffer != NULL )
    {
        ...
        while ( size > 0 )
        {
            ...
            sem_result = host_rtos_get_semaphore( &driver->tx_complete, SCB_UART_TX_MAX_WAIT_TIME_MS, WICED_TRUE );
            if ( sem_result != WWD_SUCCESS )
            {
                /* Can't get the semaphore */
                result = ( sem_result == WWD_TIMEOUT ) ? PLATFORM_TIMEOUT : PLATFORM_ERROR;
...
platform_result_t platform_uart_init( platform_uart_driver_t* driver, const platform_uart_t* peripheral, const platform_uart_config_t* config, wiced_ring_buffer_t* optional_ring_buffer )
{
    ...
    if ( platform_peripheral_clock_init( platform_scb->pclk, baudrate_conf->divider, 0 ) != PLATFORM_SUCCESS )
    {
        return PLATFORM_ERROR;
    }
...
WICED_SLEEP_EVENT_HANDLER( platform_uart_sleep_event_handler )
{
    platform_uart_driver_t* driver;
    int port;

    if ( state == WICED_LOW_POWER_DEEP_SLEEP )
    {
        switch ( event )
        {
            case WICED_DEEP_SLEEP_EVENT_ENTER:
                for ( port = 0; port < SCB_UART_PORT_MAX; port++ )
                {
                    driver = scb_uart_driver[ port ];
                    if ( driver != NULL )
                    {
                        if ( WICED_SUCCESS != platform_uart_ok_to_enter_deep_sleep( driver ) )
                        {
                            *result = WICED_ERROR;
                            return;
                        }
