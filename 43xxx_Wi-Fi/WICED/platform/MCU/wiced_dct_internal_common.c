...
void* wiced_dct_get_current_address( dct_section_t section )
{
    ...
    else if ((dct1_sdk_version != DCT_BOOTLOADER_SDK_UNKNOWN) && (dct2_sdk_version != DCT_BOOTLOADER_SDK_UNKNOWN))
    {
        ...
        else if (dct1_sdk_version == dct2_sdk_version)
        {
            ...
            if ((dct1_initial_write != 0) && (dct1_initial_write != 0xFF))
            {
                use_dct1 = WICED_TRUE;
            }
            else if ((dct2_initial_write != 0) && (dct2_initial_write != 0xFF))
            {
                use_dct1 = WICED_FALSE;
            }
...
/*
 * So Far, this is PSoc6 specific code (SECTOR_SIZE_COPY defined in WICED/platform/CMU/PSoC6/WAF/waf_platform.h)
 * However, we do not support allocations in Bootloader or the SFLASH Write application
 */
#if defined(SECTOR_SIZE_COPY) && !defined(BOOTLOADER) && !defined(WICED_SFLASH_WRITE_APP)
static wiced_result_t wiced_dct_copy_sflash( uint32_t dest_loc, uint32_t src_loc, uint32_t size )
{
