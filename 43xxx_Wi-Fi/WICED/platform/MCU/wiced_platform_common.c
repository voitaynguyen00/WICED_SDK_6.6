...
wiced_result_t wiced_gpio_input_get_bool( wiced_gpio_t gpio, wiced_bool_t *value )
{
    ...
    wiced_assert( "bad argument", (value != NULL));
    if (value == NULL)
    {
        return PLATFORM_BADARG;
    }

    gpio_value = (int)platform_gpio_input_get( &platform_gpio_pins[gpio] );
    if ((gpio_value == WICED_FALSE) || (gpio_value == WICED_TRUE))
    {
        *value = (wiced_bool_t)gpio_value;
        return  PLATFORM_SUCCESS;
    }

    return  (platform_result_t)gpio_value;
}
