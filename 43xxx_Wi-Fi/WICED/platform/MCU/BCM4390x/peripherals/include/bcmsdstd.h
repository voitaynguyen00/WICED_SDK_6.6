...
typedef struct sdmmc_data_s {
    uint32 block_size;
    uint32 block_cnt;
    void *data_buf;
    uint32 flags;
} sdmmc_data_t;
#define SDMMC_DATA_IN                   0
#define SDMMC_DATA_OUT                  1
...
struct sdioh_info {
	...
	bool sd_cmd23;          /* SD card support cmd23 */
	sdmmc_data_t *psd_data;
};
...
extern void sdstd_isr_handler(sdioh_info_t *sd);
...
extern int sdstd_interrupt_events_wait(sdioh_info_t *sd, uint16 norm, uint16 err, uint32 timeout, uint32 *events);
extern int sdstd_interrupt_events_set(sdioh_info_t *sd, uint32 events);
