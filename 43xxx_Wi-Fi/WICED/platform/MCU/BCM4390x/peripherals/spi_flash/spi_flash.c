...
int bcm4390x_sflash_read_data( sflash_handle_t* sflash_handle, uint32_t device_address, void* rx_data, uint32_t data_len, read_data_mode_t mode )
{
    ...
    {
        if ( ( semret = PLATFORM_INTERFACE_SEMAPHORE( sflash_handle->hw_interface, SEMAPHORE_GET ) ) != SFLASH_MSG_OK )
        {
...
exit:
    ...
    if ( sflash_handle->read_blocking != WICED_FALSE )
    {
        if ( ( semret = PLATFORM_INTERFACE_SEMAPHORE( sflash_handle->hw_interface, SEMAPHORE_SET ) ) != SFLASH_MSG_OK )
        {
