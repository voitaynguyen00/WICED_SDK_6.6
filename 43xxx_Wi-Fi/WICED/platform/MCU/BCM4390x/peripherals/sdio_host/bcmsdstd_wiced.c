...
struct sdos_info
{
    ...
    wiced_event_flags_t interrupt_events;
};
...
sdstd_osinit(sdioh_info_t *sd)
{
    ...
    wiced_result = wiced_rtos_init_event_flags( &sdos->interrupt_events);
    if (wiced_result != WICED_SUCCESS) {
        host_rtos_deinit_semaphore( &sdos->lock );
        return BCME_ERROR;
    }
...
sdstd_osfree(sdioh_info_t *sd)
{
    ...
    wiced_rtos_deinit_event_flags( &sdos->interrupt_events );
...
int
sdstd_interrupt_events_wait(sdioh_info_t *sd, uint16 norm, uint16 err, uint32 timeout, uint32 *events)
{
    struct sdos_info *sdos = (struct sdos_info *)sd->sdos_info;
    ...
    sd_trace(("%s: int 0x%04x, err 0x%04x, to %u\n", __FUNCTION__, norm, err, timeout));
    ...
    wait_events = (((uint32_t)err) << 16) | ((uint32_t) norm);

    interrupt_events = 0;
    result = wiced_rtos_wait_for_event_flags(&sdos->interrupt_events, wait_events, &interrupt_events, WICED_TRUE, WAIT_FOR_ANY_EVENT, timeout);
    if (result != WICED_SUCCESS) {
        /* Timeout */
        return -1;
    }

    *events = (uint32) interrupt_events;
    return 0;
}

int
sdstd_interrupt_events_set(sdioh_info_t *sd, uint32 events)
{
    struct sdos_info *sdos = (struct sdos_info *)sd->sdos_info;

    wiced_rtos_set_event_flags( &sdos->interrupt_events, events );
    return 0;
}
