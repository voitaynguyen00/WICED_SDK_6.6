...
#define ADMA_BOUNCE_BUFFER_SUPPORT
...
#else
#define SD_PAGE_BITS    8                                       /* 4390x doesn't support paging. Fake the page size to 256 Bytes */
#endif

#define SD_PAGE                                     (1 << SD_PAGE_BITS)
#define SDSTD_MAX_TUNING_PHASE                      5
#define SDSTD_USING_ADMA2_ONLY                                  /* Only use ADMA2, 4390x SD host only support ADMA2. */

/* Limit the length of a ADMA2 buffer to 24K bytes. The HW has problem for length >= 32K Bytes*/
#define SDSTD_ADMA2_BUF_LENGTH_MAX                  (24 * 1024)
#define SDSTD_BLOCK_READ_INIT_TIME                  10          /* 10 ms */
#define SDSTD_BLOCK_WRITE_INIT_TIME                 200         /* 200 ms */
#define SDSTD_BLOCK_READ_WAIT_TIME                  1           /* 1 ms */
#define SDSTD_BLOCK_WRITE_WAIT_TIME                 5           /* 5 ms */
/*
...
#ifdef ADMA_BOUNCE_BUFFER_SUPPORT
#ifndef ADMA_BOUNCE_BUFFER_SIZE
#define ADMA_BOUNCE_BUFFER_SIZE         512        /* Default: 512 Bytes */
#endif  /* ADMA_BOUNCE_BUFFER_SIZE */
#if (ADMA_BOUNCE_BUFFER_SIZE < BLOCK_SIZE_4328)
#error ADMA_BOUNCE_BUFFER_SIZE too small!! Minimum size is 512 bytes.
#endif

static DEFINE_CACHE_LINE_ALIGNED_ARRAY(uint8_t, adma_bounce_buffer, ADMA_BOUNCE_BUFFER_SIZE);
#endif  /* ADMA_BOUNCE_BUFFER_SUPPORT */
...
static inline int sdstd_wait_command_idle(sdioh_info_t *sd, int timeout);
static inline int sdstd_wait_data_idle(sdioh_info_t *sd, int timeout);

static int sdstd_adma_prepare_data(sdioh_info_t *sd, sdmmc_data_t *pdata);
static int sdstd_wait_data_transfer_done(sdioh_info_t *sd, sdmmc_data_t *pdata);
static int sdstd_do_pio_block_read(sdioh_info_t *sd, sdmmc_data_t *pdata);
static int sdstd_do_pio_block_write(sdioh_info_t *sd, sdmmc_data_t *pdata);
...
static inline int sdstd_wait_command_idle(sdioh_info_t *sd, int timeout)
{
    ...
    while (GFIELD(sdstd_rreg(sd, SD_PresentState), PRES_CMD_INHIBIT) && --timeout)
    {
        /* Do nothing */
    }
    if (timeout == 0)
    {
        sd_err(("%s: Wait Command Inhibit timeout\n", __FUNCTION__));
...
static inline int sdstd_wait_data_idle(sdioh_info_t *sd, int timeout)
{
    ...
    while (GFIELD(sdstd_rreg(sd, SD_PresentState), PRES_DAT_INHIBIT) && --timeout)
    {
        /* Do nothing */
    }
    if (timeout == 0)
...
static inline int sdstd_wait_transfer_done(sdioh_info_t *sd, int timeout)
{
    uint32 isr;
    int done;

    do {
        isr = sdstd_rreg(sd, SD_IntrStatus);
        done = GFIELD(isr, INTSTAT_XFER_COMPLETE) != 0;
    } while(!done && --timeout);
    if (!done) {
        sd_err(("Data transfer timeout: intr:0x%x\n", isr));
        return ERROR;
    }

    /* Clear transfer complete interrupt */
    sdstd_wreg16(sd, SD_IntrStatus, isr);
    return SUCCESS;
}
...
static int sdstd_adma_prepare_data(sdioh_info_t *sd, sdmmc_data_t *pdata)
{
    ...
    remain_len = pdata->block_cnt * pdata->block_size;

    if (pdata->flags & SDMMC_DATA_OUT) {
        platform_dcache_clean_range(pdata->data_buf, remain_len);
    } else {
        platform_dcache_clean_and_inv_range(pdata->data_buf, remain_len);
    }
    address = (uint32) pdata->data_buf;
    flags = ADMA2_ATTRIBUTE_VALID | ADMA2_ATTRIBUTE_ACT_TRAN;

    /* The length field of a AMDA2 descriptor is only 16bits which limits
     * a single buffer to 65535 bytes. Our HW has a smaller limit 32767 bytes.
     * Split the incoming buffer to a smaller buffer chain.
     */
    for (i=0; remain_len > 0; i++) {
        trans_len = remain_len;
        if (trans_len > SDSTD_ADMA2_BUF_LENGTH_MAX)
            trans_len = SDSTD_ADMA2_BUF_LENGTH_MAX;
        remain_len -= trans_len;
        if (remain_len == 0)
            flags |= ADMA2_ATTRIBUTE_INT | ADMA2_ATTRIBUTE_END;
        sd_create_adma_descriptor(sd, i, address, trans_len, flags);
        address += trans_len;
    }
    ...
    sdstd_wreg(sd, SD_ADMA_SysAddr, sd->adma2_dscr_phys);
    return SUCCESS;
}
...
print_regs(sdioh_info_t *sd)
{
    ...
    uint16 reg16 = 0;
    uint32 reg32 = 0;
#ifdef SD3_HOST_SUPPORT
    uint8 presetreg;
    int i;
#endif  /* SD3_HOST_SUPPORT */
...
sdstd_cmd_issue(sdioh_info_t *sdioh_info, bool use_dma, uint32 cmd, uint32 arg)
{
    ...
    if (sdstd_wait_command_idle(sdioh_info, RETRIES_SMALL) != 0) {
        if (trap_errs)
            ASSERT(0);
        return ERROR;
    }
    ...
    case SDIOH_CMD_16:
    case SDIOH_CMD_23:    /* Set block count - Response R1 */
    case SDIOH_CMD_55:
        sd_data(("%s: CMD%d\n", __FUNCTION__, cmd));
        cmd_reg = SFIELD(cmd_reg, CMD_RESP_TYPE, RESP_TYPE_48);
        cmd_reg = SFIELD(cmd_reg, CMD_CRC_EN, 1);
    ...
    case SDIOH_ACMD_51:
        ...
        sdstd_wreg16(sdioh_info, SD_BlockSize, SCR_REGISTER_LEN);
    ...
    case SDIOH_CMD_18:
        ...
        xfer_reg = SFIELD(xfer_reg, XFER_DATA_DIRECTION, 1);
        if (use_dma)
            xfer_reg = SFIELD(xfer_reg, XFER_DMA_ENABLE, 1);

        /* Check Data Inhibit before changing data related registers */
        if (sdstd_wait_data_idle(sdioh_info, RETRIES_SMALL) != 0)
            return ERROR;
        break;
    ...
    */
    ...
    if (sdioh_info->psd_data != NULL) {
        sdstd_wreg16(sdioh_info, SD_BlockSize, sdioh_info->psd_data->block_size);
        sdstd_wreg16(sdioh_info, SD_BlockCount, sdioh_info->psd_data->block_cnt);
        if (sdioh_info->psd_data->block_cnt > 1) {
            xfer_reg = SFIELD(xfer_reg, XFER_MULTI_BLOCK, 1);
            xfer_reg = SFIELD(xfer_reg, XFER_BLK_COUNT_EN, 1);
        }
        if (use_dma) {
            sdstd_adma_prepare_data(sdioh_info, sdioh_info->psd_data);
        }
        sdioh_info->psd_data = NULL;
    }
    sdstd_wreg16(sdioh_info, SD_TransferMode, xfer_reg);
...
static int
get_sdmmc_scr(sdioh_info_t *sd)
{
    int status;
    uint32 scr[SCR_REGISTER_LEN/sizeof(uint32)];
    int i;

    /* Read SD CARD Configration Register (SCR). */
    if ((status = sdstd_acmd_issue(sd, USE_DMA(sd), SDIOH_ACMD_51, 0)) != SUCCESS) {
            sd_err(("%s: ACMD51 failed\n", __FUNCTION__));
            return status;
    }

    if (sdstd_wait_transfer_done(sd, RETRIES_LARGE) != SUCCESS) {
        sd_err(("SCR: wait data timeout!!!\n"));
        return ERROR;
    }

    for (i=0; i < sizeof(scr)/sizeof(scr[0]); i++)
        scr[i] = sdstd_rreg(sd, SD_BufferDataPort0) ;

    /* Check if the card supports cmd23 or not */
    sd->sd_cmd23 = SCR_CMD23_SUPPORT(scr[0]);
    sd_info(("SCR: 0x%08x_%08x, cmd23=%d\n", scr[0], scr[1], sd->sd_cmd23));
    return (SUCCESS);
}
...
sdstd_sd_init(sdioh_info_t *sd)
{
    ...
    if ((status = sdstd_sdmmc_set_clock(sd, SDIO_CLK_HI_FREQ)) == FALSE) {
...
sdioh_sdmmc_read_block(sdioh_info_t *sd, uint32_t address, uint8_t* data)
{
    ...
    int status;
    sdmmc_data_t sd_data;
#ifdef ADMA_BOUNCE_BUFFER_SUPPORT
    uint8_t *orig_data_buffer = NULL;
#endif  /* ADMA_BOUNCE_BUFFER_SUPPORT */

    prev = host_rtos_get_time();

    if (USE_DMA(sd) && (PLATFORM_L1_CACHE_LINE_OFFSET(data) != 0))
    {
#ifdef ADMA_BOUNCE_BUFFER_SUPPORT
        /* Read buffer is not cache line aligned. Use bounce buffer to read */
        orig_data_buffer = data;                /* Save caller's buffer address */
        data = adma_bounce_buffer;              /* Switch to bounce buffer */
#else
        /* Buffer is not cacheline aligned. Return fail to avoid cacheline tearing issue. */
        sd_err(("%s: buffer is not cacheline aligned\n", __FUNCTION__));
        return SDIOH_API_RC_FAIL;
#endif  /* ADMA_BOUNCE_BUFFER_SUPPORT */
    }

    sd_data.block_cnt = 1;
    sd_data.block_size = BLOCK_SIZE_4328;
    sd_data.data_buf = (void *) data;
    sd_data.flags = SDMMC_DATA_IN;
    sd->psd_data = &sd_data;
    ...
    if (!USE_DMA(sd)) {
        /* PIO mode */
        if (sdstd_do_pio_block_read(sd, &sd_data) == 0)
            result = SDIOH_API_RC_SUCCESS;
    }
    else
    {
        /* DMA mode*/
        if (sdstd_wait_data_transfer_done(sd, &sd_data) == 0)
        {
#ifdef ADMA_BOUNCE_BUFFER_SUPPORT
            if (orig_data_buffer != NULL)
                memcpy(orig_data_buffer, data, BLOCK_SIZE_4328);
#endif  /* ADMA_BOUNCE_BUFFER_SUPPORT */
            result = SDIOH_API_RC_SUCCESS;
        }
    }
...
sdioh_sdmmc_read_blocks(sdioh_info_t *sd, uint32_t address, uint32_t count, uint8_t* data)
{
    ...
    /* Send command23 if the card support it. */
    if (sd->sd_cmd23)
    {
        status = sdstd_cmd_issue(sd, USE_DMA(sd), SDIOH_CMD_23, count);
        if (status != SUCCESS)
        {
            sd_err(("%s: CMD23 failed\n", __FUNCTION__));
            return SDIOH_API_RC_FAIL;
        }
    }

    sd_data.block_cnt = count;
    sd_data.block_size = BLOCK_SIZE_4328;
    sd_data.data_buf = (void *) data;
    sd_data.flags = SDMMC_DATA_IN;
    sd->psd_data = &sd_data;
    ...
    /* Send com12 to stop data transfer if cmd23 is not supported. */
    if (!sd->sd_cmd23)
    {
        ...
        /* Need to poll command inhibit bit. The host won't generate transfer done interrupt. */
...
sdioh_sdmmc_write_block(sdioh_info_t *sd, uint32_t address, const uint8_t* data)
{
    ...
    {
#ifdef ADMA_BOUNCE_BUFFER_SUPPORT
        /* Source buffer is not cacheline aligned. Copy the write data to bounce buffer */
        memcpy(adma_bounce_buffer, data, BLOCK_SIZE_4328);
        data = adma_bounce_buffer;
#else
        /* Buffer is not cacheline aligned. Return fail to avoid cacheline tearing issue. */
        sd_err(("%s: buffer is not cacheline aligned\n", __FUNCTION__));
        return SDIOH_API_RC_FAIL;
    ...
    if (!USE_DMA(sd))
    {
        if (sdstd_do_pio_block_write(sd, &sd_data) == 0)
            result = SDIOH_API_RC_SUCCESS;
    }
    else
    {
...
void sdstd_isr_handler(sdioh_info_t *sd)
{
    uint16 normal_interrupts;
    uint16 error_interrupts = 0;
    uint32_t events;

    /* XXX: No printf in this function which is running in interrupt level.*/

    normal_interrupts = sdstd_rreg16(sd, SD_IntrStatus);
    normal_interrupts &= sd->intmask;
    if (!normal_interrupts)
        return;
    ...
    /* Ack interrup status */
    sdstd_wreg16(sd, SD_IntrStatus, normal_interrupts);
    if (normal_interrupts & SFIELD(0, INTSTAT_ERROR_INT, 1))
    {
        /* Read error interrupts */
        error_interrupts = sdstd_rreg16(sd, SD_ErrorIntrStatus);
        sdstd_wreg16(sd, SD_ErrorIntrStatus, error_interrupts);
    }
    events = (((uint32_t)error_interrupts) << 16) | ((uint32_t) normal_interrupts);

    sdstd_interrupt_events_set(sd, events);
}
...
static int sdstd_wait_data_transfer_done(sdioh_info_t *sd, sdmmc_data_t *pdata)
{
    ...
    normal_mask = SFIELD(0, INTSTAT_XFER_COMPLETE, 1);
    error_mask = SFIELD(0, ERRINT_DATA_TIMEOUT, 1) | SFIELD(0, ERRINT_DATA_CRC, 1) | SFIELD(0, ERRINT_DATA_ENDBIT, 1);

    if (pdata->flags & SDMMC_DATA_OUT)
    {
        timeout = SDSTD_BLOCK_WRITE_INIT_TIME + SDSTD_BLOCK_WRITE_WAIT_TIME * pdata->block_cnt;
    }
    else
    {
        timeout = SDSTD_BLOCK_READ_INIT_TIME + SDSTD_BLOCK_READ_WAIT_TIME * pdata->block_cnt;
    }

    result = sdstd_interrupt_events_wait(sd, normal_mask, error_mask, timeout, &interrupts);
    if (result != 0)
    {
        interrupts = sdstd_rreg(sd, SD_IntrStatus);
        state = sdstd_rreg(sd, SD_PresentState);
        sd_err(("%s: isr=0x%x, st=0x%x, bkcnt=%u, bksz=%u, flags=0x%x\n", __FUNCTION__, interrupts, state, pdata->block_cnt, pdata->block_size, pdata->flags));
        /* Timeout */
        if (pdata->block_cnt > 1)
        {
            /* Send command12 to stop current transaction. */
            if (SUCCESS != sdstd_cmd_issue(sd, USE_DMA(sd), SDIOH_CMD_12, 0))
            {
                return -1;
            }
            /* Need to poll command inhibit bit. The host won't generate transfer done interrupt. */
            retries = READ_RETRIES;
            while (GFIELD((state = sdstd_rreg(sd, SD_PresentState)), PRES_CMD_INHIBIT) && --retries)
            {
                OSL_DELAY(READ_DELAY);
            }
            if (retries == 0)
            {
                sd_err(("%s: PRES_CMD_INHIBIT is on. st=0x%x\n", __FUNCTION__, state));
                return -1;
            }
        }
        sd_err(("%s: Wait intr error: timeout\n", __FUNCTION__));
        return -1;
    }
    /* Check error interrupts */
    if (((interrupts >> 16) & error_mask) != 0)
    {
        /* Data Error */
        sd_err(("%s: data error: ints=0x%x\n", __FUNCTION__, interrupts));
        return -1;
    }
...
static int sdstd_do_pio_block_read(sdioh_info_t *sd, sdmmc_data_t *pdata)
{
    ...
    pdestination = (uint32 *) pdata->data_buf;
    for (i = 0; i < pdata->block_cnt; i++)
    {
        /* Wait for data ready */
        retries = READ_RETRIES;
        do
        ...
        {
            sd_err(("%s: int_reg = 0x%x, retries = %d\n", __FUNCTION__, int_reg, retries));
            sd_err(("%s: INTSTAT_BUF_READ_READY not ready\n", __FUNCTION__));
            sdioh_sdmmc_transfer_error_check(sd, __FUNCTION__);
            return result;
        }
        int_reg = SFIELD(0, INTSTAT_BUF_READ_READY, 1);
        ...
        for (j = 0; j < pdata->block_size/sizeof(uint32); j++)
        {
            *pdestination++ = sdstd_rreg(sd, SD_BufferDataPort0);
        }
    }
    ...
    {
        sd_err(("%s: int_reg = 0x%x, retries = %d\n", __FUNCTION__, int_reg, retries));
        sd_err(("%s: INTSTAT_XFER_COMPLETE not ready\n", __FUNCTION__));
        sdioh_sdmmc_transfer_error_check(sd, __FUNCTION__);
        return result;
    }
    int_reg = SFIELD(0, INTSTAT_XFER_COMPLETE, 1);
...
static int sdstd_do_pio_block_write(sdioh_info_t *sd, sdmmc_data_t *pdata)
{
    ...
    for (i = 0; i < pdata->block_cnt; i++)
    {
        ...
        if (retries == 0)
        {
            sd_err(("%s: i=%d, int_reg = 0x%x, retries = %d\n", __FUNCTION__, i, int_reg, retries));
            sd_err(("%s: INTSTAT_BUF_WRITE_READY not ready\n", __FUNCTION__));
            sdioh_sdmmc_transfer_error_check(sd, __FUNCTION__);
            return result;
        }
        ...
        for (j = 0; j < pdata->block_size/sizeof(uint32); j++)
        {
            sdstd_wreg(sd, SD_BufferDataPort0, *psource++);
        }

        /* Wait FIFO drain out. INTSTA_BUW_WRITE_READY is set before the FIFO fully drained. */
        ...
        if (retries == 0)
        {
            sd_err(("%s: PRES_DAT_SIGNAL not clear\n", __FUNCTION__));
            return SDIOH_API_RC_FAIL;
        }
    }
