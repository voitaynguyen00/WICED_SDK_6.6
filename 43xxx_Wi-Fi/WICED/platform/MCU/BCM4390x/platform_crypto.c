...
{
    unsigned char aligned_inbuf[HWCRYPTO_BLOCK_SIZE] ALIGNED(HWCRYPTO_ALIGNMENT_BYTES);
    unsigned char aligned_outbuf[HWCRYPTO_BLOCK_SIZE] ALIGNED(HWCRYPTO_ALIGNMENT_BYTES);
    unsigned char *hwcrypto_outbuf = output;
    const unsigned char *hwcrypto_inbuf = input;
    ...
    process_bytes = length < HWCRYPTO_BLOCK_SIZE ? length : HWCRYPTO_BLOCK_SIZE;
    ...
    while ( length != 0 )
    {
        if( !is_buf_aligned( input, HWCRYPTO_ALIGNMENT_BYTES ) )
        {
            memcpy( aligned_inbuf, input, process_bytes);
            hwcrypto_inbuf = aligned_inbuf;
        }
        if( !is_buf_aligned( output, HWCRYPTO_ALIGNMENT_BYTES ) )
        {
        ...
        hw_aes_crypt_cbc( ctx, mode, process_bytes, iv, hwcrypto_inbuf, hwcrypto_outbuf );

        if( !is_buf_aligned( output, HWCRYPTO_ALIGNMENT_BYTES ) )
        {
            memcpy( output, hwcrypto_outbuf, process_bytes);
        }

        input += process_bytes;
        output += process_bytes;
...
int wiced_hw_des_crypt_cbc( wiced_hw_des_context *ctx, int mode, size_t length, unsigned char iv[ 8 ], const unsigned char *input, unsigned char *output )
{
    ...
    while(length != 0 )
    {
        ...
        hw_des_crypt_cbc( ctx, mode, process_bytes, iv, hwcrypto_inbuf, hwcrypto_outbuf );
...
int wiced_hw_des3_crypt_cbc( wiced_hw_des_context *ctx, int mode, size_t length, unsigned char iv[ 8 ], const unsigned char *input, unsigned char *output )
{
    ...
    while(length != 0 )
    {
        ...
        hw_des3_crypt_cbc( ctx, mode, process_bytes, iv, hwcrypto_inbuf, hwcrypto_outbuf );
