...
extern const platform_gpio_t platform_gpio_pins[WICED_GPIO_MAX];
...
WEAK platform_result_t platform_button_get_state( platform_button_t button, wiced_bool_t *value )
{
    wiced_gpio_t gpio_pin;
    wiced_bool_t result;
    ...
    gpio_pin = platform_gpio_buttons[ button ].gpio;
    result = (wiced_result_t)platform_gpio_input_get( &platform_gpio_pins[gpio_pin] );

    /* Only report if no errors */
    if ( (result == WICED_FALSE) || (result == WICED_TRUE))
    {
        /* assume not pressed */
        *value = WICED_FALSE;

        if ( ((result == WICED_FALSE) && (platform_gpio_buttons[ button ].polarity == WICED_ACTIVE_LOW)) ||
             ((result == WICED_TRUE)  && (platform_gpio_buttons[ button ].polarity == WICED_ACTIVE_HIGH)) )
        {
            *value = WICED_TRUE;
        }
