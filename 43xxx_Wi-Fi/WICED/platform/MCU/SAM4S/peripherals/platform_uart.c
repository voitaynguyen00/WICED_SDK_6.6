...
//    /* Initialise TX and RX complete semaphores */
    if ( WWD_SUCCESS != host_rtos_init_semaphore( &driver->tx_dma_complete ) )
    {
        result =  PLATFORM_ERROR;
        goto platform_uart_init_tx_dma_semaphore_init_error;
    }

    if ( WWD_SUCCESS != host_rtos_init_semaphore( &driver->rx_dma_complete ) )
    {
        result = PLATFORM_ERROR;
        goto platform_uart_init_rx_dma_semaphore_init_error;
    }

    /* Set Tx and Rx pin mode to UART peripheral */
    result = platform_gpio_peripheral_pin_init( peripheral->tx_pin, ( IOPORT_MODE_MUX_A | IOPORT_MODE_PULLUP ) );
    if ( PLATFORM_SUCCESS != result )
    {
        goto platform_uart_init_tx_pin_init_error;
    }

    result = platform_gpio_peripheral_pin_init( peripheral->rx_pin, ( IOPORT_MODE_MUX_A | IOPORT_MODE_PULLUP ) );
    if ( PLATFORM_SUCCESS != result )
    {
        goto platform_uart_init_rx_pin_init_error;
    }

    /* Init CTS and RTS pins (if available) */
    if ( peripheral->cts_pin != NULL )
    {
        result = platform_gpio_peripheral_pin_init( peripheral->cts_pin, ( IOPORT_MODE_MUX_A | IOPORT_MODE_PULLUP ) );
        if ( PLATFORM_SUCCESS != result )
        {
            goto platform_uart_init_cts_pin_init_error;
        }
    }

    if ( peripheral->rts_pin != NULL )
    {
        result = platform_gpio_peripheral_pin_init( peripheral->rts_pin, ( IOPORT_MODE_MUX_A | IOPORT_MODE_PULLUP ) );
        if ( PLATFORM_SUCCESS != result )
        {
            goto platform_uart_init_rts_pin_init_error;
        }
    }
    ...
    if ( 0 != usart_init_rs232( peripheral->peripheral, &settings, CPU_CLOCK_HZ ) )
    {
        result = PLATFORM_ERROR;
        goto platform_uart_init_usart_init_rs232_error;
    }
    ...
    /* No error checking in error cleanup functions, as nothing can be done to handle the error */

platform_uart_init_usart_init_rs232_error:
    if ( peripheral->rts_pin != NULL )
    {
        (void) platform_gpio_deinit( peripheral->rts_pin );
    }

platform_uart_init_rts_pin_init_error:
    if ( peripheral->cts_pin != NULL )
    {
        (void) platform_gpio_deinit( peripheral->cts_pin );
    }

platform_uart_init_cts_pin_init_error:
    (void) platform_gpio_deinit( peripheral->rx_pin );

platform_uart_init_rx_pin_init_error:
    (void) platform_gpio_deinit( peripheral->tx_pin );

platform_uart_init_tx_pin_init_error:
    (void) host_rtos_deinit_semaphore( &driver->rx_dma_complete );

platform_uart_init_rx_dma_semaphore_init_error:
    (void) host_rtos_deinit_semaphore( &driver->tx_dma_complete );

platform_uart_init_tx_dma_semaphore_init_error:
    return result;
}
...
platform_result_t platform_uart_deinit( platform_uart_driver_t* driver )
{
    ...
    (void) platform_gpio_deinit( driver->peripheral->tx_pin );
    (void) platform_gpio_deinit( driver->peripheral->rx_pin );

    if ( driver->peripheral->cts_pin != NULL )
    {
        (void) platform_gpio_deinit( driver->peripheral->cts_pin );
    }

    if ( driver->peripheral->rts_pin != NULL )
    {
        (void) platform_gpio_deinit( driver->peripheral->rts_pin );
    }
...
platform_result_t platform_uart_transmit_bytes( platform_uart_driver_t* driver, const uint8_t* data_out, uint32_t size )
{
    ...
    if ( data_out >= (const uint8_t*)RAM_START_ADDR && data_out < (const uint8_t*)RAM_END_ADDR )
    {
        ...
        if ( WWD_SUCCESS != host_rtos_get_semaphore( &driver->tx_dma_complete, NEVER_TIMEOUT, WICED_FALSE ) )
        {
    ...
    else
    {
        while ( size > 0 )
        {
            if ( 0 != usart_putchar( driver->peripheral->peripheral, (uint32_t)*data_out++ ) )
            {
                return PLATFORM_ERROR;
            }
            size--;
...
platform_result_t platform_uart_receive_bytes( platform_uart_driver_t* driver, uint8_t* data_in, uint32_t* expected_data_size, uint32_t timeout_ms )
{
    ...
    if ( driver->rx_ring_buffer != NULL )
    {
        while ( bytes_left != 0 )
        {
            uint32_t transfer_size = MIN( driver->rx_ring_buffer->size / 2, bytes_left );
            ...
            do
            {
                ...
                if ( WICED_SUCCESS != ring_buffer_get_data( driver->rx_ring_buffer, &available_data, &bytes_available ) )
                {
                    result = PLATFORM_ERROR;
                    goto platform_uart_receive_bytes_ring_buffer_get_data_error;
                }
                ...
                if ( WICED_SUCCESS != ring_buffer_consume( driver->rx_ring_buffer, bytes_available ) )
                {
                    result = PLATFORM_ERROR;
                    goto platform_uart_receive_bytes_ring_buffer_consume_error;
                }
            }
...
platform_uart_receive_bytes_ring_buffer_consume_error:
platform_uart_receive_bytes_ring_buffer_get_data_error:
...
void platform_uart_irq( platform_uart_driver_t* driver )
{
    ...
    if ( ( mask & US_IMR_ENDTX ) && ( status & US_CSR_ENDTX ) )
    {
        ...
        (void) host_rtos_set_semaphore( &driver->tx_dma_complete, WICED_TRUE );
    }
    ...
    if ( mask & US_CSR_RXRDY )
    {
        ...
        {
            (void) host_rtos_set_semaphore( &driver->rx_dma_complete, WICED_TRUE );
            driver->rx_transfer_size = 0;
        }
    }
}
