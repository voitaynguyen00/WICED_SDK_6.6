...
void application_thread_main( ULONG thread_input )
{
...
#ifndef WICED_DISABLE_WATCHDOG
    {
        ...
        result = wiced_rtos_create_thread( &system_monitor_thread_handle, RTOS_HIGHEST_PRIORITY, "system monitor", (wiced_thread_function_t)system_monitor_thread_main, SYSTEM_MONITOR_THREAD_STACK_SIZE, NULL );

        if ( WICED_SUCCESS != result )
        {
            WPRINT_WICED_ERROR(("Couldn't create system monitor thread; err = %d\n", (int)result));
        }
        /* try to hobble on */
    }
#endif /* WICED_DISABLE_WATCHDOG */
