...
/* For FreeRTOS-LwIP Higher the Priority Lower the NATIVE Free RTOS priority(9 - x: where 9 is the max priority).
 * The MAX highest priority is 9, hence add smaller offset values for traffic
 * differentiation.use (1) for priority boost (Rx vs Tx) and for offset between AC's.
 */
#define PRIO_BOOST_OF_RX_THREAD_WRT_TX (0)
