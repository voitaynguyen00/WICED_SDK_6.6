...
{
    /* Check if timed event is registered. If registered, the timer would be already running */
    if ( wiced_rtos_is_timer_running( &event_object->timer ) != WICED_SUCCESS )
    {
        /* Timed event is not registered. Hence, don't proceed further. Just return */
        return WICED_ERROR;
    }
