...
sub process_section( $area, $section, $size, $modulefn, $decaddress, $hexaddress )
{
    ...
    elsif ( $modulefn =~ m/\/SPI_Flash_Library/sgi )
    {
        $module = "SPI Flash Library";
            ...
            $modulefn =~ m/\/PSoC6_Peripheral_Drivers\.a/sgi ||
            $modulefn =~ m/\/PSoC6_Peripheral_Libraries\.a/sgi ||
            $modulefn =~ m/\/PSoC6_Peripheral_CapSense\.a/sgi ||
            $modulefn =~ m/\/PSoC6\.a/sgi ||
            $modulefn =~ m/.*cyfitter_cfg\.o/sgi)
    {
