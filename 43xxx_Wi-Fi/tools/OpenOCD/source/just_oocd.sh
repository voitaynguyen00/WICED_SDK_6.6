...
git am ../patches/0001-v0.10.0-dev.patch
git am ../patches/0002-Fix-Cortex-A-step-logic-when-polling-for-halt.patch
git am ../patches/0003-Add-NuttX-RTOS-awareness.patch
git am ../patches/0004-Add-FreeRTOS-Cortex-R-support.patch
git am ../patches/0005-Add-support-for-Cypress-PSoC6-family-of-devices.patch
git am ../patches/0006-Flash-performance-fixes-for-PSoC6.patch
git am ../patches/0007-Add-STM32H7-config-files.patch
git am ../patches/0008-flash-Add-new-stm32h7x-driver-support.patch
...
if [ "$DEBUG_OPENOCD" == "yes" ]; then
    ...
    # This is the default optimization level for OpenOCD
    # configuration.  If the optimization isn't specified
    # the PSoC6/CMSIS-DAP shows issues on Win7-32 when doing
    # a double flash erase.
