...
open class Peripheral : NSObject, CBPeripheralDelegate {
    ...
    @nonobjc open func peripheral(_:CBPeripheral!, didModifyService invalidatedServices:[AnyObject]!) {
        if let delegate = self.readPeripheralDelegate, let services = invalidatedServices as? [CBService] {
    ...
    open func peripheral(_ peripheral:CBPeripheral, didDiscoverServices error:Error?) {
        Logger.debug("Peripheral#didDiscoverServices: \(self.name) error: \(String(describing: error))" as AnyObject)
        if (error == nil) {
            if let _:ReadPeripheralProtocol = self.readPeripheralDelegate, let services = peripheral.services {
                for service:CBService in services {
    ...
    open func peripheral(_:CBPeripheral, didDiscoverIncludedServicesFor service:CBService, error:Error?) {
        Logger.debug("Peripheral#didDiscoverIncludedServicesForService: \(self.name) error: \(String(describing: error))" as AnyObject)
    }
    
    // characteristics
    open func peripheral(_:CBPeripheral, didDiscoverCharacteristicsFor service:CBService, error:Error?) {
        Logger.debug("Peripheral#didDiscoverCharacteristicsForService: \(self.name) error: \(String(describing: error))" as AnyObject)
    ...
    open func peripheral(_:CBPeripheral, didUpdateNotificationStateFor characteristic:CBCharacteristic, error:Error?) {
        Logger.debug("Peripheral#didUpdateNotificationStateForCharacteristic error: \(String(describing: error))" as AnyObject)
    }
    ...
    open func peripheral(_ peripheral:CBPeripheral, didWriteValueFor characteristic:CBCharacteristic, error: Error?) {
        Logger.debug("Peripheral#didWriteValueForCharacteristic error: \(String(describing: error))" as AnyObject)
    ...
    open func peripheral(_:CBPeripheral, didDiscoverDescriptorsFor characteristic:CBCharacteristic, error:Error?) {
        Logger.debug("Peripheral#didDiscoverDescriptorsForCharacteristic error: \(String(describing: error))" as AnyObject)
    }
    
    open func peripheral(_:CBPeripheral, didUpdateValueFor descriptor:CBDescriptor, error:Error?) {
        Logger.debug("Peripheral#didUpdateValueForDescriptor error: \(String(describing: error))" as AnyObject)
    }
    
    open func peripheral(_:CBPeripheral, didWriteValueFor descriptor:CBDescriptor, error:Error?) {
        Logger.debug("Peripheral#didWriteValueForDescriptor error: \(String(describing: error))" as AnyObject)
    }
