...
class WirelessSetupViewController: UIViewController, UITextFieldDelegate{
    ...
    override func viewDidLoad() {
        ...
        DispatchQueue.main.async {
            if let temp = SSIDNetwork().getSSID() {
                self.deviceAP.text = "\u{2022} \(temp)";
                self.ssidName = temp
            }
        }
    }
    ...
    @IBAction func Connect(_ sender: AnyObject) {
        ...
        guard let _ = SSIDNetwork().getSSID() else {
            print("NOT CONNECTED TO NETWORK, NO Network SSID found!")
            return
        }
    ...
    func makeConfirmRequest() {
        ...
        afManager?.request("http://www.wiced.com/confirm-onboarding", method: .put)
            .responseData { response in
                ...
                {
                    print("confirm onboarding success", response.response?.statusCode as Any)
                } else {
                    print("confirm onboarding error", response.response?.statusCode as Any)
                }
        }
    }
...
//                                                             options: [.CaseInsensitiveSearch], range: nil, locale: nil) == .OrderedSame)
    ...
    func makeGetRequest() {
        ...
        var result = 0;
        _ = self.afManager?.request("https://www.wiced.com/config-status", method: .get)
            .responseData {
                response in
                ...
                print ("response value as config\(String(describing: response.value))")
                print("response result value as config\(String(describing: response.result.value))")
                ...
                response.value?.copyBytes(to: uint8Pointer, count: 1)
                print( "array is", uint8Pointer[0])
                if( uint8Pointer[0] == 6 )
                {
                    result = 1
    ...
    func makePostRequest() {
        ...
        let req = self.afManager?.request("https://www.wiced.com/networkconfig", method: .post, parameters: parameters, encoding: JSONEncoding.default, headers: nil)
            .response {
                response in
                print( "status code is: ", response.response?.statusCode as Any)
