 ...
 *
 * 4. For Demo, flash the 20719B1 device which will act as a bulb device
 * - go to the wiced installed folder and issue the following command
 * - .\wiced_tools\ChipLoad\Win32\ChipLoad.exe -BLUETOOLMODE -PORT COMX -BAUDRATE 115200 -MINIDRIVER 20719-B1_Bluetooth\platforms\minidriver-20739B1-uart.hex -BTP 20719-B1_Bluetooth\platforms\CYW920719Q40EVB_01\20719_OCF.btp -CONFIG 43xxx_Wi-Fi\apps\demo\iot_gateway\peerapps\HomeDeviceApps\iwa-exosite_iot_bulb-CYW920719Q40EVB_01-rom-ram-Wiced-release.hex -LOGTO logto.log
 *
 * Cloud host Carriots
 * **************************************
 ...
 *
 * 4. 4. For Demo, flash the 20719B1 device which will act as a bulb device
 * - go to the wiced installed folder and issue the following command
 * - .\wiced_tools\ChipLoad\Win32\ChipLoad.exe -BLUETOOLMODE -PORT COMX -BAUDRATE 115200 -MINIDRIVER 20719-B1_Bluetooth\platforms\minidriver-20739B1-uart.hex -BTP 20719-B1_Bluetooth\platforms\CYW920719Q40EVB_01\20719_OCF.btp -CONFIG 43xxx_Wi-Fi\apps\demo\iot_gateway\peerapps\HomeDeviceApps\iwa-carriots_iot_bulb-CYW920719Q40EVB_01-rom-ram-Wiced-release.hex -LOGTO logto.log
 *
 * Note:
 ...
 * 5. Connect to GATT server using any BLE client App to configure the Access point information.
 * 6. Configure the  WiFi Access point credentials using BLE client app. Then trigger connect (writing 1 to connect characteristic) command from app.
...
void application_start( void )
{
    ...
    /* Gateway app goes into powersave mode very early and does not give chance to WPL host to
     * enable logging. Wait for 10 secs for the host to enable the same.
     */
    WPRINT_APP_INFO( ("Waiting for few seconds for WPL to hook.... \n") );
...
static int wifi_net_up( )
{
    ...
    WPRINT_APP_INFO( ("wiced_network_up success. WiFi Channel: %lu\r\n", sta_channel) );
    return result;
}
...
{
    int i = 0;
    uint8_t* bd = p_status->bd_addr;
    WPRINT_APP_INFO( ( "iot_gateway_conn_up [%X:%X:%X:%X:%X:%X] id:%d\n:", bd[0], bd[1], bd[2], bd[3], bd[4], bd[5], p_status->conn_id) );
...
static wiced_bt_gatt_status_t iot_gateway_gatts_connection_down( wiced_bt_gatt_connection_status_t *p_status )
{
    ...
    for ( i = 0; i < IOT_GATEWAY_GATTS_MAX_CONN; i++ )
    {
        ...
        {
            uint8_t* bd = iot_gateway_devices[i].remote_addr;
            WPRINT_APP_INFO (( "connection_down [%X:%X:%X:%X:%X:%X] conn_id:%d reason:%d\n", bd[0], bd[1], bd[2], bd[3], bd[4], bd[5], p_status->conn_id, p_status->reason ));
