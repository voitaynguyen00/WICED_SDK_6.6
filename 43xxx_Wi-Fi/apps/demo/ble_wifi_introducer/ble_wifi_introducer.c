...
DEFINE_APP_DCT(wiced_bt_device_link_keys_t)
{
       .bd_addr = {0},                                    /**< [in] BD Address of remote */
       .key_data = {0}
};
...
static uint8_t wifi_introducer_char_notify_wifi_join_status                   = 0; // 0 means not configured , 1 means configured
...
attribute_t gatt_user_attributes[] =
{
        ...
        { HANDLE_WIFI_INTRO_SENS_NW_SERVICE_CHAR_NOTIFY_VAL,           sizeof( wifi_introducer_char_notify_wifi_join_status ), &wifi_introducer_char_notify_wifi_join_status },
...
void wifi_introducer_save_link_key( wiced_bt_device_link_keys_t paired_device_keys )
{
    WPRINT_APP_INFO( ("[WiFi Introducer] wifi_introducer_save_link_key\n"));
...
void wifi_introducer_read_link_key( wiced_bt_device_link_keys_t* paired_device_keys )
{
    ...
    memset( paired_device_keys, 0x00, sizeof( wiced_bt_device_link_keys_t) );
    memcpy( paired_device_keys, &wifi_introducer_device_link_keys, sizeof( wiced_bt_device_link_keys_t));
}
...
static void wifi_join_thread(wiced_thread_arg_t arg)
{
    ...
    do
    {
        ...
        if (0 != result_wifi_join)
        {
             ...
             wifi_introducer_char_notify_wifi_join_status = 0;
        ...
        else
        {
            ...
            //writing BLE notification
            wifi_introducer_char_notify_wifi_join_status = 1;
        }
...
static wiced_result_t wifi_introducer_bt_management_callback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
...
#ifndef ENABLE_APP_POWERSAVE
#ifdef WICED_BT_LOW_POWER_API_SUPPORTED
        if ( wiced_bt_dev_get_low_power_mode( ) == WICED_TRUE )
        {
            WPRINT_APP_INFO(("hello_sensor_management_callback disabling sleep \n"));
            wiced_bt_dev_set_low_power_mode(WICED_FALSE);
        }
#endif
#endif
    ...
    {
        wiced_bt_device_link_keys_t key={0};
        WPRINT_APP_INFO( ("[WiFi Introducer] Paired Device Link Keys Request Event\n") );

        wifi_introducer_read_link_key(&key);

        if(memcmp(p_event_data->paired_device_link_keys_request.bd_addr, key.bd_addr, 6))
        {
            WPRINT_APP_INFO(( "BD_ADDR DOES NOT MATCH\n" ));
            result = WICED_BT_ERROR;
        }
        else
        {
            WPRINT_APP_INFO(( "Key retrieval success\n" ));
            memcpy(&p_event_data->paired_device_link_keys_request, &key, sizeof(wiced_bt_device_link_keys_t));
        }
        break;
    }
...
static void wifi_introducer_send_message( void )
{
    ...
    {
        uint8_t *p_attr = &wifi_introducer_char_notify_wifi_join_status;
        WPRINT_APP_INFO(("wifi_introducer_char_notify_wifi_join_status = %d \n", wifi_introducer_char_notify_wifi_join_status));
        wiced_bt_gatt_send_notification( wifi_introducer_state.conn_id, HANDLE_WIFI_INTRO_SENS_NW_SERVICE_CHAR_NOTIFY_VAL, sizeof(wifi_introducer_char_notify_wifi_join_status), p_attr );
    }
}
...
static wiced_bt_gatt_status_t wifi_introducer_gatt_server_connection_down( wiced_bt_gatt_connection_status_t *p_status )
{
     ...
     */
    if ( wifi_introducer_state.flag_stay_connected && (wifi_introducer_char_notify_wifi_join_status == 0 ) )
    {
        result =  wiced_bt_start_advertisements( BTM_BLE_ADVERT_UNDIRECTED_LOW, 0, NULL );
        WPRINT_APP_INFO( ( "wiced_bt_start_advertisements %d\n", result ) );
    }
