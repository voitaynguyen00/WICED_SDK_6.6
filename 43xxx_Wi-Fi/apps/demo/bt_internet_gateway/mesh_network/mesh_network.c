 ...
 * The Application enables Bluetooth Internet Gateway (BIG) as a Mesh Node, there by making it a Mesh Gateway
 * When the application comes up BIG will be ready to be provisioned.The WICED app connects to configured IoT protocol such as REST/MQTT broker (AWS/Bluemix)
 ...
 * 3> A peer Android App can be used to provision & configure mesh devices (such as lights) and Mesh Gateway
 * 4> The peer Android app is located in the peer app section of the app, please refer to readme section of the Android apk
 *
 ...
 *
 *  Note : At the moment only REST transport is supported, MQTT BLUEMIX and AWS support will be added in the next release
 *
...
extern wiced_result_t mesh_send_status(uint8_t* status, int len );
...
wiced_bool_t is_connected = WICED_FALSE;
...
{
    /* Start HTTP server */
    WPRINT_APP_INFO(("wiced_http_server_start\n"));
...
void proxy_send_cb(const uint8_t *packet, uint32_t packet_len)
{
    ...
    val[0] = 0x01;
    mesh_send_status(val, packet_len + 1);
    free(val);
    ptr = NULL;
}
...
void process_connect_network()
{
    WPRINT_APP_INFO(("[%s] \n",__func__));
    is_connected = WICED_TRUE;
    wiced_bt_mesh_proxy_connect(1);
    uint8_t value[] = {0x00,0x01};
    mesh_send_status(value, 2);
}

void process_disconnect_network()
{
    is_connected = WICED_TRUE;
    wiced_bt_mesh_proxy_connect(0);
    uint8_t* value = (uint8_t*)malloc(sizeof(uint8_t ) * 2);
    value[0] = (char)0x00;
    value[1] = (char)0x00;
    mesh_send_status(value, 2);
    free(value);
...
static wiced_result_t mesh_management_cback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    WPRINT_APP_INFO(("mesh_management_cback: %x\n", event ));
    ...
    case BTM_ENABLED_EVT:
        ...
        if(1)
        {
            WPRINT_APP_INFO( ("push NVM chunks " ) );
        ...
        else
        {
            ...
            /* intialize mesh */
            WPRINT_APP_INFO( ("Device is not provisioned push NVM chunks " ) );
            wiced_bt_mesh_init(mesh_provision_end, proxy_send_cb, mesh_write_nvram_data_cb, mesh_status_cb);
        }
...
static wiced_result_t mesh_write_dct( mesh_dct_t* dct_to_write, int id, uint8_t *packet, uint32_t packet_len )
{
  ...
  //  WPRINT_APP_INFO((" *** mesh_write_dct start ***\n"));
   // int j = 0;
    //int val = 0;
...
//        {
//            val = mesh_dct_info.mesh_nv_data[ dct_index ].data[i];
//            WPRINT_APP_INFO(("%d ", val));
//        }

//        for (j = 0 ; j < mesh_dct_info.mesh_nv_data[dct_index].len ; j++)
//        {
//            WPRINT_APP_INFO(( "  %d " , mesh_dct_info.mesh_nv_data[dct_index].data[j] ));
//        }
...
//        for (j = 0 ; j < mesh_dct_info.mesh_nv_data[index].len ; j++)
//        {
//            WPRINT_APP_INFO(( "  %d " , mesh_dct_info.mesh_nv_data[index].data[j] ));
//        }
