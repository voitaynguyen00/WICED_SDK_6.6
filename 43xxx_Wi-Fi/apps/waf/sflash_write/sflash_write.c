 ...
 ******************************************************/
static uint8_t   Rx_Buffer[__JTAG_FLASH_WRITER_DATA_BUFFER_SIZE__];   /* A temporary buffer used for reading data from the Serial flash when performing verification */
 ...
 ******************************************************/
WEAK int sflash_sector_support_erase_range( void );
...
WEAK int sflash_sector_erase_range ( const sflash_handle_t* handle, unsigned long device_address, unsigned long device_address_end );
 ...
 */
    ...
    if ( sflash_sector_support_erase_range() )
    {
       DEBUG_PRINTF(( "Support Range Erase Command!!\n" ));
       mem_read_size =sizeof( Rx_Buffer );
    }
    else
    {
       DEBUG_PRINTF(( "NO Support Range Erase Command!!\n" ));
       mem_read_size = SECTOR_SIZE;
    }
    ...
    while ( 1 == 1 )
    {
        ...
        if ( ( data_transfer.command & MFG_SPI_FLASH_COMMAND_WRITE_ERASE_IF_NEEDED ) != 0 )
        {
            ...
            {
                uint32_t cmp_len;
                if ( 0 != sflash_read( &sflash_handle, pos, Rx_Buffer, (unsigned int) mem_read_size ) )
                {
                ...
                cmp_len = (uint32_t) MIN( (int32_t) ( mem_read_size - compare_start ), size_left_to_compare );
...
#endif /* ALWAYS_ERASE_SECTOR */
                {
                   ...
                   {
                      /* Existing data different */
                      DEBUG_PRINTF(( "Erasing sector range pos:0x%08lX, end_pos: 0x%08lX\n", pos, (pos + cmp_len ) ));

                      if ( 0 != sflash_sector_erase_range( &sflash_handle, pos, ( pos + cmp_len ) ) )
                      {
...
#ifdef VERIFY_ERASE_SECTOR
                    {
                        ...
                        cmp_ptr2 = Rx_Buffer;
#ifdef DEBUG
                        memset(Rx_Buffer,0xa5,mem_read_size);
#endif /* DEBUG */
                        if ( 0 != sflash_read( &sflash_handle, pos, Rx_Buffer, (unsigned int) mem_read_size ) )
                        {
                            /* Verify Error - Chip not read properly */
                        ...
                        end_ptr = &Rx_Buffer[mem_read_size];
                    ...
                    if ( 0 != sflash_write( &sflash_handle, pos, Rx_Buffer, (unsigned int) cmp_len ) )
                    {
        ...
        if ( ( data_transfer.command & MFG_SPI_FLASH_COMMAND_POST_WRITE_VERIFY ) != 0 )
        {
            ...
            while ( pos < data_transfer.size )
            {
                ...
                if ( 0 != memcmp( Rx_Buffer, &data_transfer.data[pos], (size_t) read_size ) )
                {
                    ...
                    DEBUG_PRINTF(( "Verify error - pos: 0x%08lX, read_size: 0x%08lX!\n", (uint32_t) pos, (uint32_t) read_size ));
                    {
                       int i;
                       for ( i = 0; i < read_size; i++ )
                       {
                           if (data_transfer.data[i] != Rx_Buffer[i])
                           {
                              DEBUG_PRINTF(( "data_transfer.data[%d]: 0x%02lX, Rx_Buffer[%d]: 0x%02lX\n",
                                  i, (uint32_t) data_transfer.data[i], i, (uint32_t) Rx_Buffer[i] ));
                           }
                       }
                    }
                    goto back_to_idle;
