...
void application_start( )
{
    ...
    {
        /* We failed to read the DCT !!! Bad news - try to failsafe from the Update
         * If that fails we will do a Factory Reset
         */
    ...
    if (dct_ota2_config.boot_type == OTA2_BOOT_FAILSAFE_UPDATE)
    {
        if (wiced_ota2_image_validate(WICED_OTA2_IMAGE_TYPE_STAGED) == WICED_SUCCESS)
        {
            ...
            if (result == WICED_SUCCESS)
            {
                ...
                /* If this DCT update is interrupted, the DCT will not be updated, boot_type will still be FAILSAFE_UPDATE, we will try again on reboot*/
                if (wiced_dct_write( &dct_ota2_config, DCT_OTA2_CONFIG_SECTION, 0, sizeof(platform_dct_ota2_config_t) ) == WICED_SUCCESS)
                {
                    ...
                    /* If this call fails (or there is a reset during the call), the download may be corrupted and will fail to extract.
                     * In that case, the call to wiced_ota2_image_validate() will fail (above) and we will do a Factory reset on reboot.
                     */
                    if (wiced_ota2_image_update_staged_status(WICED_OTA2_IMAGE_EXTRACT_ON_NEXT_BOOT) != WICED_SUCCESS)
                    {
                        OTA2_FAILSAFE_PRINTF(("Setting ON NEXT BOOT Failed!\r\n"));
                    }
                }
    ...
    OTA2_FAILSAFE_PRINTF(("OTA2_Failsafe Failed !!!\r\n"));
    while ( 1 )
    {
