...
#define SOFTAP_START_BUTTON_MILLISECONDS_MIN     4000           // hold minimum 4 seconds
#define SOFTAP_START_BUTTON_MILLISECONDS_MAX     8000           // release before 8 seconds
#define FACTORY_RESET_BUTTON_MILLISECONDS       PLATFORM_FACTORY_RESET_TIMEOUT  // typically 10 seconds
#else
...
int main( void )
{
    ...
    {
        /* Bad situation - couldn't read DCT, use failsafe */
    ...
    boot.entry_point        = 0;
    if (wiced_dct_read_with_copy( &boot, DCT_INTERNAL_SECTION, OFFSET( platform_dct_header_t, boot_detail ), sizeof(boot_detail_t) ) != WICED_SUCCESS)
    {
        BOOTLOADER_PRINTF(("OTA2 Config DCT read failed, continue on. We will reset if there is a problem and try again.\r\n"));
    }
...
static wiced_result_t load_program( const load_details_t * load_details, uint32_t* new_entry_point )
{
    ...
    if ( load_details->load_once != 0 )
    {
        ...
        result = wiced_dct_write_boot_details( &boot );
    }
