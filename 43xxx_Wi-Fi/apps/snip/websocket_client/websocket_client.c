 ...
 ******************************************************/
/*
 * Keep TX_BUFFER_LENGTH as multiple of RX_BUFFER_LENGTH
 * for testing purpose in the present snip
 */
#define RX_BUFFER_LENGTH     (1000)
#define TX_BUFFER_LENGTH     (3000)
...
static wiced_result_t on_message_websocket_callback( void* websocket, uint8_t* data, uint32_t length, wiced_websocket_frame_type_t type, wiced_websocket_frame_flags_t flags , uint32_t total_payload_length, int is_data_chunked);
static wiced_result_t on_chunked_message_websocket_callback( void* websocket, uint8_t* data, uint32_t length, wiced_websocket_frame_type_t type, wiced_websocket_frame_flags_t flags , uint32_t total_payload_length, int is_data_chunked);
...
static uint8_t * rx_buffer_ptr= NULL;           //ptr for pointing to rx_buffer
static uint8_t * is_chunked_buffer_ptr = NULL;  //ptr to save the data in malloced rx_buffer on every message callback
static uint32_t c_data_received=0;
static wiced_websocket_frame_type_t frame_type_c;
static wiced_websocket_frame_flags_t flags_c;
...
{
    char c= 65; int i;
    for(i=0; i< (TX_BUFFER_LENGTH/RX_BUFFER_LENGTH) ; i++){
         memset(tx_buffer+ (i*RX_BUFFER_LENGTH), c, RX_BUFFER_LENGTH );
         c+=1;
     }
...
void application_start( )
{
    ...
    wiced_websocket_initialise( &websocket, rx_buffer, RX_BUFFER_LENGTH );

    /*register websocket callbacks */
    wiced_websocket_register_callbacks( &websocket, on_open_websocket_callback, on_close_websocket_callback, on_message_websocket_callback, on_error_websocket_callback  , on_chunked_message_websocket_callback);
...
#ifdef USE_WEBSOCKET_SECURE_CLIENT
    WEBSOCKET_SNIPPET_VERIFY( wiced_websocket_secure_connect( &websocket, &websocket_header, &tls_identity, NULL, NULL, (uint16_t)WEBSOCKET_SECURE_SERVER_STD_PORT, WICED_STA_INTERFACE ) );
    ...
    wiced_websocket_send( &websocket, (uint8_t*)"Hi, ", strlen("Hi, "),  WEBSOCKET_TEXT_FRAME, WEBSOCKET_FRAME_FLAG_UNFRAGMENTED );
    wiced_rtos_get_semaphore( &received_message_event, WICED_WAIT_FOREVER );

    wiced_websocket_send( &websocket, (uint8_t*)"I'm a Wiced ", strlen("I'm a Wiced "), WEBSOCKET_TEXT_FRAME, WEBSOCKET_FRAME_FLAG_UNFRAGMENTED );
    wiced_rtos_get_semaphore( &received_message_event, WICED_WAIT_FOREVER );

    wiced_websocket_send( &websocket, (uint8_t*)"Websocket Client!!" , strlen("Websocket Client!!"), WEBSOCKET_TEXT_FRAME, WEBSOCKET_FRAME_FLAG_UNFRAGMENTED );
    wiced_rtos_get_semaphore( &received_message_event, WICED_WAIT_FOREVER );
...
static wiced_result_t on_chunked_message_websocket_callback( void* websocket, uint8_t* data, uint32_t length, wiced_websocket_frame_type_t type, wiced_websocket_frame_flags_t flags , uint32_t total_payload_length, int is_data_chunked){
    ...
    WPRINT_APP_INFO(("[App] On chunked message callback @websocket:%p\n", websocket ) );

   //Use the cached flag and frame type to process the data
   c_data_received += length;
   memcpy(is_chunked_buffer_ptr, data, length);

   if (frame_type_c == WEBSOCKET_BINARY_FRAME && (length !=0) ){
       print_binary_data(is_chunked_buffer_ptr, length);
   }
   else
       WPRINT_APP_INFO( ( "[%d] \t%.*s\r\n", __LINE__, (int)length, (char*)is_chunked_buffer_ptr ) );

   if(c_data_received != total_payload_length){
       is_chunked_buffer_ptr =(uint8_t *) is_chunked_buffer_ptr + length;
   }
   else //if(c_data_received == total_payload_length)
   {
       WPRINT_APP_INFO( ( "Received the complete frame\r\n" ) );
       free(rx_buffer_ptr);
       is_data_chunked =0;  // NR
       rx_buffer_ptr = NULL;
       is_chunked_buffer_ptr = NULL;
   }
...
static wiced_result_t on_message_websocket_callback( void* socket, uint8_t* data, uint32_t length, wiced_websocket_frame_type_t type, wiced_websocket_frame_flags_t flags , uint32_t total_payload_length, int is_data_chunked)
{
    ...
    if(is_data_chunked ==1){
        rx_buffer_ptr  = (uint8_t*)malloc(sizeof(uint8_t )*total_payload_length); //ptr for pointing to rx_buffer
        memcpy(rx_buffer_ptr, data, length);
        is_chunked_buffer_ptr = rx_buffer_ptr + length;
        frame_type_c = type;
        flags_c = flags;
        c_data_received += length;
    }

    //default case where no extra memory is allotted
    switch( type )
    {
