...
static void publisher_observe_results (wiced_bt_ble_scan_results_t *p_scan_result, uint8_t *p_adv_data)
{
    numCalls ++;


    WPRINT_APP_INFO( ( "%s call: %u \n", __FUNCTION__, numCalls ) );



    if (p_scan_result != NULL)
    {
        WPRINT_APP_INFO( ( "BDAddr = %02X %02X %02X %02X %02X %02X RSSI=%i \n", p_scan_result->remote_bd_addr[0], p_scan_result->remote_bd_addr[1],
                p_scan_result->remote_bd_addr[2], p_scan_result->remote_bd_addr[3], p_scan_result->remote_bd_addr[4], p_scan_result->remote_bd_addr[5],
                p_scan_result->rssi));
    }
    else
    {
        WPRINT_APP_INFO( ( "Scan result empty...\n"));
    }
}
...
#endif
            result =  wiced_bt_ble_observe (TRUE, 0, publisher_observe_results);
