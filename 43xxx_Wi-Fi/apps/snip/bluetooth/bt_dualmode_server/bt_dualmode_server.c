 ...
 * 3. On application start the device simultenously starts RFCOMM server and a GATT server and advertises itself as WICED_DUALMODE_SERVER.
 * 4. The RFCOMM server will have the address given in bt_config_dct.h file, while the GATT server will have a different address.
 * 5  Download any Peer SPP phone application which acts as RFCOMM client. This can be used to send serial data.
 * 6. If SPP Peer application not found, Windows BT can be used as a RFCOMM client.
 *   - To use this, establish connection from Windows BT with RFCOMM server.
 *   - Open the COM port using teraterm/putty which enumerates to the BT port of the PC. On the COM port, data can be sent/received.
 * 7. Connect to Proximity reporter using one of the LE clients (LEExplorer(android)) or (BLE Utility(Apple Store))
 * 8. Once connected to a LE client , user can read/write Link Level and Alert levels
 * 9. User can establish simultaneous connections from both the apps
 */
