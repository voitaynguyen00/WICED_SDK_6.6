...
void app_powersave_sleep( void )
{
    ...
    if ( wiced_bt_dev_allow_host_sleep( ) == WICED_TRUE )
    {
        ...
        WPRINT_APP_INFO( ("App going to sleep...\n") );
    }
}
...
static int hello_sensor_send_notification( int argc, char *argv[] )
{
    ...
    {
        WPRINT_APP_INFO( ("Cannot receive, hello sensor not connected\n") );
        return ERR_CMD_OK;
    }
}
...
static int hello_sensor_set_adv_tx_power( int argc, char *argv[] )
{
    ...
    int power_val;
    WPRINT_APP_INFO( ( "hello_sensor_set_adv_tx_power\n" ) );
    if ( !is_connected )
    {
    ...
    {
        WPRINT_APP_INFO( ( "Cannot change ADV tx power, hello sensor already connected\n" ) );
        return ERR_CMD_OK;
    }
}
...
static int hello_sensor_read_adv_tx_power( int argc, char *argv[] )
{
        ...
        {
            WPRINT_APP_INFO( ( "Error in reading ADV TX power\n" ) );
            return ERR_CMD_OK;
        }
}
...
static int hello_sensor_read_rssi( int argc, char *argv[] )
{
    ...
    {
        WPRINT_APP_INFO( ( "Error in reading RSSI value\n" ) );
        return ERR_UNKNOWN;
    }
}
...
static void hello_sensor_tx_power_callback( wiced_bt_tx_power_result_t *p_tx_power )
{
    ...
    {
        WPRINT_APP_INFO( ("ADV TX power: %i\n", p_tx_power->tx_power) );
    }
    else
    {
        WPRINT_APP_INFO( ("Unable to read ADV TX power.(btm_status=0x%x, hci_status=0x%x)\n", p_tx_power->status, p_tx_power->hci_status) );
    }
}
...
static void hello_sensor_read_rssi_callback( wiced_bt_dev_rssi_result_t *p_rssi )
{
    ...
    {
        WPRINT_APP_INFO(("RSSI value: %d\n",p_rssi->rssi));
    }
    else
    {
        WPRINT_APP_INFO( ("Unable to read RSSI value.(btm_status=0x%x, hci_status=0x%x)\n", p_rssi->status, p_rssi->hci_status) );
    }
}
...
static void hello_sensor_application_init( void )
{
    ...
    WPRINT_APP_INFO( ( "hello_sensor_application_init\n" ) );
...
static wiced_result_t hello_sensor_management_callback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    WPRINT_APP_INFO(("hello_sensor_management_callback: %x\n", event ));
...
static void hello_sensor_send_message( void )
{
    ...
    else
    {
        ...
        {
            WPRINT_APP_INFO( ( " Indication message\n" ) );
...
static wiced_bt_gatt_status_t hello_sensor_gatt_server_write_request_handler( uint16_t conn_id, wiced_bt_gatt_write_t * p_data )
{
    ...
    case HANDLE_HSENS_SERVICE_CHAR_BLINK_VAL:
        ...
        {
            WPRINT_APP_INFO(( "hello_sensor_write_handler:num blinks: %d\n", hello_sensor_hostinfo.number_of_blinks
            ));
        }
        hello_sensor_write = attribute_value;
        WPRINT_APP_INFO( ("The value written is: %i\n", attribute_value) );
        break;
...
{
    WPRINT_APP_INFO(( "hello_sensor_indication_confirmation, conn %d hdl %d\n", conn_id, handle ));

    if ( !hello_sensor_state.flag_indication_sent )
    {
        WPRINT_APP_INFO(("Hello: Wrong Confirmation!"));
        return WICED_BT_GATT_SUCCESS;
    }
...
static wiced_bt_gatt_status_t hello_sensor_gatts_connection_up( wiced_bt_gatt_connection_status_t *p_status )
{
    ...
    WPRINT_APP_INFO( ( "hello_sensor_conn_up  id:%d\n:", p_status->conn_id) );
...
static wiced_bt_gatt_status_t hello_sensor_gatt_server_request_handler( wiced_bt_gatt_attribute_request_t *p_data )
{
    ...
    WPRINT_APP_INFO(( "hello_sensor_gatt_server_request_handler. conn %d, type %d\n", p_data->conn_id, p_data->request_type ));
...
static void application_sleep_event_handler( wiced_sleep_event_type_t event, int handler )
{
    switch ( event )
    {
        case WICED_SLEEP_EVENT_ENTER:
            WPRINT_APP_INFO( ( "ble_hello_sensor : App Entering  Low power mode \n" ) );
            break;

        case WICED_SLEEP_EVENT_CANCEL:
            WPRINT_APP_INFO( ( "ble_hello_sensor : App Cancelling Low power mode \n") );
            break;

        case WICED_SLEEP_EVENT_LEAVE:
            WPRINT_APP_INFO( ( "ble_hello_sensor : App Leaving Low power mode \n") );
            break;
