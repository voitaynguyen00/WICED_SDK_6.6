...
void sw_md5_test(void)
{
    ...
    while (test_vector_ptr[i] != NULL)
    {
        ...
        mbedtls_md5_update_ret( &md5_ctx, (unsigned char *)test_vector_ptr[i], strlen(test_vector_ptr[i]) );
        mbedtls_md5_finish_ret( &md5_ctx, (unsigned char *)hash_value );
...
void sw_sha1_test(void)
{
    ...
    while (test_vector_ptr[i] != NULL)
    {
        ...
        mbedtls_sha1_update_ret( &sha1_ctx, (unsigned char *)test_vector_ptr[i], strlen(test_vector_ptr[i]) );
    ...
    for (i = 0; i < 10000; i++)
    {
        mbedtls_sha1_update_ret( &sha1_ctx, (unsigned char *)&long_test_vector, strlen(long_test_vector) );
    }
    mbedtls_sha1_finish_ret( &sha1_ctx, (unsigned char *)hash_value );
...
void sw_sha256_test(void)
{
    ...
    while (test_vector_ptr[i] != NULL)
    {
        ...
        mbedtls_sha256_update_ret( &sha256_ctx, (unsigned char *)test_vector_ptr[i], strlen(test_vector_ptr[i]) );
        mbedtls_sha256_finish_ret( &sha256_ctx, (unsigned char *)hash_value );
