...
static wiced_result_t websocket_server_on_message_callback  ( void* websocket, uint8_t* data, uint32_t length, wiced_websocket_frame_type_t type, wiced_websocket_frame_flags_t flags , uint32_t total_payload_length, int is_data_chunked);
static wiced_result_t websocket_server_on_chunked_message_callback( void* websocket, uint8_t* data, uint32_t length, wiced_websocket_frame_type_t type, wiced_websocket_frame_flags_t flags , uint32_t total_payload_length, int is_data_chunked);
...
static wiced_websocket_callbacks_t myserver_callbacks =
{
    ...
    .on_chunked_message = websocket_server_on_chunked_message_callback,
};
...
{
    uint8_t echo_payload[MAX_ECHO_LENGTH] = {0}; //Note: MAX_ECHO_LENGTH is equal to BUFFER_LENGTH
...
static wiced_result_t websocket_server_on_chunked_message_callback( void* websocket, uint8_t* data, uint32_t length, wiced_websocket_frame_type_t type, wiced_websocket_frame_flags_t flags , uint32_t total_payload_length, int is_data_chunked){
   ...
   {
       WPRINT_APP_DEBUG( ( "%d [App] Received the complete frame\r\n", __LINE__ ) );
       /*Send the echo frame here(for text data and ping-pong response)*/
       if(type == WEBSOCKET_TEXT_FRAME || type == WEBSOCKET_PING || type == WEBSOCKET_PONG)
           send_echo_frame( websocket, rx_buffer_ptr, total_payload_length, type );
       free(rx_buffer_ptr);
...
static wiced_result_t websocket_server_on_message_callback  ( void* socket, uint8_t* data, uint32_t length, wiced_websocket_frame_type_t type, wiced_websocket_frame_flags_t flags , uint32_t total_payload_length, int is_data_chunked)
{
    wiced_websocket_t* websocket = ( wiced_websocket_t* )socket;
    ...
    switch( type )
    {
        case WEBSOCKET_TEXT_FRAME :
        {
            ...
            /*Send the echo frame only if all the data is received*/
            if(is_data_chunked ==0)
