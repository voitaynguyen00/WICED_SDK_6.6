...
#define OTA2_CONSOLE_COMMANDS \
    ...
    { (char*) "factory_failsafe",ota2_console_command,    0, NULL, NULL, (char *)"", (char *)"Factory Reset - Update using failsafe on boot" }, \
    ...
    { (char*) "update_failsafe",ota2_console_command,    0, NULL, NULL, (char *)"", (char *)"OTA2 update - Update using failsafe on boot" }, \
...
static cmd_lookup_t command_lookup[OTA2_CONSOLE_CMD_MAX] = {
        ...
        { "factory_failsafe", PLAYER_EVENT_FACTORY_RESET_FAILSAFE },
        ...
        { "update_failsafe", PLAYER_EVENT_UPDATE_FAILSAFE },
...
wiced_result_t my_ota2_callback(void* session_id, wiced_ota2_service_status_t status, uint32_t value, void* opaque )
{
    ...
    case OTA2_SERVICE_UPDATE_ENDED:     /* All update actions for this check are complete.
        ...
        wiced_log_msg( WLF_OTA2, WICED_LOG_NOTICE, "        return SUCCESS (not used by service). This is informational \r\n");
...
wiced_result_t ota2_test_get_update(ota2_data_t* player)
{
    ...
    player->ota2_bg_params.update_url               = player->uri_to_stream;
...
static void ota2_test_mainloop(ota2_data_t *player)
{
    ...
    while (player->tag == PLAYER_TAG_VALID)
    {
        ...
        if (events & PLAYER_EVENT_FACTORY_RESET_FAILSAFE)
        {
            ...
            if (wiced_dct_read_with_copy( &dct_ota2_config, DCT_OTA2_CONFIG_SECTION, 0, sizeof(platform_dct_ota2_config_t) ) == WICED_SUCCESS)
            {
                ...
                wiced_log_msg( WLF_OTA2, WICED_LOG_NOTICE, "Mark the factory reset to be extracted on next Boot after failsafe.\r\n");
                /* set boot to failsafe */
                ...
                {
                    wiced_log_msg( WLF_OTA2, WICED_LOG_NOTICE, "OTA2 DCT Write failed try again.\r\n");
                }
                else
                {
                    wiced_log_msg( WLF_OTA2, WICED_LOG_NOTICE, "ReBoot now to watch the failsafe action.\r\n");
                }
            }
        }
        ...
        if (events & PLAYER_EVENT_UPDATE_FAILSAFE)
        {
            platform_dct_ota2_config_t  dct_ota2_config;
            ...
            {
                /* Fake the Staged Area download status so we can extract it */
                wiced_log_msg( WLF_OTA2, WICED_LOG_NOTICE, "Mark the update valid for failsafe.\r\n");
                wiced_ota2_image_fakery(WICED_OTA2_IMAGE_VALID);
                /* set boot to failsafe */
