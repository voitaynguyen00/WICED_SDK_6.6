...
/** @file
 *
 * RSA PKCS#1 snip Application
 *
 * This application snippet demonstrates how to sign data using RSA PKCS#1
 *
 * Features demonstrated
 *  - RSA signing using Private key
 *  - Verifying the RSA signature using Public key
 *  - RSA signing using Public key
 *  - Verifying the RSA signature using Private key
 *
 ...
 *  - displays WICED startup information
 *  - signs a message using Private key and verifies it using Public key
 *  - signs a message using Public key and verifies it using Private key
 *
 */
...
#if defined(PLATFORM_HAS_HW_CRYPTO_SUPPORT)
    WPRINT_APP_INFO(("\r\nHardware Crypto Engine initialization...."));
    wiced_hwcrypto_init( );
