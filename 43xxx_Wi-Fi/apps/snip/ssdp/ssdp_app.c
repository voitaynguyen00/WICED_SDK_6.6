...
#define NUM_ADDITIONAL_HDRS ( sizeof(ssdp_app_additional_hdr_names) / sizeof(wiced_ssdp_hdr_name_t) )
...
static wiced_ssdp_hdr_name_t ssdp_app_additional_hdr_names[] = {
    { .hdr_name = "X-User-Agent" },
    { .hdr_name = "01-NLS" },
};
...
static void release_app_info(application_info_t* app_info)
{
    uint8_t i;
    uint8_t response_count = 0;

    response_count = sizeof(app_info->msearch_responses)/sizeof(wiced_ssdp_msearch_response_t);
    for(i = 0; i < response_count; i++)
    {
        if(app_info->msearch_responses[i].additional_hdrs != NULL)
        {
            free(app_info->msearch_responses[i].additional_hdrs);
            app_info->msearch_responses[i].additional_hdrs = NULL;
        }
    }
    free(app_info);
}
...
static wiced_result_t ssdp_app_send_msearch(application_info_t *app_info)
{
    ...
    msearch_params.additional_hdr_names = ssdp_app_additional_hdr_names;
    msearch_params.num_additional_hdrs  = NUM_ADDITIONAL_HDRS;
    ...
    if( wiced_ssdp_send_msearch_wait_for_results(app_info->ssdp_info, &msearch_params) == WICED_SUCCESS)
    {
        ...
        for(i = 0; i < msearch_params.num_responses; i++)
        {
            ...
            j = 0;
            while( j < msearch_params.num_additional_hdrs)
            {
                if( app_info->msearch_responses[i].additional_hdrs[j].hdr_name[0] != '\0' )
                {
                    WPRINT_APP_INFO(("     %-14s:%s\r\n", app_info->msearch_responses[i].additional_hdrs[j].hdr_name, app_info->msearch_responses[i].additional_hdrs[j].hdr_val));
                }
                j++;
            }
        }
...
static void ssdp_notify_callback(wiced_ssdp_notify_info_t* notify_info, void *data)
{
    ...
    if ( notify_info->additional_hdrs != NULL ) /* This will be NULL until the additional headers have been specified by an MSEARCH */
    {
        while( j < NUM_ADDITIONAL_HDRS) /* num_additional_hdrs should match what was specified in MSEARCH */
        {
            if( notify_info->additional_hdrs[j].hdr_name[0] != '\0' )
            {
                WPRINT_APP_INFO(("    %-15s:%s\r\n", notify_info->additional_hdrs[j].hdr_name, notify_info->additional_hdrs[j].hdr_val));
            }
            j++;
        }
    }
...
static application_info_t *init_applicaiton(void)
{
    ...
    uint8_t  additional_hdrs_count = 0;
    uint8_t  response_count = 0;
    uint8_t  i;
    ...
    /* Allocate space to hold additional headers in each response structure */
    additional_hdrs_count = sizeof(ssdp_app_additional_hdr_names)/sizeof(wiced_ssdp_hdr_name_t);
    response_count = sizeof(app_info->msearch_responses)/sizeof(wiced_ssdp_msearch_response_t);
    for(i = 0; i < response_count; i++)
    {
        app_info->msearch_responses[i].additional_hdrs = (wiced_ssdp_hdr_t*)calloc(sizeof(wiced_ssdp_hdr_t), additional_hdrs_count);
        if (app_info->msearch_responses[i].additional_hdrs == NULL)
        {
            WPRINT_APP_ERROR(("Failed to allocate space for additional headers application info structure!\r\n"));
            goto _init_app_exit;
        }
    }
...

    {
        ...
        if (memcmp(&wiced_mac, &app_info->dct_wifi->mac_address, sizeof(wiced_mac_t)) != 0)
        {
            ...
            if (memcmp(&wiced_mac, &app_info->dct_wifi->mac_address, sizeof(wiced_mac_t)) != 0)
            {
                ...
                if (wwd_wifi_set_mac_address( app_info->dct_wifi->mac_address , WWD_STA_INTERFACE) != WWD_SUCCESS)
                {
