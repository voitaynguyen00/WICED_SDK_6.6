...
CONSOLE_BT_COEX_TEST := 0
ifeq ($(CONSOLE_BT_COEX_TEST),1)
# by default enterise security is enabled and this causes memory issue on certain platforms while executing COEX tests.
GLOBAL_DEFINES += CONSOLE_DISABLE_ENTERPRISE_COMMANDS
ifeq ($(PLATFORM),$(filter $(PLATFORM), BCM94343WWCD2 CY8CKIT_062 CYW943012EVB_04))
$(NAME)_COMPONENTS += utilities/command_console/bt_coex_test \
                      libraries/drivers/bluetooth/dual_mode
GLOBAL_DEFINES  += BT_BUS_RX_FIFO_SIZE=512
$(NAME)_DEFINES += CONSOLE_INCLUDE_BT_COEX_TEST
endif
endif
...
# Disable P2P component for these platforms due to limitation of APP_CODE length.
# To use P2P component, get rid of other unnecessary components.
...
ifeq ($(PLATFORM),$(filter $(PLATFORM),  BCM943364WCD1 CYW94343WWCD1_EVB BCM94343WWCD2 NEB1DX_01 BCM943438WCD1 CY8CKIT_062 CYW9WCD760PINSDAD2))
...
# Enterprise security commands are disabled to save code space.
...
ifeq ($(PLATFORM),$(filter $(PLATFORM), CYW9WCD760PINSDAD2 ))
...
GLOBAL_DEFINES += TX_PACKET_POOL_SIZE=50 \
                  RX_PACKET_POOL_SIZE=50 \
...
# Enterprise security commands are disabled to save code space.
BCM943903_PLATFORMS := BCM943903*
$(eval BCM943903_PLATFORMS := $(call EXPAND_WILDCARD_PLATFORMS,$(BCM943903_PLATFORMS)))
ifeq ($(PLATFORM),$(filter $(PLATFORM), $(BCM943903_PLATFORMS)))
GLOBAL_DEFINES += CONSOLE_DISABLE_ENTERPRISE_COMMANDS
endif
...
ifeq ($(CONSOLE_BT_COEX_TEST),1)
# modified TX and RX packet pool size to fit the overall memory for COEX usecase.
...
CYW943012EVB_PLATFORMS := CYW943012P6EVB*
$(eval CYW943012EVBx_PLATFORMS := $(call EXPAND_WILDCARD_PLATFORMS, $(CYW943012EVB_PLATFORMS)))
ifeq ($(PLATFORM), $(filter $(PLATFORM), $(CYW943012EVBx_PLATFORMS)))
$(info <<<<< CYW943012P6EVBx Platform BUFFER POOLS INITIALIZED >>>>>)
GLOBAL_DEFINES += TX_PACKET_POOL_SIZE=15 \
                  RX_PACKET_POOL_SIZE=15 \
                  WICED_TCP_TX_DEPTH_QUEUE=16 \
                  WICED_TCP_WINDOW_SIZE=131072
...
# Disable components due to limitation of APP_CODE length on BCM943438WCD1 debug build
ifneq (,$(findstring BCM943438WCD1,$(PLATFORM)))
ifeq ($(BUILD_TYPE),debug)
CONSOLE_NO_IPERF :=1
CONSOLE_DISABLE_WIFI_COMMANDS ?= 1
CONSOLE_DISABLE_WPS_COMMANDS ?= 1
...
#==============================================================================
#comment the below line to disable WLAN IOCTL,IOVARS & events logging
GLOBAL_DEFINES += WWD_IOCTL_LOG_ENABLE

#==============================================================================
# Enable Memory Stats
#==============================================================================
#GLOBAL_DEFINES += KPI_METRICS
#GLOBAL_DEFINES += __STACKSIZE__=$$($(RTOS)_START_STACK)
#$(NAME)_COMPONENTS += utilities/mem_usage
