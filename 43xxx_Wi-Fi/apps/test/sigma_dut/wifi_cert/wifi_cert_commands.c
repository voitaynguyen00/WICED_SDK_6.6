...
#define P2P_RESET_WAIT              1100    /* 1100 ms, should larger than 1 watchdog tick */

// Global variables
static char test_passphrase[64]   = TEST_PASSPHRASE_DEFAULT;
static char test_sectype[16]      = TEST_SECTYPE_DEFAULT;
...
static char test_bssid[18]        = TEST_BSSID_DEFAULT;
static char test_channel[4]       = TEST_CHANNEL_DEFAULT;
...
wiced_security_t get_authtype( char* encptype, char* keymgmttype, char* sectype );
wiced_bool_t get_mfptype ( char *mfp_string, uint8_t *mfp );
...
static wiced_result_t sta_scan_result_handler ( wiced_scan_handler_result_t* malloced_scan_result );
...
int ca_get_version( int argc, char *argv[] )
{
    printf( "status,COMPLETE,version,%s_%s\n", VENDOR, SOFTWARE_VERSION );
    return 0;
}
...
int sta_set_security( int argc, char *argv[] )
{
    int i = 1;
    ...
    while (i < (argc - 1))
    {
        ...
        {
            strncpy(test_sectype, argv[i+1], sizeof( test_sectype ) - 1 );
        }
...
int sta_associate( int argc, char *argv[] )
{
    ...
    if (ret != WWD_SUCCESS)
    {
        printf( "problem setting tx power result=%d\n", ret );
    }

    auth_type = get_authtype( test_encptype, test_keymgmttype, test_sectype );
    ...
    else
    {
        ...
        {
            wwd_wifi_enable_sup_set_passphrase( (uint8_t*) security_key, key_length, auth_type);
        }
    ...
    if (get_mfptype( test_pmf, &mfp_value ))
    {
...
int wiced_scan(int argc, char *argv[])
{
    WIFI_SIGMA_LOG( ("  # Type  BSSID         RSSI Rate Chan  Security               SSID                            CCode    Flag\n" ) );
    WIFI_SIGMA_LOG( ("------------------------------------------------------------------------------------------------------------------\n" ) );
    wiced_wifi_scan_networks(sta_scan_result_handler, NULL );
    return 0;
}

int wiced_status(int argc, char *argv[])
{
    wiced_status_result();
    return 0;
}
...
int wicedlog_read ( int argc, char *argv[] )
{
    wwd_result_t result = WWD_SUCCESS;
...
int sta_p2p_reset( int argc, char *argv[] )
{
    ...
    if ( p2p_workspace.p2p_initialised )
    {
        ...
        /* Wait until wifi chip returned to idle. */
        wiced_rtos_delay_milliseconds( P2P_RESET_WAIT );
    }
...
int sta_scan( int argc, char *argv[] )
{
    /* Scan for test SSID and possibly modify authentication type. */
    wiced_wifi_scan_networks(sta_scan_result_handler, NULL );
    printf("status,COMPLETE\n");
...
int sta_reassoc( int argc, char *argv[] )
{
   ...
   memset(test_bssid, 0, sizeof( test_bssid ));
   while (i < (argc - 1))
   {
        if ( strcasecmp( argv[i], "BSSID" ) == 0 )
        {
            strncpy(test_bssid, argv[i+1], sizeof( test_bssid ) - 1 );
        }
        if ( strcasecmp( argv[i], "channel" ) == 0 )
        {
            strncpy(test_channel, argv[i+1], sizeof( test_channel ) - 1 );
        }
   ...
   if ( WICED_SUCCESS != wiced_wifi_join_specific(test_ssid, strlen(test_ssid), auth_type, security_key, key_length, test_bssid, test_channel, dut_ip_addr, dut_netmask, dut_gateway))
   {
        /* it is kept for debugging
        printf("Failed to join using join_specific\n");
        */
   }
   printf("status,COMPLETE\n");
...
void init_security( void )
{
    ...
    strncpy(test_sectype, TEST_SECTYPE_DEFAULT, sizeof( test_sectype ) - 1 );
