...
#define TS_THREAD_STACK_SIZE ( 6124 )
#if ( defined(NETWORK_LwIP) )
#define FREERTOS_PRIORITY_ADJUSTMENT (1)
#endif
...
int spawn_ts_thread( void *ts_function, traffic_stream_t *ts )
{
    ...
    if ( ts->direction == TRAFFIC_SEND )
    {
        ...
        prio = WICED_RTOS_HIGHEST_PRIORITY + FREERTOS_PRIORITY_ADJUSTMENT +  PRIO_BOOST_OF_RX_THREAD_WRT_TX + ac_priority[ts->ac] * PRIO_OFFSET_BETWEEN_AC + tx_ac_priority_num[ts->ac]++;
    }
    else if ( ts->direction == TRAFFIC_RECV )
    {
        prio = FREERTOS_PRIORITY_ADJUSTMENT + ac_priority[ts->ac] * PRIO_OFFSET_BETWEEN_AC + rx_ac_priority_num[ts->ac]++ - 1;
    }
#else
    if ( ts->direction == TRAFFIC_SEND )
    {
...
#endif
 //   printf("P %d AC %d ac_prior:%d tx_ac_priority_num[ts->ac]:%d\n", prio, ts->ac, ac_priority[ts->ac], tx_ac_priority_num[ts->ac]);
...
void ts_thread_wrapper( uint32_t arg )
{
    ...
    if ( ts->direction == TRAFFIC_SEND && ts->enabled ) // Print results from all tx streams terminating at the same time
    {
        ...
        WIFI_SIGMA_LOG(("status,COMPLETE,streamID,"));
        ...
        for ( i = 0; i < NUM_STREAM_TABLE_ENTRIES; ++i )
        {
            if ( ts->stop_time == stream_table[i].stop_time && stream_table[i].direction == TRAFFIC_SEND )
            {
                ...
                WIFI_SIGMA_LOG(("%u ", (unsigned int)stream_table[i].stream_id));
            }
        }
        ...
        for ( i = 0; i < NUM_STREAM_TABLE_ENTRIES; ++i )
        {
            if ( ts->stop_time == stream_table[i].stop_time && stream_table[i].direction == TRAFFIC_SEND )
            {
                ...
                WIFI_SIGMA_LOG(("%d ", stream_table[i].frames_sent ));
            }
        }

        printf( ",rxFrames," );
        WIFI_SIGMA_LOG((",rxFrames,"));
        for ( i = 0; i < NUM_STREAM_TABLE_ENTRIES; ++i )
        {
            if ( ts->stop_time == stream_table[i].stop_time && stream_table[i].direction == TRAFFIC_SEND )
            {
                printf( "%d ", stream_table[i].frames_received );
                WIFI_SIGMA_LOG(("%d ", stream_table[i].frames_received  ));
            }
        }

        printf( ",txPayloadBytes," );
        WIFI_SIGMA_LOG((",txPayloadBytes,"));
        ...
        for ( i = 0; i < NUM_STREAM_TABLE_ENTRIES; ++i )
        {
            if ( ts->stop_time == stream_table[i].stop_time && stream_table[i].direction == TRAFFIC_SEND )
            {
                ...
                WIFI_SIGMA_LOG(("%d ", stream_table[i].bytes_sent));
            }
        }

        printf( ",rxPayloadBytes," );
        WIFI_SIGMA_LOG((",rxPayloadBytes,"));
        for ( i = 0; i < NUM_STREAM_TABLE_ENTRIES; ++i )
        {
            if ( ts->stop_time == stream_table[i].stop_time && stream_table[i].direction == TRAFFIC_SEND )
            {
                printf( "%d ", stream_table[i].bytes_received );
                WIFI_SIGMA_LOG(("%d ", stream_table[i].bytes_received ));
            }
        }

        printf( ",outOfSequenceFrames," );
        WIFI_SIGMA_LOG(( ",outOfSequenceFrames," ));
        for ( i = 0; i < NUM_STREAM_TABLE_ENTRIES; ++i )
        {
            if ( ts->stop_time == stream_table[i].stop_time && stream_table[i].direction == TRAFFIC_SEND )
            {
                printf( "%d ", stream_table[i].out_of_sequence_frames );
                WIFI_SIGMA_LOG(("%d ", stream_table[i].out_of_sequence_frames ));
            }
        }
