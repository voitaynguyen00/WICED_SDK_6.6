...
static uint32_t      stop_ping_immediately = 0;
...
wiced_result_t wifi_traffic_send_ping( void *arg )
{
    ...
    while ( (num_frames > 0 ) && ( host_rtos_get_time( ) < finish_time ) && ( stop_ping_immediately == 0 ) )
    {
        ++num_ping_requests;
...
{
    stop_ping_immediately = 1; // indicate that the action of ping should be stopped.

    /*
     * Wait 500ms for make sure that the previous PING had been stopped completely.
     */
