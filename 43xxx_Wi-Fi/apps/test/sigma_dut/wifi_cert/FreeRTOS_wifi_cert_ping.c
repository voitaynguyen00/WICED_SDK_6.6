...
char buf[WICED_PING_PAYLOAD_MTU + sizeof(struct icmp_echo_hdr) + sizeof(struct ip_hdr)] = {0};
...
//    wiced_time_t end_time;
    ...
    if ( ( frame_size > WICED_PING_PAYLOAD_MTU ) || ( frame_size < 0 ) )
    {
       printf("max ping length: %d, min: 0\n\r", WICED_PING_PAYLOAD_MTU);
       return ERR_CMD_OK;
    }

   // printf("entering ping, frame size %d\n", frame_size);
