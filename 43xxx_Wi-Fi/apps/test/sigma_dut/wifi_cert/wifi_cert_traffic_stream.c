...
int udp_tx( traffic_stream_t* ts )
{
    ...
    if ( (result = wiced_udp_create_socket( &ts->tx_socket, ts->dest_port, WICED_STA_INTERFACE )) != WICED_SUCCESS )
    {
        printf( "Could not create transmit UDP socket:%d\n\n", result );
        WIFI_SIGMA_LOG(("Could not create transmit UDP socket:%d\n\n", result));
        return -7;
    }
    ...
    while ( ts->enabled )
    {
        ...
        if (wiced_packet_create_udp_no_wait(&ts->tx_socket, ts->payload_size, &packet, (uint8_t**)&udp_data, &available_data_length) == WICED_SUCCESS)
        {
            ...
            /* Send the UDP packet */
            if ( (result = wiced_udp_send(&ts->tx_socket, &target_ip_addr, ts->dest_port, packet)) != WICED_SUCCESS)
            {
                printf("UDP packet send failed result:%d\n", result);
                WIFI_SIGMA_LOG(("UDP packet send failed result:%d\n", result));
            }
            else
            {
                ++ts->frames_sent;
