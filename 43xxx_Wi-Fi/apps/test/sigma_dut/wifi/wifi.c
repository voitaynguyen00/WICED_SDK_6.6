...
int wifi_join(char* ssid, uint8_t ssid_length, wiced_security_t auth_type, uint8_t* key, uint16_t key_length, char* ip, char* netmask, char* gateway)
{
    ...
    if( ( auth_type == WICED_SECURITY_WPA3_SAE ) || ( auth_type == WICED_SECURITY_WPA3_WPA2_PSK ) )
    {
        memcpy((char*)dct_wifi_config->stored_ap_list[0].security_key, (char*)key, MAX_SAE_PASSWORD_LEN);
    }
    else
    {
