...
// Based on the BT Controller ARM architecture, we can optimize these macros.
//TODO - change the #if 1 to some flag that is only set when compiling for the BT Controller
#ifdef DO_NOT_OPTIMISE_FOR_LITTLE_ENDIAN
...
#else
    #define UINT32_TO_STREAM(p, u32) {*(UINT32 *)(p) = u32; (p) += 4;}
    #define UINT16_TO_STREAM(p, u16) {*(UINT16 *)(p) = u16; (p) += 2;}
