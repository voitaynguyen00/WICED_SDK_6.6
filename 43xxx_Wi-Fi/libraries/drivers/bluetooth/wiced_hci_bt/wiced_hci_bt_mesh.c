...
static void wiced_hci_mesh_cb(uint16_t event, uint8_t* payload, uint32_t len)
{
    ...
    switch(event)
    {
        ...
        {
            WICED_HCI_DEBUG_LOG(("HCI_CONTROL_MESH_EVENT_PROXY_DATA\n "));
            uint32_t packet_len;
            uint8_t *packet;
            packet_len = len;
...
wiced_result_t wiced_bt_mesh_proxy_connect(uint8_t connection_state)
 {
     ...
     wiced_hci_send( HCI_CONTROL_MESH_COMMAND_SEND_CONN_STATUS, data, length );
     free(data);
     return result;
 }
...
wiced_result_t wiced_bt_mesh_push_nvram_data(uint8_t *data_in , uint8_t data_len , uint16_t idx)
{
    uint8_t*                data  = NULL;
    uint8_t*                p     = NULL;
    uint16_t                length  = 0;
