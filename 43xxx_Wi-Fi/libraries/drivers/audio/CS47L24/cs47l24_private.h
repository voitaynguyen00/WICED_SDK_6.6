...
/**
 * @file Private CS47L24 driver functions
 */
...
#define CS47L24_VERIFY_GOTO( expr, res_var, label ) \
    do \
    { \
        res_var = (expr); \
        if (res_var != WICED_SUCCESS) \
        { \
            wiced_log_msg(WLF_DRIVER, WICED_LOG_ERR, "%s: [%s] failed (%d)\n", __func__, #expr, (int)res_var); \
            goto label; \
        } \
    } while (0)


#define SC_FF_DUET_REG_MASK (0xFFFFFFUL)
