...
/** @file CS47L24 WICED audio driver implementation
 *
 */

#define _GNU_SOURCE /* For pow10()/log10() */
#include <string.h>
#include <stdlib.h>
#include <math.h>
...
#include "cs47l24_firmware_register_map.h"
...
#define CS47L24_VOLUME_IN_DB_MIN             (-64.0)
#define CS47L24_VOLUME_IN_DB_MAX             (31.5)
#define CS47L24_VOLUME_IN_DB_STEP            (0.5)

#define CS47L24_VOLUME_OUT_DB_MIN            (CS47L24_VOLUME_IN_DB_MIN)
#define CS47L24_VOLUME_OUT_DB_MAX            (6.0) /* actually, CS47L24 can do up to +31.5dB on the analog output */
#define CS47L24_VOLUME_OUT_DB_STEP           (CS47L24_VOLUME_IN_DB_STEP)
...
#define CS47L24_DMIC_VOLUME_DEFAULT          (10.0)
#define CS47L24_AIF_MIXER_VOLUME_DEFAULT     (0.0)
#define CS47L24_DSP2_MIXER_VOLUME_DEFAULT    (3.0)
...
#ifdef CS47L24_ENABLE_DSP
static wiced_result_t cs47l24_get_wupd_params(cs47l24_device_cmn_data_t* cs47l24, wiced_audio_wupd_params_t* params);
static wiced_result_t cs47l24_set_wupd_params(cs47l24_device_cmn_data_t* cs47l24, wiced_audio_wupd_params_t* params);
#endif /* CS47L24_ENABLE_DSP */
...
wiced_result_t cs47l24_block_write(cs47l24_device_cmn_data_t* cs47l24, uint8_t* buffer, uint32_t buffer_length)
{
    ...
    if (buffer_length < (2 * sizeof(uint32_t)))
    {
        result = WICED_ERROR;
        goto _exit;
    }
    ...
    /*
     * Make sure we're initiating a write operation;
     * assuming here first 4 bytes in the buffer is a register address
     */
    ...
    {
        wiced_log_msg(WLF_DRIVER, WICED_LOG_ERR, "cs47l24_block_write(0x%02X%02X%02X%02X) fails !\n", buffer[0], buffer[1], buffer[2], buffer[3]);
    }
 _exit:
    return result;
}
...
wiced_result_t cs47l24_block_read(cs47l24_device_cmn_data_t* cs47l24, uint8_t* buffer, uint32_t buffer_length)
{
    wiced_result_t              result  = WICED_SUCCESS;
    wiced_spi_message_segment_t msg;
    ...
    /*
     * Make sure we're initiating a read operation;
     * assuming here first 4 bytes in the buffer is a register address
     */
    ...
    msg.tx_buffer = buffer;
    msg.rx_buffer = buffer;
    msg.length    = buffer_length;

    result = wiced_spi_transfer( cs47l24->spi_data, &msg, 1 );
    if (result != WICED_SUCCESS)
    {
        wiced_log_msg(WLF_DRIVER, WICED_LOG_ERR, "cs47l24_block_read(0x%02X%02X%02X%02X) fails !\n", buffer[0], buffer[1], buffer[2], buffer[3]);
    }
 _exit:
    return result;
}
...
wiced_result_t cs47l24_dsp_word_read(cs47l24_device_cmn_data_t *cs47l24, uint32_t reg, uint32_t *value)
{
    wiced_result_t result;
    dsp_word_t     dsp_word;

    dsp_word.register_address = htonl(reg);
    dsp_word.padding          = 0;
    dsp_word.word             = 0;
    result = cs47l24_block_read(cs47l24, (uint8_t*)&dsp_word, sizeof(dsp_word));
    if (result != WICED_SUCCESS)
    {
        *value = 0;
    }
    ...
    {
        *value = ntohl(dsp_word.word);
    }
    return result;
}


wiced_result_t cs47l24_dsp_word_write(cs47l24_device_cmn_data_t *cs47l24, uint32_t reg, uint32_t value)
{
    dsp_word_t dsp_word;

    dsp_word.register_address = htonl(reg);
    dsp_word.padding          = 0;
    dsp_word.word             = htonl(value);
    return cs47l24_block_write(cs47l24, (uint8_t*)&dsp_word, sizeof(dsp_word));
}
...
static inline uint16_t cs47l24_percent_to_index(uint8_t percentage)
{
    ...
    idx = (uint16_t)((double)((CS47L24_VOLUME_IN_DB_MAX - CS47L24_VOLUME_IN_DB_MIN) / CS47L24_VOLUME_IN_DB_STEP) * percentage);
...
/* Convert a n-indexed integer range to a dB range. */
static inline double cs47l24_index_with_offset_to_double(uint16_t index, uint16_t index_offset, double volume_max, double volume_min, double volume_step)
{
    double volume_db;

    volume_db = volume_min + ((double)(index - index_offset) * volume_step);
...
static inline uint16_t cs47l24_adc_volume_index(double volume_db)
{
    return cs47l24_double_range_with_offset_to_index(volume_db, CS47L24_VOLUME_IN_DB_MAX, CS47L24_VOLUME_IN_DB_MIN, CS47L24_VOLUME_IN_DB_STEP, 0);
}


static inline uint16_t cs47l24_dac_volume_index(double volume_db)
{
    return cs47l24_double_range_with_offset_to_index(volume_db, CS47L24_VOLUME_OUT_DB_MAX, CS47L24_VOLUME_OUT_DB_MIN, CS47L24_VOLUME_OUT_DB_STEP, 0);
}
...
static inline double cs47l24_adc_volume_in_db(uint16_t index)
{
    return cs47l24_index_with_offset_to_double(index, 0, CS47L24_VOLUME_IN_DB_MAX, CS47L24_VOLUME_IN_DB_MIN, CS47L24_VOLUME_IN_DB_STEP);
}


static inline double cs47l24_dac_volume_in_db(uint16_t index)
{
    return cs47l24_index_with_offset_to_double(index, 0, CS47L24_VOLUME_OUT_DB_MAX, CS47L24_VOLUME_OUT_DB_MIN, CS47L24_VOLUME_OUT_DB_STEP);
}


static inline double cs47l24_mixer_volume_in_db(uint16_t index)
{
    return cs47l24_index_with_offset_to_double(index, CS47L24_MIXER_VOLUME_INDEX_OFFSET, CS47L24_MIXER_VOLUME_DB_MAX, CS47L24_MIXER_VOLUME_DB_MIN, CS47L24_MIXER_VOLUME_DB_STEP);
}
...
static wiced_result_t cs47l24_set_initial_clocking(cs47l24_device_data_t *dd)
{
    ...
    CS47L24_CHECK_RETRY_COUNT(loop_count, result, _exit);
    wiced_log_msg(WLF_DRIVER, WICED_LOG_DEBUG4, "cs47l24_set_initial_clocking: HP output disabled...\n");

    /* Disable SPKR output */
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_OUTPUT_ENABLES_1, CS47L24_OUT4L_ENA_MASK, 0), result, _exit);
    ...
    CS47L24_CHECK_RETRY_COUNT(loop_count, result, _exit);
    wiced_log_msg(WLF_DRIVER, WICED_LOG_DEBUG4, "cs47l24_set_initial_clocking: SPKR output disabled...\n");
    ...
    for ( input_source_reg = CS47L24_EQ1MIX_INPUT_1_SOURCE; input_source_reg <= CS47L24_EQ2MIX_INPUT_4_SOURCE; input_source_reg += 0x2)
    {
        WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, input_source_reg, CS47L24_INPUT_SOURCE_MASK, CS47L24_DCORE_INPUT_SILENCE <<  CS47L24_INPUT_SOURCE_SHIFT), result, _exit );
    }
    ...
    /* Set FX rate for EQ/DRC/LHPF blocks */
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_FX_CTRL1, CS47L24_FX_RATE_MASK, CS47L24_DCORE_SAMPLERATE_3 << CS47L24_FX_RATE_SHIFT), result, _exit);

    wiced_log_msg(WLF_DRIVER, WICED_LOG_DEBUG4, "cs47l24_set_initial_clocking: IN/OUT/FX rates are OK !\n");
...
static wiced_result_t cs47l24_enable_dsp(cs47l24_device_data_t *dd)
{
    ...
    uint32_t                         input_source_reg;
    uint32_t                         i;
    uint16_t                         mixer_volume_for_dsp2;
    ...
    /*
     * Connect DSP2 to DSP3 for WUPD through EQ1
     */
    mixer_volume_for_dsp2 = cs47l24_mixer_volume_index(CS47L24_DSP2_MIXER_VOLUME_DEFAULT);

    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_EQ1MIX_INPUT_1_VOLUME, CS47L24_INPUT_VOLUME_MASK, mixer_volume_for_dsp2 << CS47L24_INPUT_VOLUME_SHIFT), result, _exit );
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_EQ1MIX_INPUT_1_SOURCE, CS47L24_INPUT_SOURCE_MASK, CS47L24_DCORE_INPUT_DSP3_CHANNEL2 << CS47L24_INPUT_SOURCE_SHIFT), result, _exit );
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DSP2AUX1MIX_INPUT_1_SOURCE, CS47L24_INPUT_SOURCE_MASK, CS47L24_DCORE_INPUT_EQ1 << CS47L24_INPUT_SOURCE_SHIFT), result, _exit );
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_EQ1_1, CS47L24_EQ1_ENA_MASK, CS47L24_EQ1_ENA << CS47L24_EQ1_ENA_SHIFT), result, _exit );
...
static wiced_result_t cs47l24_enable_dmic_route(cs47l24_device_data_t *dd)
{
    ...
    uint16_t                         mixer_volume_dmic_left;
    ...
    volume_default         = cs47l24_adc_volume_index(CS47L24_DMIC_VOLUME_DEFAULT);
    mixer_volume_dmic_left = cs47l24_mixer_volume_index(0.0);
    ...
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DSP3RMIX_INPUT_1_VOLUME, CS47L24_INPUT_VOLUME_MASK, mixer_volume_dmic_left << CS47L24_INPUT_VOLUME_SHIFT), result, _exit );
    ...
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_OUT4LMIX_INPUT_1_SOURCE, CS47L24_INPUT_SOURCE_MASK, CS47L24_DCORE_INPUT_ASRC2L <<  CS47L24_INPUT_SOURCE_SHIFT), result, _exit );
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_OUT4LMIX_INPUT_1_VOLUME, CS47L24_INPUT_VOLUME_MASK, mixer_volume_aif2rx << CS47L24_INPUT_VOLUME_SHIFT), result, _exit );
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_OUT4LMIX_INPUT_2_SOURCE, CS47L24_INPUT_SOURCE_MASK, CS47L24_DCORE_INPUT_ASRC2R <<  CS47L24_INPUT_SOURCE_SHIFT), result, _exit );
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_OUT4LMIX_INPUT_2_VOLUME, CS47L24_INPUT_VOLUME_MASK, mixer_volume_aif2rx << CS47L24_INPUT_VOLUME_SHIFT), result, _exit );
...
static wiced_result_t cs47l24_init(void *driver_data, wiced_audio_data_port_t* data_port)
{
    ...
    wiced_result_t                 result  = WICED_SUCCESS;
#ifdef CS47L24_ENABLE_DSP
    wiced_audio_wupd_params_t      wupd;
#endif /* CS47L24_ENABLE_DSP*/
    ...
    /*
     * Set a variable to track AIF1TX mixer volume (to handle muting/unmuting AIF1TX interface)
     */
    rtd->aif1tx_gain = cs47l24_mixer_volume_index(CS47L24_AIF_MIXER_VOLUME_DEFAULT);
    ...
    /* Get current SoundClear tuning params */
    WICED_VERIFY_GOTO( cs47l24_get_wupd_params(cs47l24, &wupd), result, cs47l24_init_unlock );
    /* Modify current params with our tuned defaults */
    wupd.sc_enabled              = SC_FF_DUET_APPPROCMODE_MODE_DEFAULT;
    wupd.aec_ref_bulk_delay_msec = SC_FF_DUET_SCEC1CONFIG_BULKDELAYMSS11_DEFAULT;
    wupd.aec_mic_bulk_delay_msec = SC_FF_DUET_SCEC1CONFIG_TXBULKDELAYMSS11_DEFAULT;
    wupd.dtd_threshold           = SC_FF_DUET_SCVAD_DTDTHRESHS_DEFAULT;
    /* Apply our tuned params */
    WICED_VERIFY_GOTO( cs47l24_set_wupd_params(cs47l24, &wupd), result, cs47l24_init_unlock );
#endif /* CS47L24_ENABLE_DSP */
...
static wiced_result_t cs47l24_deinit(void *driver_data)
{
...
#ifdef CS47L24_ENABLE_PERMANENT_CAPTURE_ROUTE
    /*
...
#else
    rtd->init &= ~( 1 << cs47l24_type( dd ) );
#endif /* CS47L24_ENABLE_PERMANENT_CAPTURE_ROUTE */
    ...
    if ( ( rtd->init & ~( 1 << cs47l24_type( dd ) ) ) == 0 )
    {
        ...
        cs47l24_dsp_scc_deinit(cs47l24);
...
wiced_result_t cs47l24_spll(cs47l24_device_data_t *dd, wiced_audio_config_t *config, uint32_t mclk)
{
    ...
    if (cs47l24_type(dd) == CS47L24_DEVICE_TYPE_PLAYBACK)
    {
        ...
        /* Disable free running mode ! */
        WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_FLL2_CONTROL_1, CS47L24_FLL2_FREERUN_MASK, 0), result, _exit );
...
wiced_result_t cs47l24_hp(cs47l24_device_cmn_data_t *cs47l24, wiced_bool_t is_pwr_up)
{
    ...
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DAC_DIGITAL_VOLUME_4L, CS47L24_OUT4L_MUTE_MASK, CS47L24_OUT4L_MUTE), result, _exit );
    ...
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DAC_DIGITAL_VOLUME_4L, CS47L24_OUT_VU_MASK    , CS47L24_OUT_VU    ), result, _exit );
    ...
    if (is_pwr_up == WICED_TRUE)
    {
        ...
        /* Enable SPKR output */
        WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_OUTPUT_ENABLES_1, CS47L24_OUT4L_ENA_MASK, CS47L24_OUT4L_ENA), result, _exit);
        WICED_VERIFY_GOTO( cs47l24_reg_read(cs47l24, CS47L24_OUTPUT_STATUS_1, &output_disable), result, _exit );
        loop_count = 0;
        while (((output_disable & CS47L24_OUT4L_ENA_STS_MASK) != CS47L24_OUT4L_ENA_STS) && (loop_count < CS47L24_READ_RETRY_COUNT))
        {
            wiced_rtos_delay_milliseconds(1);
            WICED_VERIFY_GOTO( cs47l24_reg_read(cs47l24, CS47L24_OUTPUT_STATUS_1, &output_disable), result, _exit );
            loop_count++;
...
wiced_result_t cs47l24_start_play ( void* driver_data )
{
    ...
    if (cs47l24_type(dd) ==  CS47L24_DEVICE_TYPE_PLAYBACK)
    {
        ...
        WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DAC_DIGITAL_VOLUME_4L, CS47L24_OUT4L_MUTE_MASK, 0             ), result, _unlock );
        ...
        WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DAC_DIGITAL_VOLUME_4L, CS47L24_OUT_VU_MASK    , CS47L24_OUT_VU), result, _unlock );
    }
    else if (cs47l24_type(dd) ==  CS47L24_DEVICE_TYPE_CAPTURE)
    {
        /*
         ...
         */
        WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_AIF1TX1MIX_INPUT_1_VOLUME, CS47L24_INPUT_VOLUME_MASK, rtd->aif1tx_gain << CS47L24_INPUT_VOLUME_SHIFT), result, _unlock );
        WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_AIF1TX2MIX_INPUT_1_VOLUME, CS47L24_INPUT_VOLUME_MASK, rtd->aif1tx_gain << CS47L24_INPUT_VOLUME_SHIFT), result, _unlock );
    }
...
wiced_result_t cs47l24_stop_play ( void* driver_data )
{
    ...
    if (cs47l24_type(dd) ==  CS47L24_DEVICE_TYPE_PLAYBACK)
    {
        ...
        WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DAC_DIGITAL_VOLUME_4L, CS47L24_OUT4L_MUTE_MASK, CS47L24_OUT4L_MUTE), result, _unlock );
        WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DAC_DIGITAL_VOLUME_1L, CS47L24_OUT_VU_MASK    , CS47L24_OUT_VU    ), result, _unlock );
        WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DAC_DIGITAL_VOLUME_1R, CS47L24_OUT_VU_MASK    , CS47L24_OUT_VU    ), result, _unlock );
        WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DAC_DIGITAL_VOLUME_4L, CS47L24_OUT_VU_MASK    , CS47L24_OUT_VU    ), result, _unlock );
    }
...
static wiced_result_t cs47l24_set_playback_volume(void *driver_data, double decibels)
{
    ...
    volume                 = cs47l24_dac_volume_index(decibels);
    mixer_volume_aif2rx    = cs47l24_mixer_volume_index(decibels - 6.0); /* summing stereo signal into mono */
    ...
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DAC_DIGITAL_VOLUME_4L  , CS47L24_OUT4L_VOL_MASK   , volume << CS47L24_OUT4L_VOL_SHIFT)                , result, _exit );
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DAC_DIGITAL_VOLUME_1L  , CS47L24_OUT_VU_MASK      , CS47L24_OUT_VU)                                   , result, _exit );
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DAC_DIGITAL_VOLUME_1R  , CS47L24_OUT_VU_MASK      , CS47L24_OUT_VU)                                   , result, _exit );
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DAC_DIGITAL_VOLUME_4L  , CS47L24_OUT_VU_MASK      , CS47L24_OUT_VU)                                   , result, _exit );

    /*
     * Adjust gain of AEC signal going into DSP3 based on playback gain
     */
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DSP3LMIX_INPUT_1_VOLUME, CS47L24_INPUT_VOLUME_MASK, mixer_volume_aif2rx << CS47L24_INPUT_VOLUME_SHIFT), result, _exit );
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_DSP3LMIX_INPUT_2_VOLUME, CS47L24_INPUT_VOLUME_MASK, mixer_volume_aif2rx << CS47L24_INPUT_VOLUME_SHIFT), result, _exit );
...
static wiced_result_t cs47l24_set_capture_volume(void *driver_data, double decibels)
{
    ...
    volume = cs47l24_adc_volume_index(decibels);
...
static wiced_result_t cs47l24_get_playback_volume_range(void *driver_data, double *min_volume_in_decibels, double *max_volume_in_decibels )
{
    ...
    *min_volume_in_decibels = CS47L24_VOLUME_OUT_DB_MIN;
    *max_volume_in_decibels = CS47L24_VOLUME_OUT_DB_MAX;
...
static wiced_result_t cs47l24_get_capture_volume_range(void *driver_data, double *min_volume_in_decibels, double *max_volume_in_decibels )
{
    ...
    *min_volume_in_decibels = CS47L24_VOLUME_IN_DB_MIN;
    *max_volume_in_decibels = CS47L24_VOLUME_IN_DB_MAX;
...
static wiced_result_t cs47l24_get_wupd_params(cs47l24_device_cmn_data_t* cs47l24, wiced_audio_wupd_params_t* params)
{
    ...
    WICED_VERIFY_GOTO( cs47l24_reg_read(cs47l24, CS47L24_ADC_DIGITAL_VOLUME_1L, &value_lo), result, _exit );
    params->mic_gain_db = cs47l24_adc_volume_in_db(((value_lo & CS47L24_IN1L_DIG_VOL_MASK) >> CS47L24_IN1L_DIG_VOL_SHIFT));

    WICED_VERIFY_GOTO( cs47l24_reg_read(cs47l24, CS47L24_EQ1MIX_INPUT_1_VOLUME, &value_lo), result, _exit );
    params->trigger_gain_db = cs47l24_mixer_volume_in_db(((value_lo & CS47L24_INPUT_VOLUME_MASK) >> CS47L24_INPUT_VOLUME_SHIFT));

    params->recording_gain_db = cs47l24_mixer_volume_in_db(rtd->aif1tx_gain);

    /* Write SC_FF_DUET_REG_LATCH_REQUEST to SC_FF_DUET_READREGID_0 (and 0x0 to SC_FF_DUET_READREGID_1) to force register latching */
    WICED_VERIFY_GOTO( cs47l24_dsp_word_write(cs47l24, SC_FF_DUET_READREGID_1, SC_FF_DUET_REG_LATCH_REQUEST), result, _exit );
    WICED_VERIFY_GOTO( cs47l24_reg_read(cs47l24, SC_FF_DUET_READREGID_0, &value_lo), result, _exit );
    loop_count = 0;
    while ((value_lo != SC_FF_DUET_REG_LATCH_COMPLETE) && (loop_count < CS47L24_READ_RETRY_COUNT))
    {
        wiced_rtos_delay_milliseconds(1);
        WICED_VERIFY_GOTO( cs47l24_reg_read(cs47l24, SC_FF_DUET_READREGID_0, &value_lo), result, _exit );
        loop_count++;
    ...
    /*
     * Then read freshly latched/refreshed registers
     * Most of the values are using a fixed point integer
     * that must be converted to a floating point
     */
    WICED_VERIFY_GOTO( cs47l24_dsp_word_read(cs47l24, SC_FF_DUET_APPPROCMODE_MODE_1, &params->sc_enabled), result, _exit );
    WICED_VERIFY_GOTO( cs47l24_dsp_word_read(cs47l24, SC_FF_DUET_TRPGA_TXGAINS4_1, &dsp_word), result, _exit );
    params->sc_passthrough_gain_db  = (dsp_word == 0) ? 0.0 : (double)dsp_word / (double)((uint32_t)(1 << SC_FF_DUET_TRPGA_TXGAINS4_FRAC_WIDTH));
    /*
     * Convert ratio back to dB
     */
    params->sc_passthrough_gain_db = isnormal(params->sc_passthrough_gain_db) ? (20 * log10(params->sc_passthrough_gain_db)) : 0.0;
    WICED_VERIFY_GOTO( cs47l24_dsp_word_read(cs47l24, SC_FF_DUET_SCEC1CONFIG_BULKDELAYMSS11_1, &dsp_word), result, _exit );
    params->aec_ref_bulk_delay_msec = (dsp_word == 0) ? 0.0 : (double)dsp_word / (double)((uint32_t)(1 << SC_FF_DUET_SCEC1CONFIG_BULKDELAYMSS11_FRAC_WIDTH));
    WICED_VERIFY_GOTO( cs47l24_dsp_word_read(cs47l24, SC_FF_DUET_SCEC1CONFIG_TXBULKDELAYMSS11_1, &dsp_word), result, _exit );
    params->aec_mic_bulk_delay_msec = (dsp_word == 0) ? 0.0 : (double)dsp_word / (double)((uint32_t)(1 << SC_FF_DUET_SCEC1CONFIG_TXBULKDELAYMSS11_FRAC_WIDTH));
    WICED_VERIFY_GOTO( cs47l24_dsp_word_read(cs47l24, SC_FF_DUET_SCVAD_DTDTHRESHS0_1, &dsp_word), result, _exit );
    params->dtd_threshold           = (dsp_word == 0) ? 0.0 : (double)dsp_word / (double)((uint32_t)(1 << SC_FF_DUET_SCVAD_DTDTHRESHS0_FRAC_WIDTH));
...
static wiced_result_t cs47l24_set_wupd_params(cs47l24_device_cmn_data_t* cs47l24, wiced_audio_wupd_params_t* params)
{
    ...
    volume = cs47l24_adc_volume_index(params->mic_gain_db);
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_ADC_DIGITAL_VOLUME_1L, CS47L24_IN1L_DIG_VOL_MASK, volume << CS47L24_IN1L_DIG_VOL_SHIFT), result, _exit );
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_ADC_DIGITAL_VOLUME_1R, CS47L24_IN1R_DIG_VOL_MASK, volume << CS47L24_IN1R_DIG_VOL_SHIFT), result, _exit );

    volume = cs47l24_mixer_volume_index(params->trigger_gain_db);
    WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_EQ1MIX_INPUT_1_VOLUME, CS47L24_INPUT_VOLUME_MASK, volume << CS47L24_INPUT_VOLUME_SHIFT), result, _exit );

    rtd->aif1tx_gain = cs47l24_mixer_volume_index(params->recording_gain_db);
    WICED_VERIFY_GOTO( cs47l24_reg_read(cs47l24, CS47L24_AIF1TX1MIX_INPUT_1_VOLUME, &volume), result, _exit );
    /*
     * Only apply new AIF1TX gain if it's not muted !
     */
    if (((volume & CS47L24_INPUT_VOLUME_MASK) >> CS47L24_INPUT_VOLUME_SHIFT) != 0)
    {
        WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_AIF1TX1MIX_INPUT_1_VOLUME, CS47L24_INPUT_VOLUME_MASK, rtd->aif1tx_gain << CS47L24_INPUT_VOLUME_SHIFT), result, _exit );
        WICED_VERIFY_GOTO( cs47l24_upd_bits(cs47l24, CS47L24_AIF1TX2MIX_INPUT_1_VOLUME, CS47L24_INPUT_VOLUME_MASK, rtd->aif1tx_gain << CS47L24_INPUT_VOLUME_SHIFT), result, _exit );
    }
    /*
     * Write to register first; force latching after
     * We have to convert floating point values to fixed point integers
     */
    WICED_VERIFY_GOTO( cs47l24_dsp_word_write(cs47l24, SC_FF_DUET_APPPROCMODE_MODE_1, params->sc_enabled), result, _exit );
    /* Convert dB to ratio first */
    params->sc_passthrough_gain_db = isnormal(params->sc_passthrough_gain_db) ? pow10(params->sc_passthrough_gain_db / 20) : 1.0;
    dsp_word =  isnormal(params->sc_passthrough_gain_db) ? (uint32_t)round(params->sc_passthrough_gain_db * (double)((uint32_t)(1 << SC_FF_DUET_TRPGA_TXGAINS4_FRAC_WIDTH))) : 0;
    WICED_VERIFY_GOTO( cs47l24_dsp_word_write(cs47l24, SC_FF_DUET_TRPGA_TXGAINS4_1, dsp_word & SC_FF_DUET_REG_MASK), result, _exit );
    dsp_word = isnormal(params->aec_ref_bulk_delay_msec) ? (uint32_t)round(params->aec_ref_bulk_delay_msec * (double)((uint32_t)(1 << SC_FF_DUET_SCEC1CONFIG_BULKDELAYMSS11_FRAC_WIDTH))) : 0;
    WICED_VERIFY_GOTO( cs47l24_dsp_word_write(cs47l24, SC_FF_DUET_SCEC1CONFIG_BULKDELAYMSS11_1, dsp_word & SC_FF_DUET_REG_MASK), result, _exit );
    dsp_word = isnormal(params->aec_mic_bulk_delay_msec) ? (uint32_t)round(params->aec_mic_bulk_delay_msec * (double)((uint32_t)(1 << SC_FF_DUET_SCEC1CONFIG_TXBULKDELAYMSS11_FRAC_WIDTH))) : 0;
    WICED_VERIFY_GOTO( cs47l24_dsp_word_write(cs47l24, SC_FF_DUET_SCEC1CONFIG_TXBULKDELAYMSS11_1, dsp_word & SC_FF_DUET_REG_MASK), result, _exit );
    dsp_word = isnormal(params->dtd_threshold) ? (uint32_t)round(params->dtd_threshold * (double)((uint32_t)(1 << SC_FF_DUET_SCVAD_DTDTHRESHS0_FRAC_WIDTH))) : 0;
    WICED_VERIFY_GOTO( cs47l24_dsp_word_write(cs47l24, SC_FF_DUET_SCVAD_DTDTHRESHS0_1, dsp_word & SC_FF_DUET_REG_MASK), result, _exit );

    /* Write SC_FF_DUET_REG_LATCH_REQUEST to SC_FF_DUET_WRITEREGID_0 (and 0x0to SC_FF_DUET_WRITEREGID_1) to force register latching */
    WICED_VERIFY_GOTO( cs47l24_dsp_word_write(cs47l24, SC_FF_DUET_WRITEREGID_1, SC_FF_DUET_REG_LATCH_REQUEST), result, _exit );
    WICED_VERIFY_GOTO( cs47l24_reg_read(cs47l24, SC_FF_DUET_WRITEREGID_0, &volume), result, _exit );
    loop_count = 0;
    while ((volume != SC_FF_DUET_REG_LATCH_COMPLETE) && (loop_count < CS47L24_READ_RETRY_COUNT))
    {
        wiced_rtos_delay_milliseconds(1);
        WICED_VERIFY_GOTO( cs47l24_reg_read(cs47l24, SC_FF_DUET_WRITEREGID_0, &volume), result, _exit );
        loop_count++;
...
static wiced_result_t cs47l24_ioctl(void *driver_data, wiced_audio_device_ioctl_t cmd, wiced_audio_device_ioctl_data_t *cmd_data)
{
    ...
    switch (cmd)
    {
        case WICED_AUDIO_IOCTL_GET_DATA_BUFFER:
            LOCK_RTD(rtd);
            if (cs47l24_type(dd) == CS47L24_DEVICE_TYPE_CAPTURE)
            {
                result = cs47l24_dsp_scc_get_audio(cs47l24, &cmd_data->data_buffer);
            }
            UNLOCK_RTD(rtd);
            break;

        case WICED_AUDIO_IOCTL_GET_DATA_OFFSET:
            if (cs47l24_type(dd) == CS47L24_DEVICE_TYPE_CAPTURE)
            {
                result = cs47l24_dsp_scc_get_offset(cs47l24, &cmd_data->data_offset);
            }
            break;

        case WICED_AUDIO_IOCTL_REG_READ:
            result = cs47l24_reg_read(cs47l24, cmd_data->register_value.address, &val);
            if (result == WICED_SUCCESS)
            {
                cmd_data->register_value.value = val;
            }
            else
            {
                cmd_data->register_value.value = 0;
            }
            break;

        case WICED_AUDIO_IOCTL_REG_WRITE:
            result = cs47l24_reg_write(cs47l24, cmd_data->register_value.address, cmd_data->register_value.value);
            break;

        case WICED_AUDIO_IOCTL_GET_WUPD_PARAMS:
            LOCK_RTD(rtd);
#ifdef CS47L24_ENABLE_DSP
            result = cs47l24_get_wupd_params(cs47l24, &cmd_data->wupd_params);
#else /* !CS47L24_ENABLE_DSP */
            result = WICED_UNSUPPORTED;
#endif /* CS47L24_ENABLE_DSP */
            UNLOCK_RTD(rtd);
            break;

        case WICED_AUDIO_IOCTL_SET_WUPD_PARAMS:
            LOCK_RTD(rtd);
#ifdef CS47L24_ENABLE_DSP
            result = cs47l24_set_wupd_params(cs47l24, &cmd_data->wupd_params);
#else /* !CS47L24_ENABLE_DSP */
            result = WICED_UNSUPPORTED;
#endif /* CS47L24_ENABLE_DSP */
            UNLOCK_RTD(rtd);
        ...
        case WICED_AUDIO_IOCTL_GET_WUPD_STREAMING_MODE:
            LOCK_RTD(rtd);
#ifdef CS47L24_ENABLE_DSP
            result = cs47l24_dsp_scc_get_streaming_mode(cs47l24, &cmd_data->wupd_mode);
#else /* !CS47L24_ENABLE_DSP */
            result = WICED_UNSUPPORTED;
#endif /* CS47L24_ENABLE_DSP */
            UNLOCK_RTD(rtd);
        ...
        case WICED_AUDIO_IOCTL_SET_WUPD_STREAMING_MODE:
            LOCK_RTD(rtd);
#ifdef CS47L24_ENABLE_DSP
            result = cs47l24_dsp_scc_set_streaming_mode(cs47l24, &cmd_data->wupd_mode);
#else /* !CS47L24_ENABLE_DSP */
            result = WICED_UNSUPPORTED;
#endif /* CS47L24_ENABLE_DSP */
            UNLOCK_RTD(rtd);
