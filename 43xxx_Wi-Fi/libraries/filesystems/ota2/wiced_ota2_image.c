...
wiced_result_t wiced_ota2_image_extract ( wiced_ota2_image_type_t ota_type )
{
    ...
    for (index = 0; index < ota2_header.component_count; index++)
    {
        ...
        mini_printf("extracting (%d of %d): %s\r\n", (index+1), ota2_header.component_count, component_header.name);
/*
 * More debug
 *
 *      mini_printf("        entry: %d of %d\r\n", index, ota2_header.component_count);
 *      mini_printf("         type: %d\r\n", component_header.type);
 *      mini_printf("  compression: %d\r\n", component_header.compression);
 *      mini_printf("       source: 0x%x\r\n", component_header.source_offset);
 *      mini_printf("         size: 0x%x\r\n", component_header.source_size);
 *      mini_printf("  destination: 0x%x\r\n", component_header.destination);
 *      mini_printf("         size: 0x%x\r\n", component_header.destination_size);
 *      mini_printf("          crc: 0x%x\r\n", component_header.crc);
 */

        /* decompress component and write to FLASH */
        /* need to do this in chunks  !!! The file system is > 800k !!! */
        /* source size best be less than the image size! */
        if ( (component_header.source_size > 0) && (component_header.source_size < ota2_header.image_size ))
        {
