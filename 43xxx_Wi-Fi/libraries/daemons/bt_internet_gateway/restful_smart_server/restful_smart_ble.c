...
{
    uint32_t                   passkey;
    wiced_bt_smp_sc_oob_data_t secure_oob_data;
    ...
    switch ( pairing_info->expected_action )
    {
        ...
        case LE_SECURE_OOB_EXPECTED:
        {
            ...
            if ( all_params_available == WICED_TRUE )
            {
                ...
                memcpy( &pairing_info->pairing_data.secure_oob_data.peer_oob_data.addr_rcvd_from, &client_response->bdaddrb, sizeof( pairing_info->pairing_data.secure_oob_data.peer_oob_data.addr_rcvd_from ) );
            }
    ...
    switch ( event )
    {
        ...
        case BTM_SMP_SC_REMOTE_OOB_DATA_REQUEST_EVT:
        {
            ...
            if ( pairing_info != NULL )
            {
                ...
                wiced_rtos_lock_mutex( &list_mutex );
                wiced_bt_smp_sc_oob_reply( (wiced_bt_smp_sc_oob_data_t*)&pairing_info->pairing_data.secure_oob_data );
            }
            wiced_rtos_unlock_mutex( &list_mutex );
            break;
        }
