...
#define WICED_OTA2_DNS_TIMEOUT_MS               (  1000 )
...
static wiced_result_t wiced_ota2_service_network_switch_to_default_ap(wiced_ota2_service_session_t* session )
{
    ...
    if (session->ota2_ap_info == NULL && session->ota2_ap_list == NULL)
    {
        /*
         * Both ota2_ap_info and ota2_ap_list are NULL. Don't touch the network.
         */
    ...
    {
        /* No need to connect to an AP using Ethernet !!! */
        return WICED_SUCCESS;
    }
...
static wiced_result_t ota2_service_connect(wiced_ota2_service_session_t* session)
{
    ...
    wiced_log_msg(WLF_OTA2, WICED_LOG_DEBUG0, "Connect to: host:%s path:%s port:%d\r\n",
                  session->host_name, session->file_path, session->http_port);

    connect_tries = 0;
    result        = WICED_ERROR;
    while ((connect_tries++ < 3) && (result != WICED_SUCCESS))
    {
        result = wiced_hostname_lookup(session->host_name, &ip_address, WICED_OTA2_DNS_TIMEOUT_MS, session->ota2_interface);
    }
    if (result!= WICED_SUCCESS)
    {
    ...
    while ((connect_tries < 3) && (result != WICED_SUCCESS))
    {
        wiced_log_msg( WLF_OTA2, WICED_LOG_INFO, "Try %d Connecting to %s  " IPV4_PRINT_FORMAT " : %d !\r\n",
                       connect_tries, session->host_name, IPV4_SPLIT_TO_PRINT(ip_address), session->http_port );

        result = wiced_tcp_connect( &session->tcp_socket, &ip_address, session->http_port, WICED_OTA2_TCP_CONNECT_TIMEOUT_MS );
        connect_tries++;
    }
    if ( result != WICED_SUCCESS )
    {
    ...
    wiced_log_msg( WLF_OTA2, WICED_LOG_INFO, "Connected to " IPV4_PRINT_FORMAT " : %d !\r\n",
                                               IPV4_SPLIT_TO_PRINT(ip_address), session->http_port);

    strlcpy(session->last_connected_host_name, session->host_name, sizeof(session->last_connected_host_name));
...
static wiced_result_t wiced_ota2_service_wget_update(wiced_ota2_service_session_t* session, uint8_t *req_buffer, uint32_t req_length)
{
    ...
    snprintf(port_name, sizeof(port_name), ":%d", session->http_port);

    snprintf(session->http_query, sizeof(session->http_query), ota2_get_request_template, filename, hostname, port_name);

    wiced_log_msg( WLF_OTA2, WICED_LOG_INFO, "Send query for OTA file: [%s]\r\n", session->http_query);
    ...
    while ((result == WICED_SUCCESS) && (done == WICED_FALSE))
    {
        ...
        else if (reply_packet != NULL)
        {
            ...
            /* We only want to find an HTTP header body when we are at the beginning of a packet */
            if (offset == 0)
            {
            ...
            {
                result = WICED_ERROR;   /* force next section to run */
            }
...
wiced_result_t wiced_ota2_service_get_the_update(wiced_ota2_service_session_t* session)
{
    ...
    else
    {
        /*
         * Both ota2_ap_info and ota2_ap_list are NULL. Don't touch the network,
         * just try and get the update file.
         */
...
void wiced_ota2_worker_thread(uint32_t arg)
{
    ...
    while(1)
    {
        ...
        if (events & OTA2_EVENT_WORKER_CHECK_FOR_UPDATES)
        {
            ...
            if (cb_result == WICED_SUCCESS)
            {
                session->session_download_now_time = curr_utc_time;
                if (session->ota2_ap_info != NULL || session->ota2_ap_info != NULL)
                {
                    ...
                    /* determine info for next time from current time */
                    session->session_download_now_time += WAIT_FOR_APP_TO_SHUTDOWN_NETWORK_MS;
                }
                else
                {
                    /*
                     * We aren't touching the network configuration, proceed directly.
                     */
...
void*  wiced_ota2_service_init(wiced_ota2_backround_service_params_t *params, void* opaque)
{
    ...
    if (params->update_url == NULL)
    {
        WPRINT_LIB_ERROR(("OTA2: BAD update URL!\r\n"));
        return NULL;
    }
    ...
    wiced_ota2_service_uri_split(params->update_url, new_session->host_name, sizeof(new_session->host_name) - 1,
                                 new_session->file_path, sizeof(new_session->file_path) - 1, &new_session->http_port);
...
wiced_result_t wiced_ota2_service_uri_split(const char* uri, char* host_buff, uint16_t host_buff_len, char* path_buff, uint16_t path_buff_len, uint16_t* port)
{
  ...
  *port = 0;
  ...
  else
  {
      if (tls)
      {
          *port = WICED_OTA2_HTTPS_PORT;
      }
      else
      {
          *port = WICED_OTA2_HTTP_PORT;
      }
      host_end = strchr(host_start, '/');
  }
