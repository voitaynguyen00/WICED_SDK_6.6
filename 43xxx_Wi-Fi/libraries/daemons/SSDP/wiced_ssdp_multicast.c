...
wiced_result_t wiced_ssdp_mcast_collate_mcast_responses(wiced_ssdp_internal_t* internal, wiced_ssdp_callback_info_t * cb_info)
{
    ...
    if( internal->unique_responses_received >=  internal->response_array_size)
    {
        return WICED_ERROR;
    }
    ...
    temp_response.additional_hdrs = (wiced_ssdp_hdr_t*) calloc(internal->num_additional_hdrs, sizeof(wiced_ssdp_hdr_t));
    if (temp_response.additional_hdrs == NULL)
    {
    ...
    for( i = 0; i < cb_info->request_num_headers; i++)
    {
        ...
        {
            strncpy(temp_response.cache_control, cb_info->request_headers[i].value, sizeof(temp_response.cache_control)-1);
            temp_response.cache_control[sizeof(temp_response.cache_control)-1] = '\0';
        }
        else if (strcasecmp(cb_info->request_headers[i].name, "LOCATION") == 0)
        {
            strncpy(temp_response.location, cb_info->request_headers[i].value, sizeof(temp_response.location)-1);
            temp_response.location[sizeof(temp_response.location)-1] = '\0';
        }
        else if (strcasecmp(cb_info->request_headers[i].name, "ST") == 0)
        {
            strncpy(temp_response.st, cb_info->request_headers[i].value, sizeof(temp_response.st)-1);
            temp_response.st[sizeof(temp_response.st)-1] = '\0';
        }
        else if (strcasecmp(cb_info->request_headers[i].name, "USN") == 0)
        {
            strncpy(temp_response.usn, cb_info->request_headers[i].value, sizeof(temp_response.usn)-1);
            temp_response.usn[sizeof(temp_response.usn)-1] = '\0';
        }
        else if (internal->additional_hdr_names != NULL)
        {
            for(m = 0; m < internal->num_additional_hdrs; m++)
            {
                if ((strcasecmp(cb_info->request_headers[i].name, internal->additional_hdr_names[m].hdr_name) == 0) &&
                    (n < internal->num_additional_hdrs))
                {
                    strncpy(temp_response.additional_hdrs[n].hdr_name, cb_info->request_headers[i].name, SSDP_ADDNL_HDR_NAME_SIZE_MAX);
                    temp_response.additional_hdrs[n].hdr_name[SSDP_ADDNL_HDR_NAME_SIZE_MAX] = '\0';
                    strncpy(temp_response.additional_hdrs[n].hdr_val, cb_info->request_headers[i].value, SSDP_ADDNL_HDR_VALUE_SIZE_MAX);
                    temp_response.additional_hdrs[n].hdr_val[SSDP_ADDNL_HDR_VALUE_SIZE_MAX] = '\0';
                    n++;
                }
            }
        }
    ...
    if (internal->unique_responses_received > 0)
    {
        ...
        for (i = 0; i < internal->unique_responses_received; i++)
        {
            ...
            /* Compare the basic headers first */
            size = sizeof(temp_response) - sizeof(wiced_ssdp_hdr_t*);
            if(memcmp(&temp_response, response, size) == 0)
            {
                /* Compare the additional headers */
                size = (sizeof(wiced_ssdp_hdr_t) * (internal->num_additional_hdrs));
                if(memcmp(temp_response.additional_hdrs, response->additional_hdrs, size) == 0)
                {
    ...
    {
        size = sizeof(temp_response) - sizeof(wiced_ssdp_hdr_t*);
        memcpy(&internal->responses[internal->unique_responses_received], &temp_response, size);
        size = (sizeof(wiced_ssdp_hdr_t) * (internal->num_additional_hdrs));
        memcpy(internal->responses[internal->unique_responses_received].additional_hdrs, temp_response.additional_hdrs, size);
        internal->unique_responses_received++;
    }

    free(temp_response.additional_hdrs);
    temp_response.additional_hdrs = NULL;
...
wiced_result_t wiced_ssdp_send_msearch_wait_for_results(void *ssdp_info, wiced_ssdp_msearch_params_t *in_params)
{
    ...
    internal->num_additional_hdrs   = in_params->num_additional_hdrs;
    internal->additional_hdr_names  = in_params->additional_hdr_names;
...
wiced_result_t wiced_ssdp_multicast_send_notify(wiced_ssdp_internal_t* internal, wiced_bool_t byebye)
{
    ...
    if (byebye != WICED_TRUE)
    {
        ...
        if ((internal->mcast_notify_count >= WICED_SSDP_MCAST_INITIAL_NOTIFY_FAST_COUNT) ||
                (internal->is_periodic_mcast_notify == WICED_TRUE))
        {
            ...
            if(internal->is_periodic_mcast_notify == WICED_FALSE)
            {
                WPRINT_SSDP_DEBUG(SSDP_LOG_LOW,("Done with initial quick notify messages\r\n"));
                internal->is_periodic_mcast_notify = WICED_TRUE; //prevent quick notify when notify_count overflows to 0
            }
        }
...
wiced_result_t wiced_ssdp_mcast_build_default_reply(wiced_ssdp_internal_t* internal, wiced_ssdp_callback_info_t * cb_info)
{
    ...
    if (strncasecmp(cb_info->request_buffer, "NOTIFY", 6) == 0)
    {
        ...
        if (internal->notify_callback != NULL)
        {
            ...
            if (internal->num_additional_hdrs > 0)
            {
                notify_info.additional_hdrs = (wiced_ssdp_hdr_t*)calloc(sizeof(wiced_ssdp_hdr_t), internal->num_additional_hdrs);
                if (notify_info.additional_hdrs == NULL)
                {
                    return WICED_OUT_OF_HEAP_SPACE;
                }
            }
            ...
            for( i = 0; i < cb_info->request_num_headers; i++)
            {
                if (strcasecmp(cb_info->request_headers[i].name, "CACHE-CONTROL") == 0)
                {
                    strncpy(notify_info.cache_control, cb_info->request_headers[i].value, sizeof(notify_info.cache_control)-1);
                    notify_info.cache_control[sizeof(notify_info.cache_control)-1] = '\0';
                }
                else if (strcasecmp(cb_info->request_headers[i].name, "LOCATION") == 0)
                {
                    strncpy(notify_info.location, cb_info->request_headers[i].value, sizeof(notify_info.location)-1);
                    notify_info.location[sizeof(notify_info.location)-1] = '\0';
                }
                else if (strcasecmp(cb_info->request_headers[i].name, "SERVER") == 0)
                {
                    strncpy(notify_info.server, cb_info->request_headers[i].value, sizeof(notify_info.server)-1);
                    notify_info.server[sizeof(notify_info.server)-1] = '\0';
                }
                else if (strcasecmp(cb_info->request_headers[i].name, "NT") == 0)
                {
                    strncpy(notify_info.nt, cb_info->request_headers[i].value, sizeof(notify_info.nt)-1);
                    notify_info.nt[sizeof(notify_info.nt)-1] = '\0';
                }
                else if (strcasecmp(cb_info->request_headers[i].name, "NTS") == 0)
                {
                    strncpy(notify_info.nts, cb_info->request_headers[i].value, sizeof(notify_info.nts)-1);
                    notify_info.nts[sizeof(notify_info.nts)-1] = '\0';
                }
                else if (strcasecmp(cb_info->request_headers[i].name, "USN") == 0)
                {
                    strncpy(notify_info.usn, cb_info->request_headers[i].value, sizeof(notify_info.usn)-1);
                    notify_info.usn[sizeof(notify_info.usn)-1] = '\0';
                }
                else if (internal->additional_hdr_names != NULL)
                {
                    for(m = 0; m < internal->num_additional_hdrs; m++)
                    {
                        if ((strcasecmp(cb_info->request_headers[i].name, internal->additional_hdr_names[m].hdr_name) == 0) &&
                            (n < internal->num_additional_hdrs))
                        {
                            strncpy(notify_info.additional_hdrs[n].hdr_name, cb_info->request_headers[i].name, SSDP_ADDNL_HDR_NAME_SIZE_MAX);
                            notify_info.additional_hdrs[n].hdr_name[SSDP_ADDNL_HDR_NAME_SIZE_MAX] = '\0';
                            strncpy(notify_info.additional_hdrs[n].hdr_val, cb_info->request_headers[i].value, SSDP_ADDNL_HDR_VALUE_SIZE_MAX);
                            notify_info.additional_hdrs[n].hdr_val[SSDP_ADDNL_HDR_VALUE_SIZE_MAX] = '\0';
                            n++;
                        }
                    }
                }
            ...
            if (internal->num_additional_hdrs > 0)
            {
                free(notify_info.additional_hdrs);
            }
        }
...
wiced_result_t wiced_ssdp_internal_mcast_start( wiced_ssdp_internal_t*  internal, wiced_interface_t interface )
{
    ...
    /* coverity-77204 [Out-of-bounds access]
                      FALSE-POSITIVE: In both places, same variable of type 'wiced_udp_socket_t' used. So there is no chance of buffer over run.
                      Coverity tool seems to report overrun of 140bytes to 144 falsely probably because structure size is calculated in two different RTOSes
                      one while passing the value and other while receiving the value.*/
