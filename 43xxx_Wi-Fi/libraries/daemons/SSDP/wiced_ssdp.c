...
wiced_result_t wiced_ssdp_get_uuid(char *uuid_string, uint16_t uuid_string_len)
{
    ...
    if ( (uuid_string == NULL) || (uuid_string_len < WICED_SSDP_UUID_MAX) || (WICED_SSDP_UUID_MAX < sizeof( SSDP_UUID_TEMPLATE )) )
    {
        return WICED_BADARG;
    }

    strncpy( uuid_string, SSDP_UUID_TEMPLATE, sizeof( SSDP_UUID_TEMPLATE ) );
    strncat( uuid_string, mac_str, WICED_SSDP_UUID_MAX - strlen( uuid_string ) - 1 );
    uuid_string[WICED_SSDP_UUID_MAX - 1] = '\0';
...
wiced_result_t wiced_ssdp_v4_ip_addr_to_string(wiced_ip_address_t *addr, char* buff, uint16_t buff_len)
{
    ...
    {
        strncpy( buff, "Invalid IP addr", buff_len - 1 );
        buff[buff_len - 1] = '\0';
    }
...
wiced_result_t wiced_ssdp_deinit( void* ssdp_info )
{
    ...
    while (wiced_rtos_is_queue_empty( &internal->ssdp_queue ) != WICED_SUCCESS)
    {
        ...
        if(result != WICED_SUCCESS)
        {
            WPRINT_SSDP_DEBUG(SSDP_LOG_INFO,("Error[%d] while getting event from queue\n", result));
        }
    }
...
wiced_result_t wiced_ssdp_dump_debug_info( void* ssdp_info )
{
    ...
    WPRINT_SSDP_DEBUG(SSDP_LOG_OFF,("  is_periodic_mcast_notify   : %s\r\n", internal->is_periodic_mcast_notify? "TRUE": "FALSE"));
