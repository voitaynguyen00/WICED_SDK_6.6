...
#define SSDP_ADDNL_HDR_NAME_SIZE_MAX      (20)
#define SSDP_ADDNL_HDR_VALUE_SIZE_MAX     (50)
...
/**
 * Structure to hold additional header names of SSDP used for M-SEARCH request
 */
typedef struct wiced_ssdp_hdr_name_s
{
    char                hdr_name[SSDP_ADDNL_HDR_NAME_SIZE_MAX+1];
} wiced_ssdp_hdr_name_t;

/**
 * Structure to hold additional headers of SSDP used for M-SEARCH response
 */
typedef struct wiced_ssdp_hdr_s
{
    char                hdr_name[SSDP_ADDNL_HDR_NAME_SIZE_MAX+1];
    char                hdr_val[SSDP_ADDNL_HDR_VALUE_SIZE_MAX+1];
} wiced_ssdp_hdr_t;
 ...
 *  For additional_hdrs, app has to allocate and provide the memory
 */
typedef struct wiced_ssdp_msearch_response_s
{
    ...
    wiced_ssdp_hdr_t*   additional_hdrs;                                        /* Other headers in the response    */
...
typedef struct wiced_ssdp_notify_info_s
{
    ...
    char                usn[WICED_SSDP_USN_MAX + 1];                            /* Unique Service Name              */
    wiced_ssdp_hdr_t*   additional_hdrs;                                        /* Other headers in the NOTIFY packet */
...
typedef struct wiced_ssdp_msearch_params_s
{
    ...
    wiced_ssdp_hdr_name_t*          additional_hdr_names;           /* provided by application                          */
    uint16_t                        num_additional_hdrs;            /* provided by application                          */
