...
#define HTTP_SERVER_RECEIVE_TIMEOUT    (WICED_NO_WAIT)
...
wiced_result_t wiced_http_response_stream_write_resource( wiced_http_response_stream_t* stream, const resource_hnd_t* res_id )
{
    ...
    do
    {
        resource_result_t resource_result = resource_get_readonly_buffer ( res_id, pos, max_size, &res_size, &data );
...
static wiced_result_t http_server_parse_receive_packet( wiced_http_server_t* server, wiced_http_stream_t* stream, wiced_packet_t* packet )
{
    ...
    if ( server->receive_callback != NULL )
    {
...
#ifdef ENABLE_DEBUG
        {
            ...
            HTTP_DEBUG(("Payload len = [%d] and the data in Hex = [", available_data_length));
    ...
    if ( status_code == HTTP_200_TYPE )
    {
        ...
        switch ( page_found->url_content_type )
        {
            ...
            case WICED_STATIC_URL_CONTENT:
                ...
                WICED_VERIFY( wiced_http_response_stream_write( &stream->response, page_found->url_content.static_data.ptr, page_found->url_content.static_data.length ) );
                WICED_VERIFY( wiced_http_response_stream_flush( &stream->response ) );
                break;

            case WICED_RAW_STATIC_URL_CONTENT: /* This is just a Location header */
                HTTP_DEBUG((" %s() : ----- ### DBG : WICED_RAW_STATIC_URL_CONTENT\n", __FUNCTION__));
                WICED_VERIFY( wiced_http_response_stream_write( &stream->response, HTTP_HEADER_301, strlen( HTTP_HEADER_301 ) ) );
                WICED_VERIFY( wiced_http_response_stream_write( &stream->response, CRLF, strlen( CRLF ) ) );
                WICED_VERIFY( wiced_http_response_stream_write( &stream->response, HTTP_HEADER_LOCATION, strlen( HTTP_HEADER_LOCATION ) ) );
                WICED_VERIFY( wiced_http_response_stream_write( &stream->response, page_found->url_content.static_data.ptr, page_found->url_content.static_data.length ) );
                WICED_VERIFY( wiced_http_response_stream_write( &stream->response, CRLF, strlen( CRLF ) ) );
                WICED_VERIFY( wiced_http_response_stream_write( &stream->response, HTTP_HEADER_CONTENT_LENGTH, strlen( HTTP_HEADER_CONTENT_LENGTH ) ) );
                WICED_VERIFY( wiced_http_response_stream_write( &stream->response, "0", 1 ) );
                WICED_VERIFY( wiced_http_response_stream_write( &stream->response, CRLF_CRLF, strlen( CRLF_CRLF ) ) );
                WICED_VERIFY( wiced_http_response_stream_flush( &stream->response ) );
                break;
...
static void http_server_event_thread_main( uint32_t arg )
{
    ...
    {
        HTTP_DEBUG((" L%d : %s() : Waiting for HTTP(S) event\r\n", __LINE__, __FUNCTION__));
        ...
        switch ( current_event.event_type )
        {
            ...
            case SOCKET_PACKET_RECEIVED_EVENT:
            {
                ...
                res = wiced_tcp_receive( socket, &packet, HTTP_SERVER_RECEIVE_TIMEOUT );
                if ( res == WICED_SUCCESS )
                {
                    HTTP_DEBUG((" L%d : %s() : Read packet = [0x%08lX]\r\n", __LINE__, __FUNCTION__, (unsigned int)packet));
                    ...
                    if ( linked_list_find_node( &server->active_stream_list, http_server_compare_stream_socket, (void*)socket, (linked_list_node_t**)&stream ) != WICED_SUCCESS )
                    {
                        ...
                        if (res != WICED_SUCCESS)
                        {
                            ...
                            if (res != WICED_SUCCESS)
                            {
                                ...
                                HTTP_DEBUG((" L%d : %s() : Delete packet = [0x%08lX]\r\n", __LINE__, __FUNCTION__, (unsigned int)packet));
                ...
                {
                    HTTP_DEBUG((" ### DBG : Error in wiced_tcp_receive(). Error = [%d]\r\n", res));
                }

                HTTP_DEBUG((" L%d : %s() : Receive event processed. Res = [%d]\r\n", __LINE__, __FUNCTION__, res));
                break;
            }
...
static wiced_result_t http_server_deferred_connect_callback( void* arg )
{
    ...
    HTTP_DEBUG((" L%d : %s() : Connect request received. Process it\r\n", __LINE__, __FUNCTION__));
    ...
    if ( result == WICED_SUCCESS )
    {
        HTTP_DEBUG((" L%d : %s() : Error in accepting the connection. Error = [%d]\r\n", __LINE__, __FUNCTION__, result));
        http_server_receive_callback ( socket, server );
    }

    HTTP_DEBUG((" L%d : %s() : Processed connect request\r\n", __LINE__, __FUNCTION__));
    return result;
}
