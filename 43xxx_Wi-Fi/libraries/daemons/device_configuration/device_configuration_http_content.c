                ...
                case CONFIG_IPADDR_DATA:
                    {
                        ...
                        wiced_dct_read_lock( (void**)&ipaddr, WICED_FALSE, DCT_APP_SECTION, config_entry->dct_offset, config_entry->data_size );
                        snprintf(temp_buf, sizeof(temp_buf), "%u.%u.%u.%u", (uint8_t)((ipaddr->ip.v4 >> 24) & 0xFF), (uint8_t)((ipaddr->ip.v4 >> 16) & 0xFF),
                                 (uint8_t)((ipaddr->ip.v4 >> 8) & 0xFF), (uint8_t)(ipaddr->ip.v4 & 0xFF));
                        temp_buf[sizeof(temp_buf) - 1] = '\0';
                        wiced_dct_read_unlock( ipaddr, WICED_FALSE );
                        wiced_tcp_stream_write(stream, temp_buf, strlen(temp_buf));
    ...
    if ( app_configuration != NULL )
    {
        ...
        if ( app_dct != NULL )
        {
            while ( url_parameters[0] == 'v' && url_parameters[3] == '=' )
            {
                ...
                {
                    case CONFIG_STRING_DATA:
                        memcpy( (uint8_t*) ( app_dct + config_entry->dct_offset - earliest_offset ), &url_parameters[4], (size_t) ( end_of_value - &url_parameters[4] ) );
                        ( (uint8_t*) ( app_dct + config_entry->dct_offset - earliest_offset ) )[end_of_value - &url_parameters[4]] = 0;
                        break;
                    case CONFIG_IPADDR_DATA:
                        ipaddr = (wiced_ip_address_t *)(app_dct + config_entry->dct_offset - earliest_offset);
                        for (i = 0; i < sizeof(temp_buf) - 2; i++)
                        {
                            if (!isdigit(url_parameters[4 + i]) && url_parameters[4 + i] != '.')
                            {
                                break;
                            }
                            temp_buf[i] = url_parameters[4 + i];
                        }
                        temp_buf[i] = '\0';
                        if (str_to_ip((const char *)temp_buf, &ip_address) == 0)
                        {
                            *ipaddr = ip_address;
                        }
                        break;
