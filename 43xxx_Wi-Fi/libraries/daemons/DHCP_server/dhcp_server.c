...
wiced_result_t wiced_start_dhcp_server(wiced_dhcp_server_t* server, wiced_interface_t interface)
{
    ...
    /* Start DHCP server */
    result = wiced_rtos_create_thread(&server->thread, DHCP_THREAD_PRIORITY, "DHCPserver", dhcp_thread, DHCP_STACK_SIZE, server);

    if ( WICED_SUCCESS != result )
    {
        WICED_VERIFY( wiced_udp_delete_socket( &server->socket ) );
    }
