...
#if !defined(ENABLE_NXD_BSD_SOCKET)
#include "nx_api.h"
#include "sockets.h"
...
INT send( INT sockID, const CHAR *msg, INT msgLength, INT flags )
{
    switch ( sockets[sockID].type )
    {
        case BSD_SOCKET_TCP:
        {
            ...
            {
                INT data_length;
                if ( nx_packet_allocate( &wiced_packet_pools[0], &packet, NX_IPv4_TCP_PACKET, NX_WAIT_FOREVER ) != NX_SUCCESS )
                {
                    return BSD_ERROR;
                }
                data_length = MIN( msgLength, sockets[sockID].socket.tcp.nx_tcp_socket_connect_mss );
        ...
        {
            NX_PACKET* packet;
            if ( nx_packet_allocate( &wiced_packet_pools[0], &packet, NX_IPv4_UDP_PACKET, NX_WAIT_FOREVER ) != NX_SUCCESS )
            {
                return BSD_ERROR;
            }
