...
static int retval_command( int argc, char* argv[] );
...
static cmd_err_t last_err = ERR_CMD_OK;
    ...
    setvbuf( stderr, NULL, _IONBF, 0 );
     ...
     while ( 1 )
     {
        ...
        while ( 1 )
        {
            ...
            {
                /* PAD mode is enabled, release the semaphore and wait for the mode to be disabled */
                break;
            }
#endif
...
wiced_result_t command_console_init_cfg( command_console_cfg_t *cfg )
{
    ...
    /* sanity check: do not allow the console to be hihger priority of the network layer */
    if( (cfg->thread_priority <= WICED_NETWORK_WORKER_PRIORITY) ||
        (cfg->thread_priority >= 9 /*idle*/) )
    {
        return WICED_ERROR;
    }
    ...
    if ( cfg->uart == WICED_UART_MAX )
    {
        return WICED_SUCCESS;
    }

    if ( cfg->uart != STDIO_UART )
    {
        ...
        result = wiced_uart_init(cfg->uart, &uart_console_config, &cons.console_rx_ring_buffer);
        if ( result != WICED_SUCCESS )
        {
            return result;
        }
    ...
    result = wiced_rtos_create_thread( &cons.console_thread,
                                       cfg->thread_priority,
                                       "console", console_thread_func, CONSOLE_THREAD_STACK_SIZE, NULL );
...
wiced_result_t command_console_init( wiced_uart_t uart, uint32_t line_len, char* buffer, uint32_t history_len, char* history_buffer_ptr, const char* delimiter_string )
{
    ...
    command_console_cfg_t console_cfg;

    console_cfg.uart               = uart;
    console_cfg.line_len           = line_len;
    console_cfg.buffer             = buffer;
    console_cfg.history_len        = history_len;
    console_cfg.history_buffer_ptr = history_buffer_ptr;
    console_cfg.delimiter_string   = delimiter_string;
    console_cfg.thread_priority    = WICED_DEFAULT_LIBRARY_PRIORITY;
    console_cfg.params_num         = MIN_PARAMS;

    result = command_console_init_cfg( &console_cfg);
...
cmd_err_t console_parse_cmd( const char* line )
{
    ...
    /* params buffer */
    params = calloc(cons.params_num, sizeof(char*));
    if ( NULL == params )
    {
        return WICED_ERROR;
    }
