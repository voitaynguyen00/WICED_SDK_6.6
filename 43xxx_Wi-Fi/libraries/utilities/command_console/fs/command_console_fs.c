...
static mbedtls_aes_xts_context* ctx = NULL;
/******************************************************
...
int aes_xts_init(int argc, char* argv[])
{
    if(ctx == NULL)
    {
        ctx = malloc(sizeof(mbedtls_aes_xts_context));
        if(ctx == NULL)
        {
...
int aes_xts_deinit(int argc, char* argv[])
{
    /* Initialize AES XTS */
...
int aes_xts_decrypt(int argc, char* argv[])
{
    wiced_file_t          file_handle = {0};
    char*                 local_buffer;
    wiced_result_t        result;
    ...
    char*                 src_file_name;
    char*                 tgt_file_name;
    uint64_t              file_size_remaining = 0;
    ...
    if( argc < 3)
    {
        printf( "aes_xts_decrypt <source file name> <target file name>\n" );
        return -1;
    }
    ...
    if ( current_filesystem_is_mounted != WICED_TRUE )
    {
        printf( "Filesystem not currently mounted, run mount command \n" );
        return ERR_UNKNOWN;
    }

    if(ctx == NULL)
    {
        printf("AES XTS context not initialized, run aes_xts_init command \n");
        return -1;
    }

    /* Set AES XTS decryption key */
    ret = mbedtls_aes_xts_setkey_dec( ctx, aes_test_xts_key, ((sizeof(aes_test_xts_key)) * 8));
    if( ret != 0)
    {
    ...
    /* Open the source file.  */
    result = wiced_filesystem_file_open( &current_filesystem_handle, &file_handle, src_file_name, WICED_FILESYSTEM_OPEN_FOR_READ );
    if ( result != WICED_SUCCESS )
    {
        printf( "Failed to open file %s\n", src_file_name );
        return ERR_UNKNOWN;
    ...
    unsigned char plain_text[data_length_read];

    if( data_length_read > 0 )
    {
        /* Decrypt plaintext with given data_unit */
        ret = mbedtls_aes_crypt_xts( ctx, MBEDTLS_AES_DECRYPT, data_length_read, aes_test_xts_data_unit, (unsigned char*) local_buffer, plain_text );
        if( ret != 0 )
        {
        ...
        /* Copy decrypted data to local buffer */
        memcpy(local_buffer, plain_text, data_length_read);

        /* create target file to write decrypted data to it */
        create_test_file(tgt_file_name, 0);

        /* Open the target file.  */
        result = wiced_filesystem_file_open( &current_filesystem_handle, &file_handle, tgt_file_name, WICED_FILESYSTEM_OPEN_ZERO_LENGTH );
        if ( result != WICED_SUCCESS )
        {
            printf( "Failed to open file %s\n", src_file_name );
            return ERR_UNKNOWN;
        ...
        printf("Writing [%d] bytes to [%s] file \n", (int)data_length_read, tgt_file_name);

        file_size_remaining = data_length_read;
        while ( file_size_remaining > 0 )
        {
            uint64_t bytes_written;
            requested_length = MIN( file_size_remaining, LOCAL_BUFFER_SIZE );
        ...
        printf("Wrote [%d] bytes to [%s] file successfully \n", (int) total_bytes_written, tgt_file_name);
...
int aes_xts_encrypt (int argc, char* argv[])
{
    wiced_result_t      result;
    uint64_t            file_size_remaining;
    uint64_t            total_bytes_written = 0;
    ...
    if( argc < 3)
    {
        printf( "aes_xts_encrypt <source file name> <target file name>\n" );
        return -1;
    }
    ...
    ret = mbedtls_aes_xts_setkey_enc( ctx, aes_test_xts_key, ((sizeof(aes_test_xts_key)) * 8));
    if( ret != 0)
    {
    ...
    unsigned char cipher_text[data_length_read];

    if( data_length_read > 0 )
    {
        /* AES XTS encryption */
        ret = mbedtls_aes_crypt_xts( ctx, MBEDTLS_AES_ENCRYPT, data_length_read, aes_test_xts_data_unit, (unsigned char*) local_buffer, cipher_text );
        if( ret != 0 )
        {
             printf ("AES XTS encryption failed \n");
             free(local_buffer);
        ...
        printf("AES XTS encryption successful \n");

        /* Copy encrypted data to local buffer */
        memcpy(local_buffer, cipher_text, data_length_read);
