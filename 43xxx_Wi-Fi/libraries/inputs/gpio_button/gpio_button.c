...
wiced_result_t gpio_button_init( const gpio_button_t* button )
{
    ...
    return wiced_gpio_init( button->gpio, button->config );
}
...
static void gpio_button_interrupt_handler( void* args )
{
    ...
    /* wiced_gpio_input_get_bool() may return errors, do not pass errors to callback */
    result = wiced_gpio_input_get_bool( button->gpio, &gpio_state );
    if ( result == WICED_SUCCESS)
    {
        if ( ((gpio_state == WICED_FALSE) && (button->polarity == WICED_ACTIVE_LOW)) ||
             ((gpio_state == WICED_TRUE)  && (button->polarity == WICED_ACTIVE_HIGH)) )
        {
            button_state_change_callback( (void*)button, WICED_TRUE );
        }
        else
        {
            button_state_change_callback( (void*)button, WICED_FALSE );
        }
    }
}
