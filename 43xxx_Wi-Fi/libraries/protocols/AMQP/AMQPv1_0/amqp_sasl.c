...
wiced_result_t amqp_frame_get_sasl_mechansims( wiced_amqp_frame_t *frame, void *arg )
{
    ...
    /* for symbol : VARIABLE_TYPE_ONE_BYTE_SYMBOL */
    AMQP_BUFFER_GET_OCTET( &frame->buffer, temp );

    /*Decoding list*/

    if ( temp == VARIABLE_TYPE_ONE_BYTE_SYMBOL )
    {
        for ( i = 0; i < count; i++ )
        {
    ...
    else if ( temp == VARIABLE_TYPE_FOUR_BYTE_SYMBOL )
    {
        for ( i = 0; i < count; i++ )
        {
...

{
    ...
    AMQP_BUFFER_PUT_OCTET( &frame->buffer, VARIABLE_TYPE_ONE_BYTE_SYMBOL); /*  1  symbol        */
