...
{
        WICED_AMQP_DELIVERY_RECEIVED            = 0x23,
        WICED_AMQP_DELIVERY_MODIFIED            = 0x24,
        WICED_AMQP_DELIVERY_RELEASED            = 0x25,
        WICED_AMQP_DELIVERY_REJECTED            = 0x26,
        WICED_AMQP_DELIVERY_ACCEPTED            = 0x27,
...
typedef struct wiced_amqp_begin_performative_args_s
{
        ...
        uint32_t handle_max;
        wiced_bool_t handle_max_included;
...
typedef struct wiced_amqp_link_source_target_s
{
    uint8_t *address;
    uint8_t address_length;
    uint32_t durable;
    wiced_bool_t durable_included;
    uint8_t *expiry_policy;
    uint8_t expiry_policy_length;
    uint32_t timeout;
    wiced_bool_t timeout_included;
    wiced_bool_t dynamic;
    wiced_bool_t dynamic_included;
    /*TODO:handle remaining elements*/
}wiced_amqp_source_target_t;
...
typedef struct wiced_amqp_link_performative_args_s
{
        ...
        wiced_amqp_source_target_t source;
        wiced_bool_t source_included;
        wiced_amqp_source_target_t target;
        wiced_bool_t target_included;
        uint32_t initial_delivery_count;
        uint64_t max_message_size;
        wiced_bool_t max_message_size_included;
...
typedef struct wiced_amqp_flow_performative_args_s
{
        ...
        uint32_t outgoing_window;
        uint32_t handle;
        wiced_bool_t handle_included;
        uint32_t delivery_count;
        uint32_t link_credit;
        uint32_t available;
        wiced_bool_t drain;
        wiced_bool_t drain_included;
        wiced_bool_t echo;
        wiced_bool_t echo_included;
...
typedef struct wiced_amqp_end_performative_args_s
{
        amqp_error_t error;
} wiced_amqp_end_performative_args;
...
typedef struct WICED_HEADER_SECTION
{
    wiced_bool_t included;
    wiced_bool_t durable;
    wiced_bool_t durable_included;
    uint8_t priority;
    wiced_bool_t priority_included;
    uint32_t ttl;
    wiced_bool_t ttl_included;
    wiced_bool_t first_acquirer;
    wiced_bool_t first_acquirer_included;
    uint32_t delivery_count;
    wiced_bool_t delivery_count_included;
}wiced_amqp_message_header;



typedef struct WICED_PROPERTIES
{
    wiced_bool_t included;
    uint8_t *message_id;
    uint32_t message_id_length;
    uint8_t *user_id;
    ...
    uint8_t *reply_to;
    uint32_t reply_to_length;
    uint8_t *correlation_id;
    uint32_t correlation_id_length;
    uint8_t *content_type;
    uint32_t content_type_length;
    uint8_t *content_encoding;
    uint32_t content_encoding_length;
    uint64_t absolute_expiry_time;
    wiced_bool_t absolute_expiry_time_included;
    uint64_t creation_time;
    wiced_bool_t creation_time_included;
    uint8_t * group_id;
    uint32_t group_id_length;
    uint32_t group_sequence;
    wiced_bool_t group_sequence_included;
    uint8_t * reply_to_group_id;
    uint32_t reply_to_group_id_length;
}wiced_amqp_message_properties;

typedef struct WICED_APPLICATION_PROPERTIES
{
    wiced_bool_t included;
    uint8_t count;
    amqp_map properties[5];
}wiced_amqp_message_application_properties;


typedef struct wiced_amqp_message_s
{
    /*transfer performative related*/
    uint64_t  delivery_tag;
    uint32_t message_format;
    wiced_bool_t settle;
    wiced_bool_t settle_included;
    wiced_bool_t more;   /*TODO: add logic later for more bit in TX and RX*/
    uint8_t receiver_settle_mode;
    wiced_bool_t receiver_settle_mode_included;
    wiced_bool_t abort;
    wiced_bool_t abort_inlcuded;

    /*message part*/
    wiced_amqp_message_header header;
    /*TODO: add delivery annotations and message annotations structure*/
    wiced_amqp_message_properties properties;
    wiced_amqp_message_application_properties application_properties;
    /*data section*/
    uint8_t *data;
    uint32_t data_lenth;
...
typedef union wiced_amqp_performative_args_t
{
        ...
        wiced_amqp_detach_performative_args detach_args;
        wiced_amqp_end_performative_args end_args;
...
wiced_result_t amqp_send_empty_packet(wiced_amqp_connection_instance* );
