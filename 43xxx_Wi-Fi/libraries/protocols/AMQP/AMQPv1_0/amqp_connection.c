...
wiced_result_t amqp_send_empty_packet( wiced_amqp_connection_instance* connection_instance )
{
    wiced_result_t ret;
    wiced_amqp_frame_t frame;

    /* Send Protocol Header */
    ret = amqp_frame_create( WICED_AMQP_EVENT_TRANSFER_SENT, 0, AMQP_CONNECTION_FRAME_MAX, &frame, &connection_instance->conn->socket );
    if ( ret != WICED_SUCCESS )
    {
        return ret;
    }
    ...
    AMQP_BUFFER_PUT_OCTET( &frame.buffer, 0x00 );
    AMQP_BUFFER_PUT_OCTET( &frame.buffer, 0x00 );
    AMQP_BUFFER_PUT_OCTET( &frame.buffer, 0x08 );
    AMQP_BUFFER_PUT_OCTET( &frame.buffer, 0x02 );
    AMQP_BUFFER_PUT_OCTET( &frame.buffer, 0x00 );
    AMQP_BUFFER_PUT_OCTET( &frame.buffer, 0x00 );
    AMQP_BUFFER_PUT_OCTET( &frame.buffer, 0x00 );
    return amqp_frame_send( &frame, &connection_instance->conn->socket );
}
