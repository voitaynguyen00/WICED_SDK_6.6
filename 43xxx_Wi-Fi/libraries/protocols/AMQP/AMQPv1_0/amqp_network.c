 ...
 ******************************************************/
wiced_partial_frame_details_t amqp_partial_frame;
...
{
    /* Existing connection has been lost/closed, get ready to accept the next one */
    if ( socket->socket.tls_context != NULL )
    {
        /* TODO: Confirm its fine to reset the partial frame to be processed this way */
        amqp_partial_frame.partial_frame_to_be_processed = WICED_FALSE;
    }
...
{
    wiced_result_t result;
    uint16_t tcp_buffer_size_left=0;
    uint8_t* data;
    wiced_packet_t* packet;
    ...
    if ( result == WICED_SUCCESS )
    {
        ...
        do
        {
            ...
            while ( current_size < rx_data_length )
            {
                ...
                tcp_buffer_size_left=(uint16_t) (rx_data_length-current_size);
                if ( amqp_frame_recv( &buffer, socket->p_user, &size, tcp_buffer_size_left, &amqp_partial_frame ) != WICED_SUCCESS )
                {
                    return;
                }
