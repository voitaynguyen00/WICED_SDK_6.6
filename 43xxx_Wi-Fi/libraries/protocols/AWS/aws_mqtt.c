...
static wiced_result_t mqtt_connection_event_cb( wiced_mqtt_object_t mqtt_object, wiced_mqtt_event_info_t *event )
{
    ...
    switch ( event->type )
    {
        ...
        {
            wiced_aws_callback_data_t data;
            data.publish.status = WICED_SUCCESS;
            g_aws_thing.aws_callback(aws, WICED_AWS_EVENT_PUBLISHED, &data);
            break;
        }
        ...
        {
            wiced_aws_callback_data_t data;
            data.subscribe.status = WICED_SUCCESS;
            g_aws_thing.aws_callback(aws, WICED_AWS_EVENT_SUBSCRIBED, &data);
            break;
        }
        ...
        {
            wiced_aws_callback_data_t data;
            data.unsubscribe.status = WICED_SUCCESS;
            g_aws_thing.aws_callback(aws, WICED_AWS_EVENT_UNSUBSCRIBED, &data);
            break;
        }
