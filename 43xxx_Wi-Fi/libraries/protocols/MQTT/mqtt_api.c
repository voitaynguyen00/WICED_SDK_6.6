...
wiced_result_t wiced_mqtt_connect( wiced_mqtt_object_t mqtt_obj, wiced_ip_address_t *address, wiced_interface_t interface, wiced_mqtt_callback_t callback, wiced_mqtt_security_t *security, wiced_mqtt_pkt_connect_t *conninfo )
{
    ...
    {
        WPRINT_LIB_INFO(("[MQTT LIB] : error intializing the  mqtt connection setup\n"));
        return result;
    }
    ...
    /* lastWillTestament parsing */
    args.will_flag = 0;
    if( (NULL != conninfo->lastWillTopic) &&
        (NULL != conninfo->lastWillMessage) )
    {
        args.will_flag        = 1;
        args.will_topic.str   = (uint8_t*)conninfo->lastWillTopic;
        args.will_topic.len   = (uint16_t)strlen( (char*)args.will_topic.str );

        args.will_message.str = (uint8_t*)conninfo->lastWillMessage;
        args.will_message.len = (uint16_t)strlen( (char*)args.will_message.str);

        args.will_qos     = conninfo->lastWillQos;
        args.will_retain  = conninfo->lastWillRetain;
    }
