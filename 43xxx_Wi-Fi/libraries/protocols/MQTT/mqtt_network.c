...
wiced_result_t mqtt_network_init( const wiced_ip_address_t *server_ip_address, uint16_t portnumber, wiced_interface_t interface, void *p_user, mqtt_socket_t *socket, const wiced_mqtt_security_t *security )
{
    ...
    if ( security != NULL )
    {
        ...
        if (security->ca_cert != NULL)
        {
            result = wiced_tls_set_context_root_ca_certificates(&socket->tls_context, security->ca_cert, security->ca_cert_len);
            if (result != WICED_SUCCESS)
            {
...
static wiced_result_t mqtt_consume_tcp_packet( mqtt_connection_t* conn, mqtt_frame_tcp_wrapper_t* pkt, uint32_t bytes_to_consume, uint8_t chunked, uint8_t is_pending )
{
    ...
    if( chunked == 0 )
    {
        ...
        pkt->data_start += bytes_to_consume;
        ...
        {
            pkt->frag_length =  (uint16_t)( pkt->frag_length - bytes_to_consume );
        }
    ...
    */
    else if( chunked == 1 )
    {
        while( bytes_to_consume )
        {
            ...
            /* update these pointers for next mqtt frame parsing */
            pkt->data_start      += nr_bytes;

            pkt->frag_length     = (uint16_t)(pkt->frag_length - nr_bytes);

            /* First-fragment got consumed */
            first_fragment = 0;

            /* if We consumed this fragment and need more - fetch new fragments */
...
static void mqtt_process_tcp_receive( void *args )
{
    ...
    while( !packet_exhausted )
    {
        ...
        else if( bytes_expected <= pkt.remaining_bytes )
        {
            chunked = 1;
            mqtt_consume_tcp_packet( conn, &pkt, bytes_expected, chunked, 0 );
            packet_exhausted = ((pkt.consumed_bytes < pkt.total_bytes) ? 0 : 1 );
        ...
        {
            chunked = 2;
            conn->cached_frame.data_pending = bytes_expected - pkt.remaining_bytes;
            WPRINT_LIB_DEBUG( ("\tStart Assembling...\n" ) );
            WPRINT_LIB_DEBUG( ("\t\t<--- Received %d bytes more(total: %d pending: %d) in TCP packet(size:%d)\n",
                (int)pkt.remaining_bytes, (int)conn->cached_frame.total_bytes, (int)conn->cached_frame.data_pending, (int)pkt.total_bytes) );
            mqtt_consume_tcp_packet( conn, &pkt, pkt.remaining_bytes, chunked, 0 );
