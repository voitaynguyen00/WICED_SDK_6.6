...
typedef enum wiced_mqtt_err_code_s
{
    ...
    WICED_MQTT_CONN_ERR_CODE_UNACCEPTABLE_PROTOCOL_VERSION = 1,  /* Protocol version is not acceptable by MQTT Broker */
    WICED_MQTT_CONN_ERR_CODE_IDENTIFIER_REJECTED           = 2,  /* Client ID rejected by MQTT Broker */
    WICED_MQTT_CONN_ERR_CODE_SERVER_UNAVAILABLE            = 3,  /* MQTT Broker unavailable */
    WICED_MQTT_CONN_ERR_CODE_BAD_USER_NAME_OR_PASSWORD     = 4,  /* Bad user name or password */
    WICED_MQTT_CONN_ERR_CODE_NOT_AUTHORIZED                = 5,  /* Client not authorized to connect */
    WICED_MQTT_CONN_ERR_CODE_INVALID                       = 6   /* Invalid error code */
...
/**
 * Contains information about acknowledged connection
 */
typedef struct wiced_mqtt_conn_ack_s
{
    uint8_t                     session_present;                /* Session Present flag from Connect Acknowledge Flags */
    wiced_mqtt_conn_err_code_t  err_code;                       /* Connect Return Code */
} wiced_mqtt_conn_ack_t;
...
typedef struct wiced_mqtt_event_info_s
{
    ...
    union
    {
        ...
        wiced_mqtt_conn_ack_t           conn_ack;               /* Valid only for WICED_MQTT_EVENT_TYPE_CONNECT_REQ_STATUS event. Indicates the error and flags identified while connecting to Broker */
        wiced_mqtt_msgid_t              msgid;                  /* Valid only for WICED_MQTT_EVENT_TYPE_PUBLISHED, WICED_MQTT_EVENT_TYPE_SUBSCRIBED, WICED_MQTT_EVENT_TYPE_UNSUBSCRIBED events. Indicates message ID */
...
typedef struct wiced_mqtt_pkt_connect_s
{
    ...
    uint8_t*    lastWillTopic;                                  /* LWT: topic to be used */
    uint8_t     lastWillQos;                                    /* LWT: qos to be used */
    uint8_t*    lastWillMessage;                                /* LWT: message to be sent */
    uint8_t     lastWillRetain;                                 /* LWT: retain last message */
    uint8_t*    peer_cn;
