 ...
 ******************************************************/
static wiced_result_t websocket_common_connect     ( wiced_websocket_t* websocket, const wiced_websocket_client_url_protocol_t* url, wiced_tls_identity_t* tls_identity, const char* root_ca, const char* peer_cn, uint16_t port, wiced_interface_t interface );
...
static wiced_result_t websocket_tls_connect        ( wiced_websocket_t* websocket, wiced_ip_address_t* address, wiced_tls_identity_t* tls_identity, const char* root_ca, const char* peer_cn, uint16_t port, wiced_interface_t interface );
...
static wiced_result_t lock_websocket_mutex         ( void );
static wiced_result_t unlock_websocket_mutex       ( void );
#if 0
...
wiced_result_t wiced_websocket_connect( wiced_websocket_t* websocket, const wiced_websocket_client_url_protocol_t* url_entry, uint16_t port, wiced_interface_t interface )
{
    ...
    if ( websocket_common_connect( websocket, url_entry, NULL, NULL, NULL, port, interface ) != WICED_SUCCESS )
    {
...
wiced_result_t wiced_websocket_secure_connect( wiced_websocket_t* websocket, const wiced_websocket_client_url_protocol_t* url_entry, wiced_tls_identity_t* tls_identity, const char* root_ca, const char* peer_cn, uint16_t port, wiced_interface_t interface )
{
    if ( websocket_common_connect( websocket, url_entry, tls_identity, root_ca, peer_cn, port, interface ) != WICED_SUCCESS )
    {
        wiced_websocket_close( websocket, WEBSOCKET_CLOSE_STATUS_NO_CODE, NULL );
        return WICED_ERROR;
    }
...
static wiced_result_t websocket_common_connect( wiced_websocket_t* websocket, const wiced_websocket_client_url_protocol_t* url_entry, wiced_tls_identity_t* tls_identity, const char* root_ca, const char* peer_cn, uint16_t port, wiced_interface_t interface )
{
    wiced_result_t     result = WICED_SUCCESS;
    ...
    result = lock_websocket_mutex();
    if( result != WICED_SUCCESS )
    {
        WPRINT_LIB_INFO( ("Failed to lock mutex\n") );
        return result;
    }
    ...
    {
        result = websocket_tls_connect( websocket, &address, tls_identity, root_ca, peer_cn, port, interface );
    }
    else
    {
    ...
    result = unlock_websocket_mutex();
    if( result != WICED_SUCCESS )
    {
        WPRINT_LIB_INFO( ("Failed to unlock mutex...Closing the connection\n") );
        /*Close the connection. Failed in unlock mutex */
        wiced_websocket_close( websocket, WEBSOCKET_CLOSE_STATUS_NO_CODE, NULL );
        return result;
    }
...
static wiced_result_t websocket_tls_connect( wiced_websocket_t* websocket, wiced_ip_address_t* address, wiced_tls_identity_t*  tls_identity, const char* root_ca, const char* peer_cn, uint16_t port, wiced_interface_t interface )
{
    ...
    if( tls_identity != NULL )
    {
        ...
        result = wiced_tls_init_context( context, tls_identity, peer_cn );
        if( result != WICED_SUCCESS )
        {
        ...
        if(root_ca != NULL) {
            result = wiced_tls_set_context_root_ca_certificates(context, root_ca, strlen(root_ca));
            if( result != WICED_SUCCESS )
                goto _err_connect;
        }
    }

    result = wiced_tcp_create_socket( tcp_socket, interface );
    if( result != WICED_SUCCESS )
    {
        WPRINT_LIB_INFO( ("[%s] Error Creating TCP socket\n", __func__) );
    ...
    if( context != NULL)
    {
        tcp_socket->context_malloced = WICED_TRUE;
    }
...
static wiced_result_t websocket_send_stream( wiced_websocket_t* websocket, uint8_t* header, uint8_t header_length, uint8_t* data, uint32_t length, uint8_t* mask )
{
    ...
    if( mask )
    {
        ...
        mask_unmask_frame_data(data, data_out, length, mask);
        data_ptr = data_out;
    }

    /* now send the data */
    wiced_tcp_stream_write(tcp_stream, data_ptr, length );
...
wiced_result_t websocket_receive_frame( wiced_websocket_t* websocket )
{
    ...
    uint32_t bytes_left_to_read=0;
    uint32_t total_received_bytes = 0;
    uint32_t bytes_read=0;
    uint8_t is_data_chunked = 0;
    ...
    {
        WPRINT_LIB_INFO(("Payload length is given by 7 bits\n"));
        /* do nothing */
    }
    else if( payload_length == WEBSOCKET_FRAME_PAYLOAD_LENGTH_2_BYTES )
    {
        WPRINT_LIB_INFO(("Payload length is given by 7+16 bits\n"));
    ...
    {
        WPRINT_LIB_INFO(("Payload length is given by 7+64 bits\n"));
    ...
    if( is_masked )
    {
        WPRINT_LIB_INFO(("Contains masked key, MASK bit:%d\n", is_masked ) );
    ...
    WPRINT_LIB_DEBUG(("Websocket Payload length in %s: %ld\n", __FUNCTION__, data_length));
    WPRINT_LIB_DEBUG(("Websocket max_buffer_length in %s: %ld\n", __FUNCTION__, max_buffer_length));
    //frame type is not required inside the do while loop
    ...
    if( data_length > max_buffer_length )
    {
        ...
        bytes_read = max_buffer_length;//first time it should be equal to max buffer length
        is_data_chunked = 1;
    }
    else
    {
        WPRINT_LIB_INFO(("Rx Frame buffer can accommodate the payload length: %d\n", (int)data_length));
        bytes_read = data_length;
        is_data_chunked = 0; //not needed because second time when the receive_frame function is called, it declares is_data_chunked=0
    }

    do{ //while end after callback;
    /* Now read the Message payload in the application buffer */
    received_bytes = 0;
    WPRINT_LIB_DEBUG(("Reading %d bytes...\n", (int)bytes_read));
    result = wiced_tcp_stream_read_with_count( stream, buffer, (uint16_t)bytes_read, DEFAULT_WEBSOCKET_FRAME_TIMEOUT, &received_bytes );
    if( result != WICED_SUCCESS || received_bytes != bytes_read )
    {
        WPRINT_LIB_DEBUG(("[%d]: Received bytes from stream:%d result:%d \n",__LINE__, (int)received_bytes, (int)result ) );
        return WICED_ERROR;
    }

    total_received_bytes += received_bytes;
    WPRINT_LIB_DEBUG(("[%d]: Total Received bytes:%d, received bytes from tcp stream: %d\n",__LINE__, (int)total_received_bytes, (int)received_bytes ) );

    bytes_left_to_read = data_length - total_received_bytes;
    WPRINT_LIB_DEBUG(("[%d]: Bytes left to read: %d\n", __LINE__, (int)bytes_left_to_read ));

    if( total_received_bytes == data_length )
    {
    ...
    if(is_data_chunked ==2){
        websocket->callbacks.on_chunked_message( websocket, buffer, bytes_read, frame_type, flags, data_length, is_data_chunked);
    }
    else{
        websocket->callbacks.on_message( websocket, buffer, bytes_read, frame_type, flags, data_length, is_data_chunked);
    }
    bytes_read =  MIN(max_buffer_length, bytes_left_to_read);
    WPRINT_LIB_DEBUG(("[%d]: bytes_read: %d\n", __LINE__, (int)bytes_read ));
    if(is_data_chunked ==1)
        is_data_chunked =2;
    //for is_chunked_data=2, the same flag and frame_type is used

}while( total_received_bytes < data_length );
...
wiced_result_t wiced_websocket_close( wiced_websocket_t* websocket, const uint16_t code, const char* reason )
{
    ...
    if( websocket->core.role == WEBSOCKET_ROLE_CLIENT )
    {
        wiced_tcp_unregister_callbacks( tcp_socket );
...
static wiced_result_t lock_websocket_mutex( void )
{
    ...
    if ( websocket_mutex.mutex_state == MUTEX_UNINITIALISED )
    {
        ...
        result = wiced_rtos_init_mutex( &websocket_mutex.handshake_lock );
        websocket_mutex.mutex_state = MUTEX_UNLOCKED;
    }

    if ( websocket_mutex.mutex_state == MUTEX_UNLOCKED )
    {
        result = wiced_rtos_lock_mutex( &websocket_mutex.handshake_lock );
...
static wiced_result_t unlock_websocket_mutex( void )
{
    ...
    {
        result=wiced_rtos_unlock_mutex( &websocket_mutex.handshake_lock );
        websocket_mutex.mutex_state = MUTEX_UNLOCKED;
    }
    return result;
}

wiced_result_t wiced_websocket_register_callbacks ( wiced_websocket_t* websocket, wiced_websocket_callback_t on_open_callback, wiced_websocket_callback_t on_close_callback, wiced_websocket_message_callback_t on_message_callback, wiced_websocket_callback_t on_error_callback ,   wiced_websocket_message_callback_t on_chunked_message_callback)
{
    ...
    websocket->callbacks.on_chunked_message = on_chunked_message_callback;
...
void wiced_websocket_unregister_callbacks ( wiced_websocket_t* websocket )
{
    ...
    websocket->callbacks.on_chunked_message = NULL;
}
