...
#define DNS_SRV_RDATA_PRIORITY_OFFSET               ((uintptr_t)&(((dns_srv_data_t*)0)->priority))
#define DNS_SRV_RDATA_WEIGHT_OFFSET                 ((uintptr_t)&(((dns_srv_data_t*)0)->weight))
#define DNS_SRV_RDATA_PORT_OFFSET                   ((uintptr_t)&(((dns_srv_data_t*)0)->port))
...
wiced_result_t dns_get_next_record(dns_message_iterator_t* iter, uint8_t* iter_end, dns_record_t* r, dns_name_t* name)
{
    ...
    /* Convert rdata to host format based on record type */
    if (r->type == RR_TYPE_SRV)
    {
        /* Convert Priority, Weight and Port to host byte order */
        uint16_t *priority = (uint16_t *)(r->rdata + DNS_SRV_RDATA_PRIORITY_OFFSET);
        uint16_t *weight = (uint16_t *)(r->rdata + DNS_SRV_RDATA_WEIGHT_OFFSET);
        uint16_t *port = (uint16_t *)(r->rdata + DNS_SRV_RDATA_PORT_OFFSET);

        *priority = dns_read_uint16(r->rdata + DNS_SRV_RDATA_PRIORITY_OFFSET);
        *weight = dns_read_uint16(r->rdata + DNS_SRV_RDATA_WEIGHT_OFFSET);
        *port = dns_read_uint16(r->rdata + DNS_SRV_RDATA_PORT_OFFSET);
    }
