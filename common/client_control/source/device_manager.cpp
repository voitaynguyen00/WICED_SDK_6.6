...
#define RPC_BT_DEVICE_TYPE_BT_DEVICE_TYPE_BREDR 1
#define RPC_BT_DEVICE_TYPE_BT_DEVICE_TYPE_BLE   2
...
void MainWindow::SetDevicePaired(BYTE * info, int len)
{
    ...
    if (len>=7 && (info[6]==RPC_BT_DEVICE_TYPE_BT_DEVICE_TYPE_BLE || info[6]==RPC_BT_DEVICE_TYPE_BT_DEVICE_TYPE_BREDR))
    {
        ...
        if (i == -1)
        {
            ...
            AddDeviceToList(bda, cboxPtr, NULL, true);
            if (bLeDevice)
...
void MainWindow::WriteNVRAMToDevice(bool bBLEDevice)
{
    ...
    for (int i = 0; i < cb->count(); i++)
    {
        ...
        if (len <= 1000)
        {
            ...
            {
                app_host_dm_push_pairing_host_info((uint8_t *)pDev->m_nvram.constData(), len);
            }
            else
            {
                app_host_dm_push_nvram_data(pDev->m_nvram_id, (uint8_t *)pDev->m_nvram.constData(), len);
            }
        }
        else
        {
...
{
    app_host_dm_set_app_traces(true);  // enable traces
}

// Command to disable WICED HCI traces
void MainWindow::DisableAppTraces()
{
    app_host_dm_set_app_traces(false);  // disable traces
}
...
{
    qDebug("%s", (const char *)s.toStdString().c_str());
    ...
    while (!m_bClosing)
    {
        ...
        else if (packetType == HCI_EVENT_PKT)
        {
            ...
            if (au8Hdr[1] == HCI_VSE_OPCODE)
            {
                ...
                process_vse(au8Hdr, pktLen);
            }
            else
            {
                dump_hci_data_hexa("HCI Event:", au8Hdr, pktLen);
            }
        }
    }
