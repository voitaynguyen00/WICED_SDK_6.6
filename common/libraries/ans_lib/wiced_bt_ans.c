...
wiced_bt_gatt_status_t ans_lib_send_new_alert(uint16_t conn_id, uint8_t category_id)
{
    ...
    status = wiced_bt_gatt_send_notification(conn_id, ans_lib_cb.gatt_handles.new_alert.value, val_len, new_alert);
...
wiced_bt_gatt_status_t ans_lib_send_unread_alert(uint16_t conn_id, uint8_t category_id)
{
    ...
    status = wiced_bt_gatt_send_notification(conn_id, ans_lib_cb.gatt_handles.unread_alert.value , 2, unread_alert);
    if (status == WICED_BT_GATT_SUCCESS)
    {
...
wiced_bt_gatt_status_t wiced_bt_ans_process_gatt_read_req(uint16_t conn_id, wiced_bt_gatt_read_t *p_read)
{
    ...
    if (p_read->handle == ans_lib_cb.gatt_handles.new_alert.supported_category)
    {
        *p_read->p_val_len   = 2;
        memcpy(p_read->p_val, &ans_lib_cb.supported_new_alerts, 2);
    }
    else if (p_read->handle == ans_lib_cb.gatt_handles.new_alert.configuration)
    {
        *(p_read->p_val_len) = 2;
        memcpy(p_read->p_val, &ans_lib_cb.new_alert_cccd, 2);
    }
    else if (p_read->handle == ans_lib_cb.gatt_handles.unread_alert.supported_category)
    {
        *(p_read->p_val_len) = 2;
        memcpy(p_read->p_val, &ans_lib_cb.supported_unread_alerts, 2);
    }
    else if (p_read->handle == ans_lib_cb.gatt_handles.unread_alert.configuration)
    {
        *(p_read->p_val_len) = 2;
        memcpy(p_read->p_val, &ans_lib_cb.unread_alert_status_cccd, 2);
    }
    else
    {
    ...
    if (status == WICED_BT_GATT_SUCCESS)
        ANS_TRACE_DBG("handle:%04x len:%d data:%x status:%x\n", p_read->handle,
                *(p_read->p_val_len), (p_read->p_val[0] + (p_read->p_val[1]<<8)), status );
