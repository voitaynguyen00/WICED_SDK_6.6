...
#if defined __ANDROID__ || defined __APPLE__ || defined WICEDX_LINUX || defined WICED_MESH_REPO
...
char *wiced_bt_mesh_db_get_all_groups(wiced_bt_mesh_db_mesh_t *mesh_db, char *in_group)
{
    ...
    if ((in_group != NULL) && (strcmp(in_group, mesh_db->name) != 0))
    {
    ...
    for (i = 0; i < mesh_db->num_groups; i++)
    {
        if (mesh_db->group[i].parent_addr == parent_addr)
        {
            ...
            p_buf += (strlen(mesh_db->group[i].name) + 1);
        }
    }
    *p_buf = 0;
    return buf;
...
char *wiced_bt_mesh_db_get_all_provisioners(wiced_bt_mesh_db_mesh_t *mesh_db)
{
    ...
    for (i = 0; i < mesh_db->num_provisioners; i++)
    {
        ...
        p_buf += (strlen(mesh_db->provisioner[i].name) + 1);
    }
    *p_buf = 0;
    return buf;
}
...
wiced_bool_t is_group_a_perent_group(wiced_bt_mesh_db_mesh_t *mesh_db, uint16_t parent_group_addr, uint16_t group_addr)
{
    ...
    if (group->parent_addr == 0)
        return WICED_FALSE;
    return is_group_a_perent_group(mesh_db, group->parent_addr, group_addr);
}
...
uint8_t wiced_bt_mesh_db_node_sensor_get_property_value_len(uint16_t property_id)
{
