...
#define SCAN_EXTENDED_DURATION              5
...
model_element_t models_configured_for_pub[] =
{
    ...
    { 0, 0, MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_GENERIC_LOCATION_CLNT },
...
static void mesh_process_user_property_status(wiced_bt_mesh_event_t *p_event, void *p);
...
static void mesh_process_user_property_status(wiced_bt_mesh_event_t *p_event, void *p_data);
...
static wiced_bt_mesh_event_t *mesh_create_control_event(uint16_t company_id, uint16_t model_id, uint16_t dst, uint16_t app_key_idx, uint16_t num_in_group, uint16_t *group_list);
    ...
    for (j = 0; ; j++)
    {
        ...
        if (company_id == MESH_COMPANY_ID_BT_SIG)
        {
            ...
            case WICED_BT_MESH_CORE_MODEL_ID_GENERIC_POWER_LEVEL_SETUP_SRV:
                if (component_type < DEVICE_TYPE_POWER_OUTLET)
                    component_type = DEVICE_TYPE_POWER_OUTLET;
                break;
...
uint8_t mesh_client_get_component_info(char *component_name, mesh_client_component_info_status_t p_component_info_status_callback)
{
    ...
    wiced_bt_mesh_event_t *p_event = mesh_create_control_event(MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_GENERIC_USER_PROPERTY_SRV, dst, app_key->index, 0, NULL);
    if (p_event == NULL)
    {
...
int mesh_client_dfu_start(uint8_t dfu_method, char *component_name, uint16_t company_id, uint8_t *fw_id, uint8_t fw_id_len)
{
    ...
    if ((dfu_method == DFU_METHOD_APP_TO_DEVICE) || (dfu_method == DFU_METHOD_PROXY_TO_DEVICE))
    {
        if ((device_addr = get_device_addr(component_name)) == 0)
        {
            Log("select device name\n");
            return MESH_CLIENT_ERR_NOT_FOUND;
        }
        if ((dfu_method == DFU_METHOD_PROXY_TO_DEVICE) && (p_cb->proxy_addr == device_addr))
        {
            Log("proxy cann't DFU itself\n");
            return MESH_CLIENT_ERR_NOT_FOUND;
        }
    }
    // find the distributor node.
    if ((dfu_method == DFU_METHOD_APP_TO_DEVICE) || (dfu_method == DFU_METHOD_APP_TO_ALL))
    {
    ...
    // Add nodes with the same CID/PID to the list
    if ((dfu_method == DFU_METHOD_APP_TO_DEVICE) || (dfu_method == DFU_METHOD_PROXY_TO_DEVICE))
    {
        start_data.group_size = 1;
        mesh_configure_set_local_device_key(device_addr);
        start_data.update_nodes_list[0] = device_addr;
    }
    else
    {
        start_data.group_size = 0;
        fw_cid = company_id;
        fw_pid = (((uint16_t)fw_id[0]) << 8) + fw_id[1];
        fw_vid = (((uint16_t)fw_id[2]) << 8) + fw_id[3];
        for (i = 0; i < p_mesh_db->num_nodes; i++)
        {
            ...
            if ((p_mesh_db->node[i].cid == fw_cid) &&
                (p_mesh_db->node[i].pid == fw_pid) &&
                (p_mesh_db->node[i].vid == fw_vid))
            {
    ...
    start_data.firmware_id.company_id = company_id;
    start_data.firmware_id.fw_id_len = fw_id_len;
    memcpy(start_data.firmware_id.fw_id, fw_id, start_data.firmware_id.fw_id_len);

    p_event = mesh_create_control_event(MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_FW_DISTRIBUTION_CLNT, p_distributor_node->unicast_address, p_mesh_db->app_key[0].index, 0, NULL);
    if (p_event == NULL)
        return MESH_CLIENT_ERR_NO_MEMORY;

    wiced_bt_mesh_fw_provider_start(p_event, &start_data);
...
int mesh_client_dfu_stop(void)
{
    ...
    {
        p_event = mesh_create_control_event(MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_FW_DISTRIBUTION_CLNT, p_cb->distributor_addr, p_mesh_db->app_key[0].index, 0, NULL);
        if (p_event == NULL)
            return MESH_CLIENT_ERR_NO_MEMORY;
...
int mesh_client_dfu_get_status(char *distributor_name, uint16_t company_id, uint8_t *fw_id, uint8_t fw_id_len, mesh_client_dfu_status_t p_dfu_status_callback)
{
    ...
    if (p_cb->distributor_addr == 0)
    {
        ...
        dfu_cb.firmware_id.company_id = company_id;
        dfu_cb.firmware_id.fw_id_len = fw_id_len;
        memcpy(dfu_cb.firmware_id.fw_id, fw_id, fw_id_len);
    }
    for (i = 0; i < p_mesh_db->num_nodes; i++)
    {
    ...
    wiced_bt_mesh_fw_provider_get_status(p_event, &dfu_cb.firmware_id, mesh_provision_process_event);
    return MESH_CLIENT_SUCCESS;
}
...
int mesh_client_sensor_setting_set(const char *device_name, int property_id, int setting_property_id, uint8_t *val)
{
    ...
    config_data.prop_value_len = wiced_bt_mesh_db_node_sensor_get_property_value_len(setting_property_id);
    memcpy(config_data.setting_raw_val, val, config_data.prop_value_len);
    wiced_bt_mesh_event_t *p_event = mesh_create_control_event(MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_SENSOR_CLNT, dst, app_key->index, num_nodes, group_list);
                                   ...
                                   uint32_t fast_cadence_high)
{
    ...
    config_data.property_id = (uint16_t)property_id;
    config_data.prop_value_len = wiced_bt_mesh_db_node_sensor_get_property_value_len(property_id);
...
int mesh_client_scan_unprovisioned(int start)
{
    ...
    for (i = 0; i < p_mesh_db->num_nodes; i++)
    {
        ...
        // We will not use local device to scan if we are not connected to, or if connected to proxy through IP
...
void mesh_provision_process_event(uint16_t event, wiced_bt_mesh_event_t *p_event, void *p_data)
{
    ...
    case WICED_BT_MESH_USER_PROPERTY_STATUS:
        mesh_process_user_property_status(p_event, p_data);
        return;
...
{
#ifndef CLIENTCONTROL
    ods("provision connecting link status:%d provisioner:%04x\n", p_data->link_status, p_cb->provisioner_addr);
#endif
...
void mesh_provision_process_provision_end(mesh_provision_cb_t *p_cb, wiced_bt_mesh_provision_status_data_t *p_data)
{
    ...
    else
    {
        ...
        wiced_bt_mesh_db_node_t *p_node = wiced_bt_mesh_db_node_create(p_mesh_db, p_cb->provisioning_device_name, p_cb->addr, p_cb->uuid, p_cb->num_elements, p_data->dev_key, p_data->net_key_idx);
...
void mesh_configure_composition_data_get(mesh_provision_cb_t *p_cb, uint8_t is_local)
{
    ...
    if (p_event != NULL)
    {
        ...
        // A Configuration Client shall send a Config Composition Data Get message with the Page field value set to 0xFF.
        data.page_number = 0xff;
...
void mesh_process_sensor_descriptor_status(uint16_t addr, wiced_bt_mesh_sensor_descriptor_status_data_t *ptr)
{
    ...
    for (i = 0; i < ptr->num_descriptors; i++)
    {
        if ((p_op = (pending_operation_t *)wiced_bt_get_buffer(sizeof(pending_operation_t))) != NULL)
        {
            ...
            p_op->p_event = mesh_create_control_event(MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_SENSOR_CLNT, addr, app_setup_key->index, 0, NULL);
...
void configure_queue_local_device_operations(mesh_provision_cb_t *p_cb)
{
    ...
    for (i = 0; i < wiced_bt_mesh_db_num_app_keys(p_mesh_db); i++)
    {
        ...
        while (comp_data_len)
        {
            ...
            {
                if ((p_comp_data[0] + (p_comp_data[1] << 8) == MESH_COMPANY_ID_CYPRESS)
                 && ((p_op = (pending_operation_t *)wiced_bt_get_buffer(sizeof(pending_operation_t))) != NULL))
                {
                    //wiced_bt_mesh_config_model_app_bind_data_t
                    p_op->operation = CONFIG_OPERATION_MODEL_APP_BIND;
    ...
    while (comp_data_len)
    {
        ...
        for (j = 0; j < num_models; j++)
        {
            ...
            if (model_id == WICED_BT_MESH_CORE_MODEL_ID_SENSOR_SRV)
            {
                if ((p_op = (pending_operation_t *)wiced_bt_get_buffer(sizeof(pending_operation_t))) != NULL)
                {
                    ...
                    memset(p_op, 0, sizeof(pending_operation_t));
                    p_op->operation = CONFIG_OPERATION_SENSOR_DESC_GET;
                    p_op->p_event = mesh_create_control_event(MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_SENSOR_CLNT,  p_cb->addr + element_idx, app_key->index, 0, NULL);
                    // get all descriptors
        ...
        if (default_trans_time_model_present)
        {
            if ((p_op = (pending_operation_t *)wiced_bt_get_buffer(sizeof(pending_operation_t))) != NULL)
            {
                ...
                p_op->p_event = mesh_create_control_event(MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_GENERIC_DEFTT_CLNT, p_cb->addr + element_idx, app_key_setup->index, 0, NULL);
                if (p_op->p_event)
                {
...
{
    uint16_t *p_elements_array, *p_element, *p_first_element = NULL;
    wiced_bt_mesh_db_model_id_t *p_models_array;
    uint16_t j, k;
    pending_operation_t *p_op;
    uint16_t dst;
    ...
    for (p_element = p_elements_array; *p_element != 0; p_element++)
    {
        ...
        {
            p_first_element = p_element;
            break;
        }
    }
    if (p_first_element == NULL)
    {
        wiced_bt_free_buffer(p_elements_array);
        wiced_bt_free_buffer(p_group_list);
        return MESH_CLIENT_ERR_NETWORK_DB;
    }
    dst = wiced_bt_mesh_db_get_node_addr(p_mesh_db, *p_element);

    for (p_element = p_first_element; *p_element != 0; p_element++)
    {
        // Need to stop as soon as we find another primary element, which is the beginning of a new component of that node, or a new node.
        if ((p_element != p_first_element) && (!is_secondary_element(*p_element)))
            break;

        if ((p_models_array = wiced_bt_mesh_db_get_all_models_of_element(p_mesh_db, *p_element, 0)) != NULL)
        {
            for (j = 0; p_models_array[j].company_id != MESH_COMPANY_ID_UNUSED; j++)
            {
                ...
                if ((p_group_list != NULL) && ((p_models_array[j].company_id != MESH_COMPANY_ID_BT_SIG) || model_needs_sub(p_models_array[j].id, p_models_array)))
                {
                    for (k = 0; p_group_list[k] != 0; k++)
                    {
                        if (!wiced_bt_mesh_db_is_model_subscribed_to_group(p_mesh_db, *p_element, p_models_array[j].company_id, p_models_array[j].id, p_group_list[k]))
                        {
                            if ((p_op = (pending_operation_t *)wiced_bt_get_buffer(sizeof(pending_operation_t))) != NULL)
                            {
                                memset(p_op, 0, sizeof(pending_operation_t));
...
int mesh_client_move_component_to_group(const char *component_name, const char *from_group_name, const char *to_group_name)
{
    ...
    uint16_t dst;
    mesh_provision_cb_t *p_cb = &provision_cb;
    uint16_t group_addr_to;
    uint16_t group_addr_from;
    uint16_t *p_group_list_to;
    ...
    if ((group_addr_to = wiced_bt_mesh_db_group_get_addr(p_mesh_db, to_group_name)) == 0)
    {
        Log("group %s doesnot exist", to_group_name);
        return MESH_CLIENT_ERR_INVALID_ARGS;
    }
    if ((group_addr_from = wiced_bt_mesh_db_group_get_addr(p_mesh_db, from_group_name)) == 0)
    {
        Log("group %s doesnot exist", from_group_name);
        return MESH_CLIENT_ERR_INVALID_ARGS;
    }

    p_group_list_to = get_group_list(group_addr_to);

    clean_pending_op_queue(0);

    if ((p_elements_array = wiced_bt_mesh_db_get_all_elements(p_mesh_db)) == NULL)
    {
        wiced_bt_free_buffer(p_group_list_to);
        return MESH_CLIENT_ERR_NETWORK_DB;
    }
    ...
    for (p_element = p_first_element; *p_element != 0; p_element++)
    {
        if ((p_models_array = wiced_bt_mesh_db_get_all_models_of_element(p_mesh_db, *p_element, 0)) != NULL)
        {
            for (j = 0; p_models_array[j].company_id != MESH_COMPANY_ID_UNUSED; j++)
            {
                ...
                if ((p_group_list_to != NULL) && ((p_models_array[j].company_id != MESH_COMPANY_ID_BT_SIG) || model_needs_sub(p_models_array[j].id, p_models_array)))
                {
                    for (k = 0; p_group_list_to[k] != 0; k++)
                    {
                        if (!wiced_bt_mesh_db_is_model_subscribed_to_group(p_mesh_db, *p_element, p_models_array[j].company_id, p_models_array[j].id, p_group_list_to[k]))
                        {
                            Log("Model:%4x is not subscribed to the group adding subscription:%4x\n", p_models_array[j].id, p_group_list_to[k]);
                            if ((p_op = (pending_operation_t *)wiced_bt_get_buffer(sizeof(pending_operation_t))) != NULL)
                            {
                                memset(p_op, 0, sizeof(pending_operation_t));
                                p_op->operation = CONFIG_OPERATION_MODEL_SUBSCRIBE;
                                ...
                                p_op->uu.model_sub.company_id = p_models_array[j].company_id;
                                p_op->uu.model_sub.model_id = p_models_array[j].id;
                                p_op->uu.model_sub.addr[0] = p_group_list_to[k] & 0xff;
                                p_op->uu.model_sub.addr[1] = (p_group_list_to[k] >> 8) & 0xff;
                                configure_pending_operation_queue(p_cb, p_op);
                            }
                        }
                        else
    ...
    for (p_element = p_first_element; *p_element != 0; p_element++)
    {
        ...
        if ((p_models_array = wiced_bt_mesh_db_get_all_models_of_element(p_mesh_db, *p_element, group_addr_from)) != NULL)
        {
            for (j = 0; p_models_array[j].company_id != MESH_COMPANY_ID_UNUSED; j++)
            {
                if (is_core_model(p_models_array[j].company_id, p_models_array[j].id))
                ...
                if ((p_op = (pending_operation_t *)wiced_bt_get_buffer(sizeof(pending_operation_t))) != NULL)
                {
                    ...
                    p_op->uu.model_sub.company_id = p_models_array[j].company_id;
                    p_op->uu.model_sub.model_id = p_models_array[j].id;
                    p_op->uu.model_sub.addr[0] = group_addr_from & 0xff;
                    p_op->uu.model_sub.addr[1] = (group_addr_from >> 8) & 0xff;
                    configure_pending_operation_queue(p_cb, p_op);
                }
            }
            wiced_bt_free_buffer(p_models_array);
...
wiced_bool_t get_vs_model(uint16_t element_addr, uint16_t *p_company_id, uint16_t *p_model_id)
{
    wiced_bt_mesh_db_model_id_t *p_models_array;
    wiced_bool_t model_present = WICED_FALSE;
    int j;
    ...
    if ((p_models_array = wiced_bt_mesh_db_get_all_models_of_element(p_mesh_db, element_addr, 0)) != NULL)
    {
        for (j = 0; p_models_array[j].company_id != MESH_COMPANY_ID_UNUSED; j++)
        {
            if (p_models_array[j].company_id != MESH_COMPANY_ID_BT_SIG)
            {
                *p_company_id = p_models_array[j].company_id;
                *p_model_id = p_models_array[j].id;
                model_present = WICED_TRUE;
                break;
            }
        }
...
int mesh_client_identify(const char *p_name, uint8_t duration)
{
    ...
    wiced_bt_mesh_event_t *p_event = mesh_create_control_event(MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_HEALTH_CLNT, dst, app_key->index, num_nodes, group_list);
    if (p_event == NULL)
    {
        wiced_bt_free_buffer(group_list);
        return MESH_CLIENT_ERR_NO_MEMORY;
...
int mesh_client_on_off_get(const char *p_name)
{
    ...
    wiced_bt_mesh_event_t *p_event = mesh_create_control_event(MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_GENERIC_ONOFF_CLNT, dst, app_key->index, num_nodes, group_list);
    if (p_event == NULL)
    {
...
int mesh_client_level_get(const char *p_name)
{
    ...
    wiced_bt_mesh_event_t *p_event = mesh_create_control_event(MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_GENERIC_LEVEL_CLNT, dst, app_key->index, num_nodes, group_list);
    if (p_event == NULL)
    {
        Log("onoff get no mem\n");
        wiced_bt_free_buffer(group_list);
...
int mesh_client_lightness_get(const char *p_name)
{
    ...
    wiced_bt_mesh_event_t *p_event = mesh_create_control_event(MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_LIGHT_LIGHTNESS_CLNT, dst, app_key->index, num_nodes, group_list);
    if (p_event == NULL)
    {
...
int mesh_client_hsl_get(const char *p_name)
{
    ...
    wiced_bt_mesh_event_t *p_event = mesh_create_control_event(MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_LIGHT_HSL_CLNT, dst, app_key->index, num_nodes, group_list);
    if (p_event == NULL)
    {
        Log("hsl get no mem\n");
        wiced_bt_free_buffer(group_list);
...
int mesh_client_ctl_get(const char *p_name)
{
    ...
    wiced_bt_mesh_event_t *p_event = mesh_create_control_event(MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_LIGHT_CTL_CLNT, dst, app_key->index, num_nodes, group_list);
    if (p_event == NULL)
    {
        Log("ctl get no mem\n");
        wiced_bt_free_buffer(group_list);
...
int mesh_client_vendor_data_set(const char *device_name, uint8_t *buffer, uint16_t data_len)
{
    int i;
    uint16_t dst = get_device_addr(device_name);
    ...
    Log("Send VS Data addr:%04x app_key_idx:%04x ", p_event->dst, p_event->app_key_idx);
    char buf[100];
    uint8_t *p = buffer;
    len = data_len;
...
void mesh_process_user_property_status(wiced_bt_mesh_event_t *p_event, void *p)
{
    char buf[200];
    mesh_provision_cb_t *p_cb = &provision_cb;
    ...
    sprintf(buf, "Property Status from:%x AppKeyIdx:%x Element:%x Prop ID:%d Len:%d ", p_event->src, p_event->app_key_idx, p_event->element_idx, p_data->id, p_data->len);
...
void mesh_process_scan_capabilities_status(mesh_provision_cb_t *p_cb, wiced_bt_mesh_event_t *p_event, wiced_bt_mesh_provision_scan_capabilities_status_data_t *p_data)
{
    ...
    data.scanned_items_limit = p_data->max_scanned_items;
...
void mesh_process_scan_status(mesh_provision_cb_t *p_cb, wiced_bt_mesh_event_t *p_event, wiced_bt_mesh_provision_scan_status_data_t *p_data)
{
    ...
    Log("ScanStatus from:%x Status:%d Phase:%x Limit:%d Timeout:%d Type:%x\n", p_event->src, p_data->status, p_data->phase, p_data->scanned_items_limit, p_data->timeout, p_data->scanning_type);

    node->scanning_state = (p_data->phase == WICED_BT_MESH_REMOTE_PROVISION_SCANNING_PHASE_SCANNING) ? SCANNING_STATE_SCANNING : SCANNING_STATE_IDLE;
...
void mesh_process_scan_report(mesh_provision_cb_t *p_cb, wiced_bt_mesh_event_t *p_event, wiced_bt_mesh_provision_scan_report_data_t *p_data)
{
    ...
    wiced_bool_t uuid_reported = WICED_FALSE;
    wiced_bool_t uuid_received_from_rprs = WICED_FALSE;
    ...
    {
        // if UUID is reported from that server, ignore
        for (p_report = p_cb->p_first_unprovisioned; p_report != NULL; p_report = p_report->p_next)
        {
            if (memcmp(p_report->uuid, p_data->uuid, sizeof(p_data->uuid)) == 0)
            {
                uuid_reported = WICED_TRUE;

                if (p_report->provisioner_addr == p_event->src)
                {
                    uuid_received_from_rprs = WICED_TRUE;
                    break;
                }
            }
        }
        if (uuid_received_from_rprs)
        {
        ...
        for (p_report = p_cb->p_first_unprovisioned; p_report != NULL; p_report = p_report->p_next)
        {
            if (memcmp(p_report->uuid, p_data->uuid, sizeof(p_data->uuid)) == 0)
            {
                if ((p_event->src == p_cb->unicast_addr) || (p_data->rssi > p_report->rssi))
                {
...
void mesh_process_fw_distribution_status(wiced_bt_mesh_event_t *p_event, void *p)
{
    ...
    sprintf(buf, "FW Distribution Status %d from:%x Block:%d of %d Company ID:%x Firmware ID:", p_data->status, p_event->src, p_data->block_num, p_data->total_blocks, p_data->firmware_id.company_id);
    for (i = 0; i < p_data->firmware_id.fw_id_len; i++)
    {
        sprintf(&buf[strlen(buf)], "%02x ", p_data->firmware_id.fw_id[i]);
    }
    Log(buf);
    for (i = 0; i < p_data->num_nodes; i++)
    {
        Log("Node:%04x Status:%d\n", p_data->node[i].unicast_address, p_data->node[i].status);
    }

    if (p_cb->p_dfu_status_callback != NULL)
    {
        p_cb->p_dfu_status_callback(p_data->status, p_data->block_num, p_data->total_blocks); //  p_data->firmware_id.company_id, p_data->firmware_id.fw_id_len, p_data->firmware_id.fw_id);
    }
    wiced_bt_mesh_release_event(p_event);
}
...
wiced_bool_t is_core_model(uint16_t company_id, uint16_t model_id)
{
    ...
    case WICED_BT_MESH_CORE_MODEL_ID_OBJ_TRANSFER_SRV:
    case WICED_BT_MESH_CORE_MODEL_ID_OBJ_TRANSFER_CLNT:
            return WICED_TRUE;
    default:
        return WICED_FALSE;
    }
...
wiced_bt_mesh_event_t *mesh_create_control_event(uint16_t company_id, uint16_t model_id, uint16_t dst, uint16_t app_key_idx, uint16_t num_in_group, uint16_t *group_list)
{
...
char *mesh_client_get_target_methods(const char *component_name)
{
    ...
    control_list = (char *)wiced_bt_get_buffer(control_list_size);
