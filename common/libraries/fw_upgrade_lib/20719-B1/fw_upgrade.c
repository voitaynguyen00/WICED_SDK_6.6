...
uint8_t firmware_upgrade_switch_eflash_active_ds(void)
{
    ...
    WICED_BT_TRACE("[%s]  wiced_hal_eflash_erase result %d \n", __FUNCTION__, result);
...
uint32_t wiced_bt_get_fw_image_size(uint8_t partition)
{
    ...
    {
        if (wiced_hal_eflash_read(g_config_Info.active_ds_base - EF_BASE_ADDR, data, 16) == WICED_SUCCESS)
            image_size = (data[12] + (data[13] << 8) + (data[14] << 16) + (data[15] << 24)) + 16;
    }
    else if (partition == PARTITION_UPGRADE)
    {
        if (wiced_firmware_upgrade_retrieve_from_nv(0, data, 16) == 16)
            image_size = (data[12] + (data[13] << 8) + (data[14] << 16) + (data[15] << 24)) + 16;
    }
    WICED_BT_TRACE("Get FW image size - partition:%d image size:%d\n", partition, image_size);
    return image_size;
}
