...
wiced_bt_gatt_status_t wiced_bt_anc_read_server_supported_new_alerts( uint16_t conn_id )
{
    ...
    if( ( anc_lib_data.conn_id == conn_id ) && ( anc_lib_data.supported_new_alert_category_value_handle ) )
    {
        memset( &read_req, 0, sizeof( wiced_bt_gatt_read_param_t ) );
        read_req.by_handle.auth_req = GATT_AUTH_REQ_NONE;
        read_req.by_handle.handle = anc_lib_data.supported_new_alert_category_value_handle;
...
wiced_bt_gatt_status_t wiced_bt_anc_read_server_supported_unread_alerts( uint16_t conn_id )
{
    ...
    if( ( anc_lib_data.conn_id == conn_id ) && ( anc_lib_data.supported_unread_alert_category_value_handle ) )
    {
        memset( &read_req, 0, sizeof( wiced_bt_gatt_read_param_t ) );
        read_req.by_handle.auth_req = GATT_AUTH_REQ_NONE;
        read_req.by_handle.handle = anc_lib_data.supported_unread_alert_category_value_handle;
...
wiced_bt_gatt_status_t wiced_bt_anc_control_required_alerts( uint16_t conn_id , wiced_bt_anp_alert_control_cmd_id_t cmd_id, wiced_bt_anp_alert_category_id_t category)
{
    ...
    {
        write_req_len = (sizeof(wiced_bt_gatt_value_t) - 1 + sizeof(value));
        ...
        p_write_req->handle = anc_lib_data.alert_notify_control_point_value_handle;
        p_write_req->offset   = 0;
        p_write_req->len = sizeof(value);
        p_write_req->auth_req = GATT_AUTH_REQ_NONE;

        value[0] = cmd_id;
        value[1] = category;
        memcpy( &p_write_req->value[0], value, sizeof(value) );

        //if (control_point_cache_alloc(&i))
        //{
