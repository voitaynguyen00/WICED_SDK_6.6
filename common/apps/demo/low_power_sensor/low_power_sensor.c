...
* Device can be wake up using GPIO (Please check hello_sensor_sleep_config.device_wake_gpio_num
...
*    (a) To configure shutdown sleep, modify SLEEP_TYPE=SLEEP_TYPE_SHUTDOWN in make target and clean build.
*    (b) To configure non shutdown sleep, modify SLEEP_TYPE=SLEEP_TYPE_NOT_SHUTDOWN in make target and clean build.
*    (c) To configure sleep with transport, modify SLEEP_MODE=SLEEP_MODE_TRANSPORT in make target and clean build.
...
*    (d) To configure sleep without transport, modify SLEEP_MODE=SLEEP_MODE_NO_TRANSPORT in make target and clean build.
...
*     Note: when hello_sensor advertising in sleep mode, peer need to re attempt connection,
*     if fails first time
* 6. On the client side register for notifications
* 7. Push a button on the tag to send notifications to the client
* 8. if no activity on hello_sensor, enters sleep on idle time out.
* 9. Wake the device.
*     (a) Wake using configured wake source (Please check hello_sensor_sleep_config.device_wake_gpio_num
...
wiced_timer_t           hello_sensor_pairing_timer;
static uint8_t          peer_addr_type;
static uint8_t          transport_type;
...
static void                     hello_sensor_timeout_to_pair (uint32_t un_used);
...
uint32_t hello_sensor_sleep_handler(wiced_sleep_poll_type_t type )
{
    ...
    /* Do not allow sleep if we are waiting to initiate for pair or encryption */
    if ( (hello_sensor_state.application_state == HELLO_SENSOR_IDLE) && 
         wiced_is_timer_in_use (&hello_sensor_pairing_timer) )
    {
        return WICED_SLEEP_NOT_ALLOWED;
    }
...
void hello_sensor_application_init( void )
{
    ...
    /* Pairing timer to initiate pairing or encryption in case master does not initiate
    pairing or encryption within 2 seconds from conection up */
    if ( wiced_init_timer( &hello_sensor_pairing_timer, hello_sensor_timeout_to_pair, 0, WICED_MILLI_SECONDS_TIMER ) != WICED_SUCCESS )
    {
        WICED_BT_TRACE("pairing timer init fail \n");
    }
...
/*
 * Initiates pairing or encryption depends on security keys existance
 */
void hello_sensor_timeout_to_pair (uint32_t un_used)
{
    wiced_result_t result;
    if ( !hello_sensor_is_device_bonded(hello_sensor_hostinfo.bdaddr) )
    {
        result = wiced_bt_dev_sec_bond( hello_sensor_hostinfo.bdaddr, peer_addr_type, transport_type, 0, NULL );
        WICED_BT_TRACE( "wiced_bt_dev_sec_bond %d \n", result );
    }
    else
    {
        wiced_bt_ble_sec_action_type_t  encryption_type = BTM_BLE_SEC_ENCRYPT;
        result = wiced_bt_dev_set_encryption (hello_sensor_hostinfo.bdaddr, transport_type, &encryption_type);
...
wiced_result_t hello_sensor_management_cback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    /* stop the pairing timer as pairing/encryption is in progress */
    if ((event == BTM_PAIRING_IO_CAPABILITIES_BLE_REQUEST_EVT) ||
        (event == BTM_PAIRED_DEVICE_LINK_KEYS_REQUEST_EVT) ||
        (event == BTM_ENCRYPTION_STATUS_EVT)
       )
    {
        if (wiced_is_timer_in_use (&hello_sensor_pairing_timer))
        {
            wiced_stop_timer (&hello_sensor_pairing_timer);
        }
    }
...
wiced_bt_gatt_status_t hello_sensor_gatts_connection_up( wiced_bt_gatt_connection_status_t *p_status )
{
    ...
    /* hold on 2 seconds to allow master to initiate pair */
    if (wiced_is_timer_in_use (&hello_sensor_pairing_timer))
    {
        wiced_stop_timer(&hello_sensor_pairing_timer);
    }
    peer_addr_type          = p_status->addr_type;
    transport_type          = p_status->transport;
    if (wiced_start_timer( &hello_sensor_pairing_timer, 2000 ) !=  WICED_SUCCESS )
    {
        WICED_BT_TRACE("pairing timer start failure\n");
    }
