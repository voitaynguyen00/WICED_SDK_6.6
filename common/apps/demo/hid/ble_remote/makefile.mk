...
ifeq ($(ENABLE_IR), y)
 ...
 C_FLAGS += -DSUPPORT_IR
 INCS += $(DIR)/ir
 APP_SRC += ir/bleapp_appirtx.c 
endif

#enable easy pair feature
#C_FLAGS += -DEASY_PAIR
...
#allow SDS while in discoverable mode
#C_FLAGS += -DALLOW_SDS_IN_DISCOVERABLE
...
#enable find me profile
#C_FLAGS += -DSUPPORTING_FINDME

#enable digital mic support
#C_FLAGS += -DSUPPORT_DIGITAL_MIC

ifeq ($(ENABLE_MOTION), y)
 #support motion
 C_FLAGS += -DSUPPORT_MOTION
 
 # once interrupted use regular transport poll, instead of interrupt, to get motion data until motion idle
 C_FLAGS += -DPOLL_MOTION_WHILE_CONNECTED_AND_ACTIVE

 # motion data converted to mouse data, otherwise send raw data
 #C_FLAGS += -DUSE_MOTION_AS_AIR_MOUSE
 
 APP_SRC += motion/motion_icm.c motion/sensor_icm20608.c motion/interrupt.c
 
 ########################################################################
 # Sources in APP_SECRET_SRC are NOT delivered to Cypress in the
 # Application Release Package (make arp).-- only their objects
 ########################################################################
 APP_SECRET_SRC = RollCompFullDynamic.c AIR_MOTION_Lib.c
 INCS += $(DIR)/motion $(DIR)/motion/obj_secret
 APP_PATCHES_AND_LIBS += air_mouse.a
endif

ifeq ($(ENABLE_TOUCHPAD), y)
 #enable touch pad
 C_FLAGS += -DSUPPORT_TOUCHPAD
 INCS += $(DIR)/touchpad
 APP_SRC += touchpad/IQS5xx.c touchpad/touchPad.c 
endif
