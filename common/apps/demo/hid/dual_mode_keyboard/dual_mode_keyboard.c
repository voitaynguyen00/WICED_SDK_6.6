...
{
    WICED_BT_TRACE("dual mode KB create\n");
...
void kbapp_init(void)
{
    ...
    wiced_init_timer( &kb_allow_sleep_timer, kbapp_allowsleep_timeout, 0, WICED_MILLI_SECONDS_TIMER );
    ...
    wiced_dual_mode_hidd_link_register_sleep_permit_handler(kbapp_sleep_handler);

#ifdef __BLEKB_SCROLL_REPORT__
    wiced_hal_mia_notificationRegisterQuad();
#endif

    wiced_dual_mode_hidd_link_init();
...
void kbapp_shutdown(void)
{
    ...
    if((active_transport == BT_TRANSPORT_LE) && wiced_ble_hidd_link_is_connected())
        wiced_ble_hidd_link_disconnect();

    if((active_transport == BT_TRANSPORT_BR_EDR) && wiced_bt_hidd_link_is_connected())
...
void kbapp_pollReportUserActivity(void)
{
    ...
    else if(wiced_bt_hidd_link_is_connected())
    {
        // Generate a report
        if(wiced_bt_hid_cfg_settings.security_requirement_mask)
        {
...
{
    int32_t flags;    
    WICED_BT_TRACE("LE Transport state changed to %d\n", newState);
...
{
    WICED_BT_TRACE("BT Transport state changed to %d\n", newState);
