...
uint8_t  ota_fw_upgrade_initialized = WICED_FALSE;
#endif
 ...
 * */
wiced_bt_gatt_status_t dual_mode_kb_gatts_req_read_handler( uint16_t conn_id, wiced_bt_gatt_read_t * p_read_data )
{
 ...
 * */
wiced_bt_gatt_status_t dual_mode_kb_gatts_req_write_handler( uint16_t conn_id, wiced_bt_gatt_write_t * p_data )
{
...
wiced_bt_gatt_status_t dual_mode_kb_gatts_connection_up( wiced_bt_gatt_connection_status_t *p_status )
{

    WICED_BT_TRACE( "dual_mode_kb_conn_up, id %d: %B\n",  p_status->conn_id, p_status->bd_addr);
    active_transport = BT_TRANSPORT_LE;

    //disable BT discoverable
...
#ifdef TESTING_USING_HCI
    wiced_transport_send_data( HCI_CONTROL_HID_EVENT_OPENED, p_status->bd_addr, BD_ADDR_LEN );
#endif
 ...
 * */
wiced_bt_gatt_status_t dual_mode_kb_gatts_connection_down( wiced_bt_gatt_connection_status_t *p_status )
{
...
wiced_bt_gatt_status_t dual_mode_kb_gatts_conn_status_cb( wiced_bt_gatt_connection_status_t *p_status )
{
    if(p_status->connected)
    {
        return dual_mode_kb_gatts_connection_up( p_status );
    }
    else
    {
        return dual_mode_kb_gatts_connection_down( p_status );
    }
}
 ...
 */
wiced_bt_gatt_status_t dual_mode_kb_gatts_req_conf_handler( uint16_t conn_id, uint16_t handle )
{
    WICED_BT_TRACE( "dual_mode_kb_indication_cfm, conn %d hdl %d\n", conn_id, handle );
...
wiced_bt_gatt_status_t dual_mode_kb_gatts_req_cb( wiced_bt_gatt_attribute_request_t *p_data )
{
    wiced_bt_gatt_status_t result = WICED_BT_GATT_SUCCESS;

    //WICED_BT_TRACE( "_dual_mode_kb_gatts_req_cb. conn %d, type %d\n", p_data->conn_id, p_data->request_type );
    switch ( p_data->request_type )
    {
        case GATTS_REQ_TYPE_READ:
            result = dual_mode_kb_gatts_req_read_handler(p_data->conn_id, &(p_data->data.read_req));
            break;
            
        case GATTS_REQ_TYPE_WRITE:
        case GATTS_REQ_TYPE_PREP_WRITE:
            result = dual_mode_kb_gatts_req_write_handler(p_data->conn_id, &(p_data->data.write_req));
             break;
       ...
       case GATTS_REQ_TYPE_CONF:
           result = dual_mode_kb_gatts_req_conf_handler( p_data->conn_id, p_data->data.handle );
           break;
            
       default:
            WICED_BT_TRACE("Please check this dual_mode_kb_gatts_req_cb!!!\n");
            break;
    }
...
wiced_bt_gatt_status_t dual_mode_kb_gatts_callback( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_data)
{
    wiced_bt_gatt_status_t result = WICED_BT_GATT_SUCCESS;

    //WICED_BT_TRACE("\ndual_mode_kb_gatts_callback event: 0x%x\n", event);

    switch(event)
    {
        case GATT_CONNECTION_STATUS_EVT:
            result = dual_mode_kb_gatts_conn_status_cb(&p_data->connection_status);
            break;

        case GATT_ATTRIBUTE_REQUEST_EVT:
            result = dual_mode_kb_gatts_req_cb(&p_data->attribute_request);
            break;
