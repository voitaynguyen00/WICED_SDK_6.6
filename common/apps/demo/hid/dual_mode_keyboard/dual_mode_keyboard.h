 ...
 *
 * This file provides definitions and function prototypes for dual mode (BT classic and LE)  keyboard
 * device
 *
 */

#ifndef _DUAL_MODE_KB_H_
#define _DUAL_MODE_KB_H_
...
#define NUM_KEYSCAN_COLS    15   // Num of Cols in keyscan matrix
...
/// Usage codes for pin code entry reports
enum
{
    /// Signals that a valid pin character was enetered
    PIN_ENTRY_EVENT_CHAR = 0,

    /// Signals that he last digit was deleted
    PIN_ENTRY_EVENT_BACKSPACE = 1,

    /// Signals that pin code entry was restarted. Any characters entered till this time are thrown away
    PIN_ENTRY_EVENT_RESTART = 2,

    /// Invalid value
    PIN_ENTRY_EVENT_INVALID = 0xFF
};

enum 
{
    /// Signals the start of pass key
    PASS_KEY_ENTRY_EVENT_START = 0x00,
    KEY_ENTRY_EVENT_START = PASS_KEY_ENTRY_EVENT_START,

    /// Signals that a valid pass key was entered
    PASS_KEY_ENTRY_EVENT_CHAR = 0x01,
    KEY_ENTRY_EVENT_CHAR = PASS_KEY_ENTRY_EVENT_CHAR,

    /// Signals that the last digit was deleted
    PASS_KEY_ENTRY_EVENT_BACKSPACE = 0x02,
    KEY_ENTRY_EVENT_BACKSPACE = PASS_KEY_ENTRY_EVENT_BACKSPACE,

    /// Signals that pass key entry was restarted. Any characters entered till this time are thrown away
    PASS_KEY_ENTRY_EVENT_RESTART = 0x03,
    KEY_ENTRY_EVENT_RESTART = PASS_KEY_ENTRY_EVENT_RESTART,

    /// Signals that pass key entry was finished
    PASS_KEY_ENTRY_EVENT_STOP = 0x04,
    KEY_ENTRY_EVENT_STOP = PASS_KEY_ENTRY_EVENT_STOP,

        /// Max number of such events
    KEY_ENTRY_EVENT_MAX = 0x05
};
...
{
    /// No pin entry mode
    PIN_ENTRY_MODE_NONE = 0x00,

    /// Legacy pin entry mode - pin could be 16 bytes long
    LEGACY_PIN_ENTRY_IN_PROGRESS = 0x80,

    /// Enhanced pin entry - pin is restricted to 0 - 999999
    PASS_KEY_ENTRY_IN_PROGRESS = 0x81,

    /// Number of such states
    PASS_CODE_ENTRY_PROGRESS_MAX = 0x02
};
...
/// Flag indicating that the pin report has changed since it was last sent
    uint8_t pinRptChanged;
...
// Pin code entry parameters

/// Flag indicating that pin code entry is in progress
    uint8_t pinCodeEntryInProgress;

/// Current pin code size
    uint8_t pinCodeSize;

/// Max number of digits in code.
    uint8_t maxPinCodeSize;

/// Pin code buffer
    uint8_t pinCodeBuffer[MAX_PIN_SIZE];
