...
extern wiced_bt_gatt_status_t blemouse_gatts_callback( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_data);
...
/*
 * blemouse bt/ble link management callbacks
 * */
wiced_result_t blemouse_management_cback(wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data)
{
    ...
    WICED_BT_TRACE("bt_stack_management_cback %d\n", event);
    switch( event )
    {
        ...
        default:
            WICED_BT_TRACE("\n!!!unprocessing blemouse_management_cback: %d!!!\n", event );
            break;
    }
