...
void hci_control_spp_send_data( uint16_t handle, uint8_t *p_data, uint32_t length )
{
    ...
    hci_spp_send(p_scb, handle, (char *)p_data, length, "send_data");
}
...
void hci_control_port_event_cback( wiced_bt_rfcomm_port_event_t event, uint16_t handle )
{
    ...
    /* error! */
    if ( event & PORT_EV_ERR )
    {
        //WICED_BT_TRACE("last sent get failed \n");
        spp_resend_data = 1;
    }
    else if ( ( ( event & PORT_EV_TXCHAR ) || ( event & PORT_EV_FCS ) ) && ( p_scb->pending_bytes != 0 ) )
    {
        //WICED_BT_TRACE("fc_disabled \n");
