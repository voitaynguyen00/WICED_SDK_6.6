...
static void                   hci_control_le_handle_write_rsp( uint16_t conn_id, uint16_t handle, uint8_t write_status );
static void                   hci_control_le_send_write_completed( uint16_t con_handle, uint8_t result );
static void                   hci_control_le_send_gatt_read_error_event( uint16_t con_handle, uint8_t read_result );
static void                   hci_control_le_send_gatt_write_error_event( uint16_t con_handle, uint8_t write_result );
...
static void                   hci_control_le_handle_set_channel_classification ( uint8_t ble_channel_map[ BLE_CHANNEL_MAP_LEN ] );
static void                   hci_control_le_set_advertisement_data(void);

static void                   hci_control_le_handle_notify( uint16_t conn_id, uint16_t handle, uint8_t *p_data, uint16_t len );
...
wiced_result_t hci_control_le_connection_up( wiced_bt_gatt_connection_status_t *p_status )
{
    ...
    serial_gatt_service_connection_up(p_status);
#endif
...
#if (WICED_APP_LE_SERIAL_GATT_INCLUDED == TRUE)
    serial_gatt_service_connection_down(p_status);
#endif
...
wiced_result_t hci_control_le_gatt_operation_comp_cb( wiced_bt_gatt_operation_complete_t *p_complete )
{
    ...
    case GATTC_OPTYPE_READ: 
        ...
        if ( le_control_cb.conn[conn_id].state == LE_CONTROL_STATE_READ_VALUE )
        {
            ...
            if ( p_complete->status != WICED_SUCCESS )
                hci_control_le_send_gatt_read_error_event( conn_id, p_complete->status );
            else
    ...
    case GATTC_OPTYPE_EXE_WRITE:
        ...
        if ( le_control_cb.conn[conn_id].state == LE_CONTROL_STATE_WRITE_VALUE )
        {
            le_control_cb.conn[conn_id].state = LE_CONTROL_STATE_IDLE;
            if ( p_complete->status != WICED_SUCCESS )
                hci_control_le_send_gatt_write_error_event( conn_id, p_complete->status );
            else
...
wiced_bt_gatt_status_t hci_control_le_get_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t max_len, uint16_t *p_len )
{
    ...
    {
        // If handle is not contained within external lookup table pass the read req to the host
...
wiced_bt_gatt_status_t hci_control_le_set_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t len )
{
    ...
    for (i = 0; i < watch_gatt_db_ext_attr_tbl_size; i++)
    {
        if (watch_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            if ((watch_gatt_db_ext_attr_tbl[i].max_len >= len))
            {
                // Value fits within the supplied buffer; copy over the value
                watch_gatt_db_ext_attr_tbl[i].cur_len = len;
                memcpy(watch_gatt_db_ext_attr_tbl[i].p_data, p_val, len);
                res = WICED_BT_GATT_SUCCESS;
            }
            else
            {
...
{
    wiced_bt_gatt_status_t read_status;

#if (WICED_APP_LE_SERIAL_GATT_INCLUDED == TRUE)
    if (p_req->handle == HDLD_SERIAL_GATT_SERIAL_DATA_CLIENT_CONFIGURATION)
    {
        return serial_gatt_service_read_handler(conn_id, p_req);
    }
    else
#endif
    {
        /* Check if this Handle is locally managed */
        read_status = hci_control_le_get_value(p_req->handle, conn_id, p_req->p_val,
                *p_req->p_val_len, p_req->p_val_len);
        /* If this Handle is not locally managed, send the Read Request to MCU */
        if (read_status == WICED_BT_GATT_PENDING)
        {
            hci_control_le_process_data( HCI_CONTROL_GATT_EVENT_READ_REQUEST, conn_id,
                    p_req->handle, NULL, 0 );
        }
        return read_status;
    }
...
wiced_result_t hci_control_le_write_handler( uint16_t conn_id, wiced_bt_gatt_write_t *p_req )
{
    ...
    {
#if (WICED_APP_LE_SERIAL_GATT_INCLUDED == TRUE)
    case HDLC_SERIAL_GATT_SERIAL_DATA_VALUE:
    case HDLD_SERIAL_GATT_SERIAL_DATA_CLIENT_CONFIGURATION:
        status = serial_gatt_service_write_handler(conn_id, p_req);
        break;
#endif
...
#if (WICED_APP_LE_SERIAL_GATT_INCLUDED == TRUE)
    serial_gatt_service_mtu_handler(conn_id, mtu);
#endif
    return ( WICED_SUCCESS );
}
...
void hci_control_le_handle_command( uint16_t cmd_opcode, uint8_t* p, uint32_t data_len )
{
    ...
    case HCI_CONTROL_GATT_COMMAND_WRITE_RESPONSE:
        hci_control_le_handle_write_rsp( p[0] + ( p[1] << 8 ), p[2] + ( p[3] << 8 ), p[4]);
        break;
...
/*
 * Process Write Response received over UART
 */
static void hci_control_le_handle_write_rsp( uint16_t conn_id, uint16_t handle,
        uint8_t write_status )
{
    ...
    if ( conn_id > LE_CONTROL_MAX_CONNECTIONS )
    {
        WICED_BT_TRACE( "illegal conn_id:%04x\n", conn_id );
        hci_control_send_command_status_evt( HCI_CONTROL_GATT_EVENT_COMMAND_STATUS,
                HCI_CONTROL_STATUS_BAD_HANDLE );
    }
    else if ( le_control_cb.conn[conn_id].conn_id != conn_id )
    {
        WICED_BT_TRACE( "no connection:%d\n", le_control_cb.conn[conn_id].conn_id );
        hci_control_send_command_status_evt( HCI_CONTROL_GATT_EVENT_COMMAND_STATUS,
                HCI_CONTROL_STATUS_WRONG_STATE );
    }
    else if ( handle == 0 )
    {
        WICED_BT_TRACE( "illegal handle:%04x\n", handle );
        hci_control_send_command_status_evt( HCI_CONTROL_GATT_EVENT_COMMAND_STATUS,
                HCI_CONTROL_STATUS_INVALID_ARGS );
    }
    else
    {
        status = wiced_bt_gatt_send_response( write_status, conn_id, handle, 0, 0, NULL );

        hci_control_send_command_status_evt( HCI_CONTROL_GATT_EVENT_COMMAND_STATUS,
                status == WICED_BT_GATT_SUCCESS ? HCI_CONTROL_STATUS_SUCCESS : HCI_CONTROL_STATUS_FAILED );

        WICED_BT_TRACE( "GATT write_rsp conn_id:%d status:%d\n", conn_id, status );
    }
}
...
/*
 *  Send GATT Read Error event over UART
 */
void hci_control_le_send_gatt_read_error_event( uint16_t con_handle, uint8_t read_result )
{
    uint8_t cmd_event[3];
    cmd_event[0] = con_handle & 0xff;
    cmd_event[1] = ( con_handle >> 8 ) & 0xff;
    cmd_event[2] = read_result;

    wiced_transport_send_data ( HCI_CONTROL_GATT_EVENT_READ_ERROR, cmd_event, sizeof( cmd_event ) );
}

/*
 *  Send GATT Write Error event over UART
 */
void hci_control_le_send_gatt_write_error_event( uint16_t con_handle, uint8_t write_result )
{
    uint8_t cmd_event[3];
    cmd_event[0] = con_handle & 0xff;
    cmd_event[1] = ( con_handle >> 8 ) & 0xff;
    cmd_event[2] = write_result;

    wiced_transport_send_data ( HCI_CONTROL_GATT_EVENT_WRITE_ERROR, cmd_event, sizeof( cmd_event ) );
}
