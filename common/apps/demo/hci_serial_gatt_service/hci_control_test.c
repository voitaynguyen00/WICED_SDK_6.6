...
extern void btu_hcif_send_cmd (UINT8 controller_id, BT_HDR *p_buf);
...
{
    wiced_bt_buffer_statistics_t buff_stats[8];
    uint8_t i;
    if (wiced_bt_get_buffer_usage(buff_stats, sizeof(buff_stats)) == WICED_BT_SUCCESS )
    {
        for(i=0; i < 4; i++)
            WICED_BT_TRACE("pool: id:%d size:%d curr_cnt:%d max_cnt:%d total:%d \n",
