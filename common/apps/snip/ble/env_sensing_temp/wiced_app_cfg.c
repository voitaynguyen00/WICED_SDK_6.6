 ...
 * Definitions
 */
#define WICED_DEVICE_NAME                  "Thermistor"     //Device name during device discovery
...
const wiced_bt_cfg_settings_t wiced_app_cfg_settings =
{
    ...
    /*
     * Explanation for class of a device:
     * "device_class" is a parameter received during the device discovery procedure on the BR/EDR physical transport, indicating the type of the device.
     *
     * The device_class parameter is only used on BR/EDR and BR/EDR/LE devices using the BR/EDR physical transport
     *
     * Description          Bit Field
     * Major Service class  (23:13)
     * Major device class   (12:8)
     * Minor device class   (7:2)
     * Format Type Field    (1:0)
     *
     * 0x20 | 0x07 | 0x04   ->  0010 0000 0000 0111 0000 0011
     *                            |                         |->Reserved
     *                            |              |->wearable
     *                            |->Capturing (Scanner, Mic,..)
     *
     * */
    ...
    {
        .appearance                     = APPEARANCE_GENERIC_THERMOMETER,                            // GATT appearance (see gatt_appearance_e)
        ...
        .max_attr_len                   = 20,                                                        // Maximum attribute length; gki_cfg must have a corresponding buffer pool that can hold this length
...
/*  { buf_size, buf_count } */
    ...
    { 1024,     5   },      /* Large Buffer Pool  (used for HCI ACL messages) */
