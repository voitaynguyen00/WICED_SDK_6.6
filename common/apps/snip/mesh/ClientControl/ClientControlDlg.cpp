...
extern "C" uint8_t *wiced_bt_mesh_format_hci_header(uint16_t dst, uint16_t app_key_idx, uint8_t element_idx, uint8_t reliable, uint8_t retransmit_count, uint8_t retransmit_interval, uint8_t reply_timeout, uint16_t num_in_group, uint16_t *group_list, uint8_t *p_buffer, uint16_t len);
...
void CClientControlDlg::ProcessData(DWORD opcode, LPBYTE p_data, DWORD len)
{
    ...
    case HCI_CONTROL_MESH_EVENT_LIGHT_LC_PROPERTY_CLIENT_STATUS:
        ...
        break;
    case HCI_CONTROL_MESH_EVENT_VENDOR_STATUS:
...
void CClientControlDlg::OnBnClickedBatteryLevelGet()
{
    ...
    BYTE buffer[128];
    LPBYTE p = wiced_bt_mesh_format_hci_header(dst, app_key_idx, 0, 1, 0, 0, 0, 0, NULL, buffer, sizeof(buffer));
...
void CClientControlDlg::OnBnClickedSceneRecall()
{
    ...
    BYTE buffer[128];
    LPBYTE p = wiced_bt_mesh_format_hci_header(dst, app_key_idx, 0, (BYTE)((CButton *)GetDlgItem(IDC_RELIABLE_SEND))->GetCheck(), 0, 0, 0, 0, NULL, buffer, sizeof(buffer));
    *p++ = scene & 0xff;
    *p++ = (scene >> 8) & 0xff;
...
void CClientControlDlg::ReadValuesSendMsg(BYTE local_global)
{
    ...
    WCHAR buffer[20];
    GetDlgItemText(IDC_GLOBAL_LATITUDE, buffer, sizeof(buffer));
    if (buffer[0] == 0)
    {
    ...
    GetDlgItemText(IDC_GLOBAL_LONGITUDE, buffer, sizeof(buffer));
    if (buffer[0] == 0)
    {
    ...
    GetDlgItemText(IDC_GLOBAL_ALTITUDE, buffer, sizeof(buffer));
    if (buffer[0] == 0)
    {
    ...
    GetDlgItemText(IDC_LOCAL_NORTH, buffer, sizeof(buffer));
    if (buffer[0] == 0)
    {
        local_north = 0x8000;
    }
    ...
    GetDlgItemText(IDC_LOCAL_EAST, buffer, sizeof(buffer));
    if (buffer[0] == 0)
    {
        local_east = 0x8000;
    }
    ...
    BYTE msg[128];
    LPBYTE p = wiced_bt_mesh_format_hci_header(dst, app_key_idx, 0, (BYTE)((CButton *)GetDlgItem(IDC_RELIABLE_SEND))->GetCheck(), 0, 0, 0, NULL, 0, msg, sizeof(msg));
...
void CClientControlDlg::ProcessPropertiesStatus(LPBYTE p_data, DWORD len)
{
    ...
    for (len = len - 6; len > 0; len -= 2, p_data += 2)
    {
...
void CClientControlDlg::ProcessSceneRegisterStatus(LPBYTE p_data, DWORD len)
{
    ...
    for (len = len - 8, p_data = p_data + 3; len > 0; len -= 2, p_data += 2)
    {
        wsprintf(&buf[wcslen(buf)], L"%04x ", p_data[0] + (p_data[1] << 8));
    }
    m_trace->SetCurSel(m_trace->AddString(buf));
...
void CClientControlDlg::OnBnClickedOnOffSet()
{
    ...
    BYTE buffer[128];
    LPBYTE p = wiced_bt_mesh_format_hci_header(dst, app_key_idx, 0, reliable, 0, 0, 0, 0, NULL, buffer, sizeof(buffer));
    *p++ = target_state;
    *p++ = transition_time & 0xff;
    *p++ = (transition_time >> 8) & 0xff;
    *p++ = (transition_time >> 16) & 0xff;
...
void CClientControlDlg::OnBnClickedPowerLevelStatus()
{
    ...
    BYTE buffer[128];
    LPBYTE p = wiced_bt_mesh_format_hci_header(dst, app_key_idx, 0, 0, 0, 0, 0, 0, NULL, buffer, sizeof(buffer));
...
{
    m_ComHelper->SendWicedCommand(HCI_CONTROL_MESH_COMMAND_LIGHT_LC_OCCUPANCY_SET, NULL, 0);
}
...
void CClientControlDlg::OnBnClickedVsData()
{
    ...
    WCHAR buf[100];
    wsprintf(buf, L"Send VS Data to addr:%x app_key_idx:%x %d bytes:", dst, app_key_idx, data_len);
    m_trace->SetCurSel(m_trace->AddString(buf));
    DWORD len = data_len;
    DWORD i;
    while (len != 0)
...
*/
ULONGLONG local_to_tai_seconds(CTime *time_day, CTime *time_seconds)
{
    ULONGLONG l;
    int  f, d, b, z, v, a, k;
    int year, month, day, hour, minute, second;
    year = time_day->GetYear();
    month = time_day->GetMonth();
    day = time_day->GetDay();
    hour = time_seconds->GetHour();
    minute = time_seconds->GetMinute();
    second = time_seconds->GetSecond();
    f = 0;
    ...
    d = b - 730119;
    l = second + (ULONGLONG)d * 86400 + hour * 3600 + minute * 60 + f;
...
void CClientControlDlg::OnBnClickedTimeSet()
{
    ...
    ULONGLONG tai_time = local_to_tai_seconds(&time_day, &time_seconds);
    ...
    BYTE buffer[128];
    LPBYTE p = wiced_bt_mesh_format_hci_header(dst, 0xFFFF, 0, 1, 0, 0, 0, 0, NULL, buffer, sizeof(buffer));
    *p++ = tai_time & 0xff;
    *p++ = (tai_time >> 8) & 0xff;
    *p++ = (tai_time >> 16) & 0xff;
    *p++ = (tai_time >> 24) & 0xff;
...
*/
void tai_to_utc_local_time(mesh_time_t *mesh_time, RtcTime* rtc_time)
{
    int  e, f, l, d, h, s, m, b, q, c, x, z, v, a, j, k;
    int H;
    int year, month, day, hour, minute, second;

    memset(rtc_time, 0, sizeof(RtcTime));

    if (mesh_time->utc_delta.tai_of_delta_change == 0)
    {
        e = (int)(mesh_time->time_state.tai_utc_delta_current - 255);
        f = 0;
    }
    else
    {
        if (mesh_time->time_state.tai_seconds < mesh_time->utc_delta.tai_of_delta_change)
            e = (int)(mesh_time->time_state.tai_utc_delta_current - 255);
        if (mesh_time->time_state.tai_seconds >= mesh_time->utc_delta.tai_of_delta_change)
            e = (int)(mesh_time->utc_delta.tai_of_delta_change - 255);
        if ((mesh_time->time_state.tai_seconds + 1 == mesh_time->utc_delta.tai_of_delta_change) &&
            (mesh_time->time_state.tai_utc_delta_current < mesh_time->utc_delta.tai_utc_delta_new))
            f = 1;
        else
            f = 0;
    }

    l = mesh_time->time_state.tai_seconds - e - f;
    // add the time zone offset
    l = l + (int)(mesh_time->time_state.time_zone_offset_current - 64) * 60;
    d = (int)(l / 86400);
    ...
    rtc_time->year = (uint16_t)year;
    rtc_time->month = (uint16_t)month;
    rtc_time->day = (uint16_t)day;
    rtc_time->hour = (uint16_t)hour;
    rtc_time->minute = (uint16_t)minute;
    rtc_time->second = (uint16_t)second;
}
#endif
