...
typedef enum {
    ...
    MIBLE_MESH_EVENT_PROV_CAPS,             /**< Mesh provisioning device capabilities */
    MIBLE_MESH_EVENT_PROV_END,              /**< Mesh provisioning completed */
...
/**
 * @brief Provisioning device capabilities
 */
typedef struct {
...
} mible_mesh_prov_dev_caps_t;

/**
 * @brief Provisioning End
 */
typedef struct {
    uint16_t provisioner_addr;              /**< Address of the remote provisioning server, or 0 if provisioning is by local device */
    ...
    uint8_t  dev_key[MIBLE_MESH_KEY_LEN];   /**< Device key generated during provisioning.  This key shall be used by provisioner to configure the provisioned device. */
} mible_mesh_prov_end_t;
...
typedef union {
    ...
    mible_mesh_prov_dev_caps_t dev_caps;
    mible_mesh_prov_end_t prov_end;
