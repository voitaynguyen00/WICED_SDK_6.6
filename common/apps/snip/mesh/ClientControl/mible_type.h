...
typedef struct{
    ...
    uint8_t is_variable_len;
    uint8_t rd_author;                                    // read authorization. Enabel or Disable MIBLE_GATTS_READ_PERMIT_REQ event
    uint8_t wr_author;                                    // write authorization. Enabel or Disable MIBLE_GATTS_WRITE_PERMIT_REQ event
...
typedef struct{
    ...
    uint32_t char_num;
    mible_gattc_char_db_t char_list[1];
 ...
 * */
typedef struct {
    uint8_t succ; // true: exist the specified characteristic and return correctly
