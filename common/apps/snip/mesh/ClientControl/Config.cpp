...
void CConfig::ProcessScanStatus(LPBYTE p_data, DWORD len)
{
    ...
    if (p_event == NULL)
        return;
    wsprintf(buf, L"Scan Status from %04x status:%d phase:%d max items:%d timeout:%d", p_event->src, p_data[0], p_data[1], p_data[2], p_data[3]);
    m_trace->SetCurSel(m_trace->AddString(buf));
}
...
void CConfig::OnBnClickedHeartbeatSubscriptionGet()
{
    ...
    LPBYTE p = wiced_bt_mesh_format_hci_header(dst, 0xFFFF, 0, 1, 0, 0, 0, 0, 0, buffer, sizeof(buffer));
...
void CConfig::OnBnClickedHealthFaultGet()
{
    ...
    BYTE    buffer[128];
    LPBYTE p = wiced_bt_mesh_format_hci_header(dst, app_key_idx, 0, 1, 0, 0, 0, 0, 0, buffer, sizeof(buffer));
    *p++ = company_id & 0xff;
    *p++ = (company_id >> 8) & 0xff;
...
void CConfig::OnBnClickedHealthFaultClear()
{
    ...
    BYTE    buffer[128];
    LPBYTE p = wiced_bt_mesh_format_hci_header(dst, app_key_idx, 0, reliable, 0, 0, 0, 0, 0, buffer, sizeof(buffer));
    *p++ = company_id & 0xff;
    *p++ = (company_id >> 8) & 0xff;
...
void CConfig::OnBnClickedProxyFilterTypeSet()
{
    ...
    LPBYTE p = wiced_bt_mesh_format_hci_header(0, 0xFFFF, 0, 1, 0, 0, 0, 0, 0, buffer, sizeof(buffer));
    WCHAR buf[128];
