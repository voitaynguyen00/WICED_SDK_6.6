 ...
 *
 *
 * This file shows how to create a device which allows a client to configure the location and then publishes its location.
 */
...
#define MESH_FWID               0x3014000101010001
...
#define MESH_LOCATION_SETUP_SERVICE_ELEMENT_INDEX     0
 ...
 ******************************************************/
/*
 * Initialization function should initialize model libraries used.  
 */
void mesh_app_init(wiced_bool_t is_provisioned)
{
    wiced_bt_cfg_settings.device_name = (uint8_t *)"Location";
    ...
    wiced_bt_mesh_model_location_server_init(mesh_location_server_message_handler, is_provisioned);
}

/*
 * Process event received from the Location Client.  The p_data parameter should point to the Global or Local
 * location data depending on the event type.
 */
void mesh_location_server_message_handler(uint8_t element_idx, uint16_t event, void *p_data)
{
    ...
    switch(event)
    {
    case WICED_BT_MESH_LOCATION_GLOBAL_GET:
    ...
    case WICED_BT_MESH_LOCATION_LOCAL_GET:
...
/*
 * In 2 chip solutions MCU can send commands to change location.
 */
uint32_t mesh_app_proc_rx_cmd(uint16_t opcode, uint8_t *p_data, uint32_t length)
{
    ...
    p_event = wiced_bt_mesh_create_event_from_wiced_hci(opcode, MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_GENERIC_LOCATION_SETUP_SRV, &p_data, &length);
    if (p_event == NULL)
    {
    ...
    {
    case HCI_CONTROL_MESH_COMMAND_LOCATION_GLOBAL_SET:
        STREAM_TO_UINT32(app_state.global_data.global_latitude, p_data);
        STREAM_TO_UINT32(app_state.global_data.global_longitude, p_data);
        STREAM_TO_UINT16(app_state.global_data.global_altitude, p_data);
        mesh_location_server_global_changed(p_event);
        break;

    case HCI_CONTROL_MESH_COMMAND_LOCATION_LOCAL_SET:
        STREAM_TO_UINT16(app_state.local_data.local_north, p_data);
        STREAM_TO_UINT16(app_state.local_data.local_east, p_data);
        STREAM_TO_UINT16(app_state.local_data.local_altitude, p_data);
        STREAM_TO_UINT8(app_state.local_data.floor_number, p_data);
        STREAM_TO_UINT8(app_state.local_data.is_mobile, p_data);
        STREAM_TO_UINT8(app_state.local_data.update_time, p_data);
        STREAM_TO_UINT8(app_state.local_data.precision, p_data);
        mesh_location_server_local_changed(p_event);
        break;
    }
    return WICED_TRUE;
}
...
/*
 * Send Location Global Status message
 */
void mesh_location_server_global_changed(wiced_bt_mesh_event_t *p_event)
{
    wiced_bt_mesh_model_location_server_send(WICED_BT_MESH_LOCATION_GLOBAL_STATUS, p_event, &app_state.global_data);
}

/*
 * Send Location Local Status message
 */
void mesh_location_server_local_changed(wiced_bt_mesh_event_t *p_event)
{
    WICED_BT_TRACE("local reply n:%x e:%x a:%x fl:%x mo:%x up:%x p:%x\n", app_state.local_data.local_north, app_state.local_data.local_east, app_state.local_data.local_altitude,
            app_state.local_data.floor_number, app_state.local_data.is_mobile, app_state.local_data.update_time, app_state.local_data.precision);
