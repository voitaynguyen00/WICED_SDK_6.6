...
static void mesh_light_message_handler(uint8_t element_idx, uint16_t event, void *p_data);
...
void mesh_app_init(wiced_bool_t is_provisioned)
{
    ...
    wiced_bt_mesh_model_light_hsl_server_init(MESH_LIGHT_LIGHTNESS_ELEMENT_INDEX, mesh_light_message_handler, is_provisioned);
    wiced_bt_mesh_model_light_ctl_server_init(MESH_LIGHT_LIGHTNESS_ELEMENT_INDEX, mesh_light_message_handler, is_provisioned);
    wiced_bt_mesh_model_light_xyl_server_init(MESH_LIGHT_LIGHTNESS_ELEMENT_INDEX, mesh_light_message_handler, &xy_settings, is_provisioned);

    wiced_bt_mesh_model_fw_update_server_init(MESH_LIGHT_LIGHTNESS_ELEMENT_INDEX, NULL, is_provisioned);
 ...
 */
void mesh_light_message_handler(uint8_t element_idx, uint16_t event, void *p_data)
{
#if defined HCI_CONTROL
    wiced_bt_mesh_hci_event_t *p_hci_event;
#endif

    // WICED_BT_TRACE("hsl-ctl srv msg:%d\n", event);
...
uint32_t mesh_app_proc_rx_cmd(uint16_t opcode, uint8_t *p_data, uint32_t length)
{
    ...
    case HCI_CONTROL_MESH_COMMAND_LIGHT_HSL_SET:
        model_id = WICED_BT_MESH_CORE_MODEL_ID_LIGHT_HSL_SETUP_SRV;
        break;

    case HCI_CONTROL_MESH_COMMAND_LIGHT_CTL_SET:
        model_id = WICED_BT_MESH_CORE_MODEL_ID_LIGHT_CTL_SETUP_SRV;
        break;
    ...
    p_event = wiced_bt_mesh_create_event_from_wiced_hci(opcode, MESH_COMPANY_ID_BT_SIG, model_id, &p_data, &length);
    if (p_event == NULL)
    {
...
{
    wiced_bt_mesh_light_hsl_status_data_t status;

    STREAM_TO_UINT16(status.present.lightness, p_data);
    STREAM_TO_UINT16(status.present.hue, p_data);
    STREAM_TO_UINT16(status.present.saturation, p_data);
    status.remaining_time = 0;

    wiced_bt_mesh_model_light_hsl_server_state_changed(element_idx, &status);
}
...
/*
 * Send Light CTL Status event
 */
void mesh_light_ctl_server_status_changed(uint8_t element_idx, uint8_t *p_data, uint32_t length)
{
    wiced_bt_mesh_light_ctl_status_data_t status;

    STREAM_TO_UINT16(status.present.lightness, p_data);
    STREAM_TO_UINT16(status.present.temperature, p_data);
    STREAM_TO_UINT16(status.present.delta_uv, p_data);
    status.remaining_time = 0;

    wiced_bt_mesh_model_light_ctl_server_state_changed(element_idx, &status);
}
