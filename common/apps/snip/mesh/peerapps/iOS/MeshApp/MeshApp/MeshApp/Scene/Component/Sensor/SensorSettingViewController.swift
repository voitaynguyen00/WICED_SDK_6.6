...
class SensorSettingViewController: UIViewController {
    ...
    @IBAction func onSensorSettingSetButtonClick(_ sender: UIButton) {
        ...
        print("SensorSettingViewController, onSensorSettingSetButtonClick, settingPropertyId:\(String(format: "0x%04x", settingPropertyId)) set values: \(Data(bytes: bytes).dumpHexBytes())")
        ...
        MeshFrameworkManager.shared.runHandlerWithMeshNetworkConnected { (error: Int) in
            ...
            let error = MeshFrameworkManager.shared.meshClientSensorSettingSet(componentName: componentName, propertyId: self.propertyId, settingPropertyId: settingPropertyId, value: Data(bytes: bytes))
            guard error == MeshErrorCode.MESH_SUCCESS else {
