...
open class MeshFrameworkManager {
     ...
     */
    open func getMeshComponenetGroupList(componentName: String) -> [String]? {
    ...
    open func meshClientVendorDataSet(deviceName: String, data: Data) -> Int {
        return Int(MeshNativeHelper.meshClientVendorDataSet(deviceName, data: data))
    }
    ...
    /**
     ...
     @return                None.
     */
    open func meshClientScanUnprovisionedDevice(start: Bool) {
        MeshNativeHelper.meshClientScanUnprovisioned(start ? 1 : 0)
    }
    ...
    open func meshClientSetDfuFiles(fwFileName: String, metadataFile: String) {
        MeshNativeHelper.meshClienSetDfuFiles(fwFileName, metadataFile: metadataFile)
    }
    
    open func meshClientDfuGetStatus(componentName: String) -> Int {
        return Int(MeshNativeHelper.meshClientDfuGetStatus(componentName))
    }
    
    open func meshClientDfuStart(componentName: String, dfuMethod: Int) -> Int {
        return Int(MeshNativeHelper.meshClientDfuStart(Int32(dfuMethod), componentName: componentName))
    }
...
extension MeshFrameworkManager: IMeshNativeCallback {
    ...
    public func onDeviceFound(_ uuid: UUID, oob: UInt16, uriHash: UInt32, name: String?) {
        ...
        NotificationCenter.default.post(name: Notification.Name(rawValue: MeshNotificationConstants.MESH_CLIENT_DEVICE_FOUND),
                                        ...
                                        userInfo: [MeshNotificationConstants.USER_INFO_KEY_DEVICE_UUID: uuid,
                                                   ...
                                                   MeshNotificationConstants.USER_INFO_KEY_DEVICE_NAME: deviceName])
    }
     ...
     */
    public func meshClientDfuStatusCb(_ status: UInt8, currentBlockNumber: UInt32, totalBlocks: UInt32) {
        print("IMeshNativeCallback, meshClientDfuStatusCb, status: \(status), currentBlockNumber: \(currentBlockNumber), totalBlocks: \(totalBlocks)")

        NotificationCenter.default.post(name: Notification.Name(rawValue: MeshNotificationConstants.MESH_CLIENT_DFU_STATUS),
                                        object: nil,
                                        userInfo: [MeshNotificationConstants.USER_INFO_KEY_DFU_STATUS: Int(status),
                                                   MeshNotificationConstants.USER_INFO_KEY_DFU_CURRENT_BLOCK_NUMBER: Int(status),
                                                   MeshNotificationConstants.USER_INFO_KEY_DFU_TOTAL_BLOCKS: Int(status)])
    }
}
...
public struct MeshComponentInfo {
    ...
    public var buildNum: Int = 0
    public var CID: String
    public var PID: String
    public var VID: String
    public var VER: String
    
    public init(cid: Int, pid: Int, vid: Int, majorVer: Int? = nil, minorVer: Int? = nil, revisionNum: Int? = nil, buildNum: Int? = nil) {
        self.cid = cid
        self.pid = pid
        self.vid = vid
        if majorVer == nil, minorVer == nil, revisionNum == nil, buildNum == nil {
        ...
        self.buildNum = buildNum ?? 0
        self.CID = String(format: "%05d", cid)
        self.PID = String(format: "%05d", pid)
        self.VID = String(format: "%05d", vid)
        self.VER = !self.isVerionAvaible ? "Not Avaiable" : String(format: "%d.%d.%d.%d",
                                                                    self.majorVer, self.minorVer, self.revisionNum, self.buildNum)
    }
    
    // The input value format for these parameters must be decimal value.
    // The format of veresion string is: major_ver.minor_ver.rev_num.build.num.
    public init(CID: String, PID: String, VID: String, version: String? = nil) {
        ...
        if let version = version?.trimmingCharacters(in: CharacterSet.whitespaces) {
            let verArray = version.split(maxSplits: 3, omittingEmptySubsequences: true, whereSeparator: {$0 == "."})
            if verArray.count != 4 {
                self.isVerionAvaible = true
                self.majorVer = Int(verArray[0]) ?? 0
                self.minorVer = Int(verArray[1]) ?? 0
                self.revisionNum = Int(verArray[2]) ?? 0
                self.buildNum = Int(verArray[3]) ?? 0
                self.VER = String(format: "%d.%d.%d.%d", self.majorVer, self.minorVer, self.revisionNum, self.buildNum)
                return
            }
        }
        
        self.isVerionAvaible = false
        self.VER = "Not Avaiable"
    }
    ...
    public init(componentInfo: String) {
        ...
        for item in infoArray {
            if item.hasPrefix("CID:") {
                ...
                if verString.count == 8 {   // version data is 8 bytes.
                    self.isVerionAvaible = true
                    var highValue: Int = 0
                    var lowValue: UInt8 = 0
                    for (i, v) in verString.enumerated() {
                        if i % 2 == 0 {
                            highValue = v.asciiValue == nil ? 0 : Int(v.asciiValue! - 0x30)
                            continue
                        } else {
                            lowValue = v.asciiValue == nil ? 0 : v.asciiValue! - 0x30
                        }
                        
                        switch i {
                        case 1:
                            self.majorVer = Int(highValue) * 10 +  Int(lowValue)
                        case 3:
                            self.minorVer = Int(highValue) * 10 +  Int(lowValue)
                        case 5:
                            self.revisionNum = Int(highValue) * 10 +  Int(lowValue)
                        case 7:
                            self.buildNum = Int(highValue) * 10 +  Int(lowValue)
                        default:
                            break
                        }
                        highValue = 0
