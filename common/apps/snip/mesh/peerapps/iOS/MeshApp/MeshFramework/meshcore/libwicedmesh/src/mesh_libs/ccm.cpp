...
{   uint32_t cnt;
    ...
    ctx->iv_len = (uint32_t)iv_len;
    ctx->hdr_len = hdr_len;
    ctx->msg_len = msg_len;
    ctx->tag_len = (uint32_t)tag_len;
    ctx->cnt = 0;
    ctx->txt_acnt = 0;
    ctx->hdr_lim = hdr_len;
...
{   uint32_t cnt = 0, b_pos = (uint32_t)ctx->txt_acnt & BLK_ADR_MASK;
    ...
    {
        if(b_pos == BLOCK_SIZE)
            static_cast<void>(aes_encrypt(UI8_PTR(ctx->cbc_buf), UI8_PTR(ctx->cbc_buf), ctx->aes)), b_pos = 0;
        UI8_PTR(ctx->cbc_buf)[b_pos++] ^= data[cnt++];
    }
...
ret_type ccm_encrypt_message(               /* encrypt an entire message    */
            ...
            ccm_ctx ctx[1])                 /* the mode context             */
{
    ccm_init_message(iv, iv_len, (length_t)hdr_len, (length_t)msg_len, tag_len, ctx);
