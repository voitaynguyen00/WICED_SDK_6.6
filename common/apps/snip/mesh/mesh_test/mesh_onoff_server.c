...
#if defined(CYW20719B1)
#include "20739mapb1.h"
#elif defined(CYW20819A1)
#include "20819mapa1.h"
#endif
...
void pdbg_enableFSMs(void)
{
    REG32(lhl_rtc_wake_share_adr) &= ~0x4000;
    // P34 => GPIO2 => phy_debug[3] => tx_fsm
    // P38 => GPIO3 => phy_debug[4] => rx_fsm
#if 0
    REG32(iocfg_fcn_p34_adr) = 0x010C;
    REG32(iocfg_fcn_p38_adr) = 0x00FC;
    REG32(iocfg_p34_adr) = 0x0600;
    REG32(iocfg_p38_adr) = 0x0600;
#else
    // P02 => GPIO2 => phy_debug[3] => tx_fsm
    // P06 => GPIO3 => phy_debug[4] => rx_fsm
    REG32(iocfg_fcn_p28_adr) = 0x010C;
    REG32(iocfg_fcn_p29_adr) = 0x00FC;
    REG32(iocfg_p28_adr) = 0x0600;
    REG32(iocfg_p29_adr) = 0x0600;
#endif
    REG32(di_test_ctl_adr) &= ~0x560;
    REG32(di_test_ctl_adr) |= 0x10;
    REG32(cr_smux_ctl_adr1) = 0x00001E1F;
    REG32(cr_pad_config_adr0) = (REG32(cr_pad_config_adr0) & 0x0000FFFF) | (0xB0B00000);
    REG32(cr_pad_fcn_ctl_adr0) = (REG32(cr_pad_fcn_ctl_adr0) & 0xFFFF00FF) | 0x00008800;
    REG32(cr_pad_fcn_ctl_ext_adr) &= ~(0xC000);
    REG32(di_test_ctl_adr) |= 0x80;  // 0x00318400[7] : enable LHL GPIO
}
#endif
...
//#include "fid_app.h"
    ...
    wiced_bt_mesh_core_set_trace_level((1 << 3), 0);    // FID_MESH_APP__CORE_AES_CCM_C
...
void mesh_app_network_send(void)
{
    uint8_t     buffer[20 + 380] = { 0 };
    uint16_t    dst = 0x00ff;
    wiced_bt_mesh_core_hci_cmd_network_layer_transmit_t *p = (wiced_bt_mesh_core_hci_cmd_network_layer_transmit_t*)buffer;

    p->ctl = 0;
    p->ttl = 0x1f;
    p->dst[0] = (uint8_t)(dst >> 8);
    p->dst[1] = (uint8_t)dst;
    wiced_bt_mesh_core_test_mode_signal(HCI_CONTROL_MESH_COMMAND_CORE_NETWORK_LAYER_TRNSMIT, buffer, &p->pdu[8] - buffer);
}

wiced_timer_t   mesh_app_test_timer = { 0 };
uint32_t        mesh_app_test_timer_cnt = 0;
void mesh_app_test_timer_callback(TIMER_PARAM_TYPE arg)
{
    if (mesh_app_test_timer_cnt++ == 1000)
    {
        WICED_BT_TRACE("test end\n");
        return;
    }
    wiced_start_timer(&mesh_app_test_timer, 100);
    mesh_app_network_send();
}
...
void button_interrupt_handler(void* user_data, uint8_t pin)
{
    ...
    if (value == 0)
        return;
    WICED_BT_TRACE("test start\n");
    wiced_init_timer(&mesh_app_test_timer, mesh_app_test_timer_callback, 0, WICED_MILLI_SECONDS_TIMER);
    wiced_start_timer(&mesh_app_test_timer, 100);
}
