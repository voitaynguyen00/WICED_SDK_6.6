...
void CBtWin10Interface::PostNotification(int charHandle, ComPtr<IBuffer> buffer)
{
    ...
    CSerialGattDlg *pDlg = (CSerialGattDlg *)m_NotificationContext;
    ...
    pDlg->PostMessage(WM_SERIAL_DATA, (WPARAM)BTW_GATT_OPTYPE_NOTIFICATION, (LPARAM)p);
}
...
BOOL CBtWin10Interface::Init(GUID uuidServ)
{
    ods("%s", __FUNCTION__);
    ...
    hr = mDevice->add_ConnectionStatusChanged(Callback<StatusHandler>([this](IBluetoothLEDevice *dev, IInspectable *)
    {
        ...
        if (m_state == ConnectingState && status == BluetoothConnectionStatus::BluetoothConnectionStatus_Connected)
        {
            ...
            CSerialGattDlg *pDlg = (CSerialGattDlg *)m_NotificationContext;
            pDlg->PostMessage(WM_DEVICE_CONNECTED, (WPARAM)0, (LPARAM)0);
        }
        else if (m_state == ConnectedState && status == BluetoothConnectionStatus::BluetoothConnectionStatus_Disconnected)
        {
            ...
            CSerialGattDlg *pDlg = (CSerialGattDlg *)m_NotificationContext;
            pDlg->PostMessage(WM_DEVICE_DISCONNECTED, (WPARAM)0, (LPARAM)0);
        }
        return (bool)TRUE;
...
BOOL CBtWin10Interface::GetDescriptorValue(GUID uuidChar, USHORT inx, USHORT uuidDescr, BTW_GATT_VALUE *pValue)
{
    return TRUE;
}

BOOL CBtWin10Interface::SetDescriptorValue(GUID uuidChar, USHORT inx, USHORT uuidDescr, BTW_GATT_VALUE *pValue)
{
    HRESULT hr = E_FAIL;
    hr = SetDescriptorValue(&m_guid, &uuidChar, BTW_GATT_UUID_DESCRIPTOR_CLIENT_CONFIG, pValue);
    return hr == S_OK;
}

BOOL CBtWin10Interface::ReadCharacteristic(GUID uuidChar, USHORT inx, BTW_GATT_VALUE *pValue)
{
    return FALSE;
}

BOOL CBtWin10Interface::WriteCharacteristic(GUID uuidChar, USHORT inx, BTW_GATT_VALUE *pValue)
{
    BOOL without_resp = TRUE;
    WriteCharacteristic(&m_guid, &uuidChar, without_resp, pValue);
    return FALSE;
}

UINT16 CBtWin10Interface::GetMTUSize()
{
    ComPtr<IGattDeviceService> service = getNativeService(&m_guid);
    ComPtr<IGattDeviceService3> service3;
