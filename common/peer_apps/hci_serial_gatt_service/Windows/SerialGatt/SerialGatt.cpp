...
// SerialGatt.cpp : Defines the class behaviors for the application.
//
#include "stdafx.h"
#include "btwleapis.h"
...
GUID guidSvcSerialGatt; 
GUID guidCharSerialGatt;

// CSerialGattApp

BEGIN_MESSAGE_MAP(CSerialGattApp, CWinApp)
    ON_COMMAND(ID_HELP, &CWinApp::OnHelp)
END_MESSAGE_MAP()


// CSerialGattApp construction

CSerialGattApp::CSerialGattApp()
{
}


// The one and only CSerialGattApp object

CSerialGattApp theApp;
...
BOOL CSerialGattApp::InitInstance()
{
    ...
    if (dlg.m_bWin10)
    {
        ...
        guidSvcSerialGatt = GUID_SERIAL_GATT_SERVICE;
        guidCharSerialGatt = GUID_SERIAL_GATT_SERIAL_DATA;
    }
    else if (dlg.m_bWin8)
    {
        dlg.m_bWin8 = TRUE;
    ...
    else if (IsOSWin7())
    {
        ...
        {
            MessageBox(NULL, L"Failed to load BTWLeApi.Dll", L"Error", MB_OK);
            return FALSE;
        }
        HANDLE h = CreateFile (_T("\\\\.\\BTWAMPFL"), GENERIC_WRITE, FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
        ...
        BtwGuidFromGuid(&guidSvcSerialGatt, &GUID_SERIAL_GATT_SERVICE);
        BtwGuidFromGuid(&guidCharSerialGatt, &GUID_SERIAL_GATT_SERIAL_DATA);
    }
    else
    {
