...
{
    ods ("Connected:%d\n", bConnected);
    PostMessage(((CSerialGattDlg *)pRefData)->m_hWnd, WM_CONNECTED, (WPARAM)bConnected, NULL); 
}
...
)
{
    ...
    if (*pGuidService != guidSvcSerialGatt)
    {
    ...
    *p = *pValue;
    PostMessage(((CSerialGattDlg *)pRefData)->m_hWnd, WM_SERIAL_DATA, (WPARAM)dwCharInstance, (LPARAM)p);
    return ERROR_SUCCESS;
}
