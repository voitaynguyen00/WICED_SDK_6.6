...
{
    CSerialGattDlg *pDlg = (CSerialGattDlg *)Context;
	...
	pDlg->PostMessage(WM_SERIAL_DATA, (WPARAM)inx, (LPARAM)pValue);
        }
    }
}
...
BOOL CBtWin8Interface::Init(GUID uuidServ)
{
    ...
    m_ppdescr = (BTH_LE_GATT_DESCRIPTOR **)malloc(sizeof (BTH_LE_GATT_DESCRIPTOR *) * num_chars);
...
void CBtWin8Interface::RegisterNotification(GUID uuidChar, USHORT inx)
{
...
BOOL CBtWin8Interface::WriteCharacteristic(GUID uuidChar, USHORT inx, BTW_GATT_VALUE *pValue)
{
    ...
    BYTE *buf = new BYTE [sizeof (BTH_LE_GATT_CHARACTERISTIC_VALUE)+pValue->len];
    if (buf)
    {
        ...
        pWriteChar(hService, &bleChar, Value, NULL, BLUETOOTH_GATT_FLAG_WRITE_WITHOUT_RESPONSE);
        delete[] buf;
    }
