...
void CBtWin10Interface::PostNotification(int charHandle, ComPtr<IBuffer> buffer)
{
    ...
    CWsOtaUpgradeDlg *pDlg = (CWsOtaUpgradeDlg *)m_NotificationContext;
...
BOOL CBtWin10Interface::CheckForOTAServices()
{
    ComPtr<IGattDeviceService> otaService = getNativeService(&GUID_OTA_FW_UPGRADE_SERVICE);
    ComPtr<IGattDeviceService> otaSecService = getNativeService(&GUID_OTA_SEC_FW_UPGRADE_SERVICE);
...
UINT16 CBtWin10Interface::GetMTUSize()
{
    ...
    if (otaService)
        hr = otaService.As(&service3);
    else if (otaSecService)
        hr = otaSecService.As(&service3);
    else
        return 0;
    ASSERT_SUCCEEDED(hr);
