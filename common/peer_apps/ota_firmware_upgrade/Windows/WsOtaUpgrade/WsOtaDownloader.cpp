...
void DumpData(char *description, void* p, uint32_t length, uint32_t max_lines)
{
    char    buff[100];
    UINT    i, j;
    char    full_buff[3000];
...
void WSDownloader::TransferData()
{
    ...
    while ((m_PatchSize > m_offset) && (m_state != WS_UPGRADE_STATE_ABORTED))
    {
        ...
        dwBytes = (dwBytes > m_connMTU) ? m_connMTU : dwBytes;
...
void WSDownloader::ProcessEvent(BYTE Event)
{
    ...
    case WS_UPGRADE_STATE_IDLE:
        if (Event == WS_UPGRADE_CONNECTED)
        {
            ...
            m_btInterface->SendWsUpgradeCommand(WICED_OTA_UPGRADE_COMMAND_PREPARE_DOWNLOAD);
