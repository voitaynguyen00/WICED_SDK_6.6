...
print "---------- Running LE Characteristics Discovery Test ----------\n"
...
# discover Characteristics of peer device
print "discover Characteristics of peer device\n"
if( wiced.CommandGattDiscoverCharacteristics( connection_handle, 1, 0xffff ) != "success" ):
    print "fail - discover Characteristics of peer device\n"
    end_test( wiced, test_result )
...
# Get LE status
print "Waiting for discover Characteristics data\n"
characteristic_discovered = 0
for i in range( 10 ):
    characteristic_uuid128 = [0] * 16
    if( wiced.getWait( "EventGattCharacteristicDiscovered", 1 ) == "success" ):
        characteristic_discovered = characteristic_discovered + 1

        # Get params from EventGattCharacteristicDiscovered
        length = wiced.getData( "Length" )
        connection_handle = wiced.getData( "connection_handle" )
        characteristic_handle = wiced.getData( "characteristic_handle" )
        value_handle = wiced.getData( "value_handle" )
        characteristic_property = wiced.getData( "characteristic_property" )
        
        if( length == 9 ):
            characteristic_uuid = wiced.getData( "characteristic_uuid[0]" ) + ( wiced.getData( "characteristic_uuid[1]" ) << 8 )
        else:
            for i in range(16):
                characteristic_uuid128[ 15 - i ] = wiced.getData( "characteristic_uuid[%d]" % i )

        print "--EventGattCharacteristicDiscovered, characteristics: #%x--" % characteristic_discovered
        print "  [length                  = 0x%04X]" % length
        print "  [connection_handle       = 0x%04X]" % connection_handle
        print "  [characteristic_handle   = 0x%04X]" % characteristic_handle
        print "  [value_handle            = 0x%04X]" % value_handle
        print "  [characteristic_property = 0x%02X]" % characteristic_property
        
        if( length == 9 ):
            print "  [characteristic_uuid     = 0x%04X]\n" % characteristic_uuid
        else:
            print "  [characteristic_uuid     = 0x%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X]\n" % \
                                                ( characteristic_uuid128[0],  characteristic_uuid128[1],  characteristic_uuid128[2],  characteristic_uuid128[3], \
                                                  characteristic_uuid128[4],  characteristic_uuid128[5],  characteristic_uuid128[6],  characteristic_uuid128[7], \
                                                  characteristic_uuid128[8],  characteristic_uuid128[9],  characteristic_uuid128[10], characteristic_uuid128[11], \
                                                  characteristic_uuid128[12], characteristic_uuid128[13], characteristic_uuid128[14], characteristic_uuid128[15] )

    else:
        if (characteristic_discovered == 0):
            print "fail - Waiting for EventGattCharacteristicDiscovered\n"
            end_test( wiced, test_result )
        else:
            test_result = "PASS"
            break
