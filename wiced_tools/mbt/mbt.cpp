...
static int execute_le_receiver_test(char *szPort, UINT8 chan_number)
{
    ...
    BOOL  res;
    UINT8 hci_le_receiver_test[] = {0x1D, 0x20, 0x01, 0x00};
    UINT8 hci_le_receiver_test_cmd_complete_event[] = {0x0e, 0x04, 0x01, 0x1D, 0x20, 0x00};
...
static int execute_le_transmitter_test(char *szPort, UINT8 chan_number, UINT8 length, UINT8 pattern)
{
    ...
    BOOL res;
    UINT8 hci_le_transmitter_test[] = {0x1E, 0x20, 0x03, 0x00, 0x00, 0x00};
    UINT8 hci_le_transmitter_test_cmd_complete_event[] = {0x0e, 0x04, 0x01, 0x1E, 0x20, 0x00};
...
{
    printf ("Usage: mbt le_enhanced_receiver_test <rx_channel> <PHY> <modulation_index>\n");
    if (!full)
        return;
    printf ("                rx_channel = (F - 2402) / 2\n");
    printf ("                    Range: 0 - 39. Frequency Range : 2402 MHz to 2480 MHz\n");
...
static int execute_le_enhanced_receiver_test(char *szPort, UINT8 chan_number, UINT8 phy, UINT8 modulation_index)
{
    TransportHelper *pTransport = (mbt_transport_type == TRANSPORT_TYPE_COM) ? (TransportHelper *)new ComHelper : (TransportHelper *)new UsbHelper;
    if (!pTransport->OpenPort(szPort))
    {
    ...
    BOOL res;
	UINT8 hci_le_enhanced_receiver_test[] = { 0x33, 0x20, 0x03, 0x00, 0x00, 0x00 };
    UINT8 hci_le_enhanced_receiver_test_cmd_complete_event[] = {0x0e, 0x04, 0x01, 0x33, 0x20, 0x00};
    hci_le_enhanced_receiver_test[3] = chan_number;
    hci_le_enhanced_receiver_test[4] = phy;
    hci_le_enhanced_receiver_test[5] = modulation_index;

    res = send_hci_command(pTransport, hci_le_enhanced_receiver_test, sizeof(hci_le_enhanced_receiver_test), hci_le_enhanced_receiver_test_cmd_complete_event, sizeof(hci_le_enhanced_receiver_test_cmd_complete_event));
...
{
    printf ("Usage: mbt le_enhanced_transmitter_test <tx_channel> <data_length> <data_pattern> <PHY>\n");
    if (!full)
        return;
    printf ("                tx_channel = (F - 2402) / 2\n");
    printf ("                    Range: 0 - 39. Frequency Range : 2402 MHz to 2480 MHz\n");
...
static int execute_le_enhanced_transmitter_test(char *szPort, UINT8 chan_number, UINT8 length, UINT8 pattern, UINT8 phy)
{
    TransportHelper *pTransport = (mbt_transport_type == TRANSPORT_TYPE_COM) ? (TransportHelper *)new ComHelper : (TransportHelper *)new UsbHelper;
    if (!pTransport->OpenPort(szPort))
    {
    ...
    BOOL res;
    UINT8 hci_le_enhanced_transmitter_test[] = {0x34, 0x20, 0x04, 0x00, 0x00, 0x00, 0x00};
    UINT8 hci_le_enhanced_transmitter_test_cmd_complete_event[] = {0x0e, 0x04, 0x01, 0x34, 0x20, 0x00};
    hci_le_enhanced_transmitter_test[3] = chan_number;
    hci_le_enhanced_transmitter_test[4] = length;
    hci_le_enhanced_transmitter_test[5] = pattern;
    hci_le_enhanced_transmitter_test[6] = phy;

    res = send_hci_command(pTransport, hci_le_enhanced_transmitter_test, sizeof(hci_le_enhanced_transmitter_test), hci_le_enhanced_transmitter_test_cmd_complete_event, sizeof(hci_le_enhanced_transmitter_test_cmd_complete_event));
    if (!res)
...
int _tmain(int argc, _TCHAR* argv[])
{
    ...
    else if ((argc >= 2) && (_stricmp(argv[1], "le_enhanced_transmitter_test") == 0))
    {
        if (argc == 6)
        {
            chan_num = atoi(argv[2]);
            if ((chan_num >= 0) && (chan_num <= 39))
            {
                ...
                if ((length > 0) && (chan_num <= 255))
                {
                    ...
                    if ((pattern >= 0) && (pattern < 7))
                    {
                        ...
                        if ((phy == 1) || (phy == 2))
                        {
                            return (execute_le_enhanced_transmitter_test(mbt_transport_name, chan_num, length, pattern, phy));                        
                        }
                    }
                }
            }
    ...
    else if ((argc >= 2) && (_stricmp(argv[1], "le_enhanced_receiver_test") == 0))
    {
        if (argc == 5)
        {
            chan_num = atoi(argv[2]);
            if ((chan_num >= 0) && (chan_num <= 39))
            {
                ...
                if ((phy == 1) || (phy == 2))
                {
                    ...
                    if ((index == 1) || (index == 2))
                    {
                        return (execute_le_enhanced_receiver_test(mbt_transport_name, chan_num, phy, index));
                    }                        
                }
            }
        }
                        ...
                        if ((logical_channel >= 0) && (logical_channel <= 1))
                        {
                             ...
                             if ((bb_packet_type >= 3) && (bb_packet_type <= 15))
                             {
                                 ...
                                 if ((packet_length >= 0) && (packet_length <= 0xffff))
                                 {
                                     ...
                                     if ((tx_power >= -25) && (tx_power <= 3))
                                     {
                                         return execute_radio_tx_test(mbt_transport_name, argv[2], frequency, modulation_type_mapping[modulation_type], logical_channel, bb_packet_type, packet_length, tx_power);
                                     }
                                 }
                             }
                        }
                    ...
                    if ((modulation_type >= 0) && (modulation_type <= 4))
                    {
                        ...
                        if ((logical_channel >= 0) && (logical_channel <= 1))
                        {
                            ...
                            if ((bb_packet_type >= 3) && (bb_packet_type <= 15))
                            {
                                ...
                                if ((packet_length >= 0) && (packet_length <= 0xffff))
                                {
                                    return execute_radio_rx_test(mbt_transport_name, argv[2], frequency, modulation_type_mapping[modulation_type], logical_channel, bb_packet_type, packet_length);
                                }
                            }
                        }
                    }
